...
class AuthorizingHTTPService(HTTPService):
    ...
    async def with_renew_token_and_retry(self, func: Callable) -> Response:
        """
        This will call the passed function, and check if the response is a
        401 with 'Signature has expired' as message. If this is the case,
        the JWT token will be renewed and the call will be repeated.
        In both cases the response is the response of the Callable.
    ...
    def _construct_auth_url_from_application_id(cls, api_secret: str) -> str:
        """
        Takes the first part of the api_secret (app_id:app_secret),
        which contains the Application ID, and constructs the
        auth url for Cognito.
