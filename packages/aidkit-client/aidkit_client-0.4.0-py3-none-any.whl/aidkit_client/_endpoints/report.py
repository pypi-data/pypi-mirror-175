...
class ReportAPI:
    ...
    async def get_adversarial_example_details(
        self, adversarial_example_id: int
    ) -> ReportCoreMethodOutputDetailResponse:
        result = await self.api.get(
            path=f"{Constants.REPORT_PATH}/artifact_details/{adversarial_example_id}",
            parameters=None,
        )
        return ReportCoreMethodOutputDetailResponse(
            **result.body_dict_or_error(
                f"Error fetching inference results for remote file '{adversarial_example_id}'."
            )
        )
