...
class lsvc():# linear svc
    def __init__(self,penalty='l2', C=1.0, *args, **kwargs):
        self.penalty = penalty
        self.C = C
        self.kwargs = kwargs
        self.svmModel = LinearSVC(penalty=penalty, C = C, **kwargs)
    
    def fit(self,X, y):
        self.X = X
        self.y = y
        self.classes = np.unique(y)
        self.svmModel.fit(X,y)
        return self
    ...
    def decision_function(self, X):
        return self.svmModel.decision_function(X)
    
    def crossValidation_predict(self, RangeC = None, nfold = 10):
        X = self.X
        y = self.y
        if RangeC is None:
            RangeC = np.logspace(-4,5, 20)
        self.RangeC = RangeC
        yhat = np.zeros((y.shape[0],len(RangeC)))
        for i in range(len(RangeC)):
            model = lsvc(penalty=self.penalty, C = RangeC[i], **self.kwargs)
            for train, test in KFold(n_splits=nfold).split(X):
                model.fit(X[train,:], y[train])
                yhat[test,i] = model.predict(X[test,:])
        return yhat
    
    def get_optC(self,  RangeC = None, nfold = 10):
        yhat_cv = self.crossValidation_predict(RangeC = RangeC, nfold = nfold)
        accuracy_cv = []
        for i in range(yhat_cv.shape[1]):
            if len(self.classes) == 2 :
                report_cv = binaryClassificationReport(self.y, yhat_cv[:,i])
        ...
        optC = self.RangeC[np.argmax(accuracy_cv)]
        self.optC = optC
        return optC
