...
def train_val_split(params, data, df, labels=None):
    ...
    if params["val_split_timeseries"]:
        ...
        if "validation_test_split" in params and params["validation_test_split"]:
            ...
            date_time_diff = today - timedelta(days=params["data_set_initial_date"])

            train_split_date = date_time_diff + timedelta(days=params["train_days"])
            val_split_date = pd.to_datetime(train_split_date) + timedelta(
                days=params["val_days"]
            )

            ind_test = df[
                ["shipment_id", "upload_date", "service_completion_estimated_date"]
            ].sort_values("upload_date").groupby("shipment_id").last()[
                "service_completion_estimated_date"
            ] > pd.to_datetime(
                val_split_date, utc=True, infer_datetime_format=True
            )
            data["id_test"] = ind_test[ind_test].index

        else:
            train_split_date = datetime.today() - timedelta(
                days=params["val_days"]
            )  # 150 days
        ...
        ind_train = df[
            ...
            train_split_date, utc=True, infer_datetime_format=True
        )
    ...
    if "validation_test_split" in params and params["validation_test_split"]:
        ...
        data["id_val"] = list(
            set(data["id_all"]) - set(data["id_train"]) - set(data["id_test"])
        )
