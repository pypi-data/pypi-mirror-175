...
class PyTorchForecastingDataset(TimeSeriesDataSet, BaseDataset):
    ...
    def __init__(self, data, group_ids, time_idx, target,
        ...
        self.predict_mode = predict_mode
        super().__init__(data=data, time_idx=time_idx, target=target,
                         group_ids=group_ids,
    ...
    def _construct_index(self, data, *args, **kwargs):
        df_index = super()._construct_index(data=data, predict_mode=False)

        # Custom behaviour of `predict_mode` param.
        if self.predict_mode:
            # Get the rows containing the max sequence length of their group.
            # Note that if a group has multiple max values, all will be
            # returned.
