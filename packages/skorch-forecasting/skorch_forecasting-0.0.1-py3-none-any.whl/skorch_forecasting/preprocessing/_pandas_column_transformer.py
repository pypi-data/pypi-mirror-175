...
from ..exceptions import MissingFeaturesInInverseTransformation
from ..utils.data import empty_ndarray, safe_hstack


def _make_sklearn_column_transformer(transformers):
...
class PandasInverseTransformer:
    ...
    def _to_pandas(self, inverse_transforms, pandas_columns):
        ...
        dtypes = {col: dtype for col, dtype in dtypes.items() if
                  col in columns}
        arr = safe_hstack(inverse_transforms)
        return pd.DataFrame(arr, columns=columns).astype(dtypes)
    ...
    def _check_missing_features(self, X, name, trans, features):
        ...
        if missing_features:
            ...
            raise MissingFeaturesInInverseTransformation(
                name=name,
