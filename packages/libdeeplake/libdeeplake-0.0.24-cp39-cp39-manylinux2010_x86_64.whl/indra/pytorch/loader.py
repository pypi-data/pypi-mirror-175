        ...
        while 1:
            ...
            if i < len(dataloader):
                ...
                if self.compressed_tensors:
                    bytes_tensor, batch = compressed_bytes_to_tensor(batch, self.compressed_tensors)
                else:
                    bytes_tensor = None
                batch = (bytes_tensor, batch)
    ...
    while 1:
        try:
            ...
            if batch is None:
                ...
                break
            bytes_tensor, batch = batch
            if bytes_tensor is not None:
                batch = bytes_tensor_to_batch(batch, compressed_tensors, bytes_tensor)
...
def compressed_bytes_to_tensor(batch, compressed_tensors):
    sb, eb, all_bts = 0, 0 , []
    for sample in batch:
    ...
    bytes_tensor = torch.frombuffer(all_bts, dtype=torch.uint8)
    return bytes_tensor, batch

def bytes_tensor_to_batch(batch, compressed_tensors, bytes_tensor):
    data_bytes = memoryview(bytes_tensor.numpy().tobytes())
    for sample in batch:
        for tensor in compressed_tensors:
            sb, eb = sample[tensor]
