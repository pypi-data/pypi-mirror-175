...
def compose_circular_key_frames(thetas, X_WorldCenter, X_WorldGripper_init):
    key_frame_poses_in_world = [X_WorldGripper_init]
    x_WorldCenter, y_WorldCenter, z_WorldCenter = X_WorldCenter.translation()
    for theta in thetas:
        this_x = x_WorldCenter + np.cos(theta) * radius
        this_y = y_WorldCenter + np.sin(theta) * radius
        this_R = RotationMatrix.MakeYRotation(-theta)
        this_R = X_WorldCenter.rotation().multiply(this_R)
        new_point = np.array([this_x, this_y, z_WorldCenter])
        new_T = RigidTransform(p=new_point, R=this_R)
        key_frame_poses_in_world.append(new_T)
