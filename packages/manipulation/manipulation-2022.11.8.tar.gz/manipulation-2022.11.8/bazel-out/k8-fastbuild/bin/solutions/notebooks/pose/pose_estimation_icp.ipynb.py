...
def segment_scene_pcl(scene_pcl_np, scene_rgb_np, scene_pcl_np_background,
    ...
    def filter_red_color(rgb):
        r, g, b = rgb.T
        if (r > 10) and (g < 125) and (b < 125):
            return True

    relevant_pts = []
    for i in range(scene_pcl_np.shape[1]):
        if (filter_red_color(scene_rgb_np[:, i])):
            # Come up with a proper solution after I've had some sleep....
            if (scene_pcl_np[0, i] < 0) and (scene_pcl_np[0, i] > -0.5):
                relevant_pts.append(i)

    scene_pcl_np_filtered = scene_pcl_np[:, relevant_pts]
    return scene_pcl_np_filtered
