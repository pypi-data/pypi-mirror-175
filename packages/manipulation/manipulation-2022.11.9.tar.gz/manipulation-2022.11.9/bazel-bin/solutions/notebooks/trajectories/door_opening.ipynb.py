...
def create_q_knots(pose_lst):
  ...
  for i in range(len(pose_lst)):
    ...
    AddOrientationConstraint(ik, pose_lst[i].rotation(), 0.01 * np.pi)
    AddPositionConstraint(ik, pose_lst[i].translation() - [0.0, 0.0, 0.02],
                              pose_lst[i].translation() + [0.0, 0.0, 0.02])
    
    error = q_variables - q_nominal
    prog.AddCost(error.dot(error)) # Add joint centering cost. 
