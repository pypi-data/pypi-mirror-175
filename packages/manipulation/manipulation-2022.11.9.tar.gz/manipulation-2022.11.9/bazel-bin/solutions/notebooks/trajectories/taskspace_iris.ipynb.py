...
def ClosestPointOnObstacle(C, C_inv, d, o):
    ...
    x_tilde = prog.NewContinuousVariables(n)
    w = prog.NewContinuousVariables(m)
    prog.AddCost(x_tilde.dot(x_tilde))
    prog.AddConstraint(eq(x_tilde, v_tildes @ w))
    prog.AddConstraint(eq(w @ np.ones(m), np.ones(m)))
    prog.AddConstraint(ge(w, np.zeros(m)))
    
    result = Solve(prog)
    
    x_tilde_star = result.GetSolution(x_tilde)
    dist = np.sqrt(result.get_optimal_cost())
