...
def deproject_pW_to_image(p_W_mustard, cx, cy, fx, fy, X_WC):
    ...
    Input:
        ...
        - X_WC: camera pose in the world frame 
    Output:
        - mask: numpy int array (nx2)
    """

    p_C_mustard = X_WC.inverse().multiply(p_W_mustard.T).T

    uv = []
    for pnt in p_C_mustard:
        Xc, Yc, Zc = pnt
        u = Xc*fx/Zc + cx
        v = Yc*fy/Zc + cy
        uv.append([u,v])
    uv = np.asarray(uv).round().astype(int)
...
# **Solution**: 
# 
# (1) No, the resultant mask will include both the mustard bottle and the Cheez-It box. 
# 
# (2) No, the resultant mask will include both the mustard bottle and the Cheez-It box.
...
def augment_mustard_bottle(rgb_im, mask):
    ...
    """
    from copy import deepcopy
    new_img = deepcopy(rgb_im)
    new_mask = deepcopy(mask)
    # flip
    for i in range(3):
        new_img[:,:,i] = np.fliplr(new_img[:,:,i])
    new_mask = np.fliplr(new_mask)
    
    # add noise
    gauss = np.random.normal(0,10,(480, 640, 3)).astype(int)
