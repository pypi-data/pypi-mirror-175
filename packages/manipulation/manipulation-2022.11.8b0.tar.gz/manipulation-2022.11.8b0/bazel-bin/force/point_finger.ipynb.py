...
from pydrake.all import (AddMultibodyPlantSceneGraph, Box,
                         ...
                         MultibodyPositionToGeometryPose, Multiplexer,
...
#        vis.vis['drake']['4'].delete()  # Only flush the gripper geometry.
    ...
    plt.legend(lines, [("$f^{F_c}_{desired}$ = " + str(f)) for f in desired_horizontal_forces])
    plt.xlabel('t (sec)')
    plt.ylabel('x (m)')
...
def force_control_point_finger(teleop=True):
    ...
    if teleop:
        meshcat.AddSlider(
            "desired box angle", value=0, min=0, max=np.pi/2, step=0.05, increment_keycode="ArrowLeft", decrement_keycode="ArrowRight")
