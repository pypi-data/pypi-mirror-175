...
class Likelihood2DPlot(AbstractPlot):
    ...
    def draw(self, xattrib: str, yattrib: str, zattrib: str = 'qmu', xlabel: Optional[str] = "",
        ...
        if draw_bestfit and isinstance(self.data_map, pd.DataFrame):
            bestfit_idx = self.data_map[[zattrib]].idxmin()
            bestfit_x   = self.data_map[xattrib].iloc[bestfit_idx].values[0]
            bestfit_y   = self.data_map[yattrib].iloc[bestfit_idx].values[0]
            highlight_data = {
                'x': bestfit_x,
                'y': bestfit_y,
