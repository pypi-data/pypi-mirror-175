...
def extract_ids(ids):
    #TODO from publications stats script
    #self.ID_to_PubMedID_NCBI(pmc_ids)
    #self.ID_to_PubMedID_NCBI(doi_to_pubs)

    return

def complete_ids(unique_entry_ids, new_entry):
    # TODO
    if sum([bool(unique_entry_ids.get(key)) for key in unique_entry_ids]) != 3:
        # populate
        pass
                    ...
                    for unique_entry in unique_pubs_by_id.keys():
                        for id_type in ids_present:
                            if publication[id_type] in unique_entry['ids']:
                                unique_entry['ids'] = complete_ids(unique_entry['ids'], publication)
...
def worker_integration(name, groupInst, return_dict):
    return_dict[name] = []
...
def create_integrated_instances(groupInst):
    ...
    for name in groupInst.keys():
        ...
        return_dict = worker_integration(name, groupInst, return_dict)
    return(return_dict)
