...
def collect_pretools_instances(pretools, STORAGE_MODE):
    ...
    ptrools: either a 'pymongo.collection.Collection' or a list of dictionaries
    """
    ...
    for source in sources:
        ...
        if os.getenv(source, 'True') == 'True':
            #print("Collecting instances from %s"%(source))
            ...
            if STORAGE_MODE=='db':
                ...
                insts_iter = [tool_dict for tool_dict in pretools if tool_dict['source'] == [sources_labels[source]]]
            ...
            print(f"Entries from {source}: {len(insts)}")
            print(f"Generic tools from {source}: {len(names)}")
...
def run_integration():
    ...
    if STORAGE_MODE == 'db':
        ...
        pretools = connect_collection(PRETOOLS)
        # integrated collection of instances (output instances)
        tools = connect_collection(TOOLS)
    else:
    ...
    ## Create integration intances. Returns a dictionary of instances by tool name.
    inst_name_dict = create_integrated_instances(pre_integration_dict, tools)
 
    # save to file
    # save_insts_by_name(inst_name_dict, 'inst_name_dict_2.json')
