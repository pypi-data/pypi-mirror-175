...
def naive_with_mismatch (sequence,pattern,k):
	positions = []
	for i in range(0, len(sequence) - len(pattern) + 1):
		if sequence[i:i+len(pattern)] == pattern:
			positions.append(i)
		elif hamming_dist (sequence[i:i+len(pattern)],pattern) <= k:
			positions.append(i)
		elif reverse_complement (pattern) == sequence[i:i+len(pattern)]:
			positions.append(i)
		elif hamming_dist (pattern, reverse_complement (sequence[i:i+len(pattern)])) <= k:
			positions.append(i) 
	return positions
