...
def register_data(geno_binary_file_prefix: str, data_path: str) -> bool:
    ...
    if study_type == "SFGWAS":
        ...
        assert num_inds == doc_ref_dict["personal_parameters"][email]["NUM_INDS"]["value"]
        num_snps = num_rows(os.path.join(data_path, "snp_ids.txt"))
        assert num_snps == doc_ref_dict["personal_parameters"][email]["NUM_SNPS"]["value"]
        ...
        assert number_of_rows == doc_ref_dict["personal_parameters"][email]["NUM_INDS"]["value"]
        number_of_cols = num_cols(os.path.join(data_path, "data.txt"))
        assert number_of_cols == doc_ref_dict["parameters"]["num_columns"]["value"]
