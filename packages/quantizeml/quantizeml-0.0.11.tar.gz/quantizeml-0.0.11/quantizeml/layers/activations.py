...
class QuantizedGELU(Calibrable, keras.layers.Layer):
    ...
    def __init__(self, *args, name="i_gelu", quant_config={}, **kwargs):
        ...
        self._a_int = FixedPoint.quantize(-0.2888, 11, self.buffer_bitwidth)
        self._b_int = FixedPoint.quantize(-1.769, 16, self.buffer_bitwidth)
        self._c_int = FixedPoint.quantize(1.0, 0, self.buffer_bitwidth)
        # Coefficients used in i_gelu function
        self._s2_int = FixedPoint.quantize(1.0 / 1.4142, 6, self.buffer_bitwidth)
        self.intermediate_quantizer = deserialize_quant_object(
            self.quant_config, "intermediate_quantizer", False, True)
