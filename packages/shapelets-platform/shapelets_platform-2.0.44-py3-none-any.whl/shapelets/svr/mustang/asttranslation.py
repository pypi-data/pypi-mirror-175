...
def create_extractors(tree, globals, locals, special_functions, const_functions, outer_names=()):
    ...
    for node in pretranslator.externals:
        ...
        src, left_or_right_join = check_left_right_join(src)
    ...
    for child in children:
        if isinstance(child, ast.comprehension):
            src_node, left_or_right_join = check_left_right_join(child.iter.src)
            if (left_or_right_join is not None):
                lr_join = left_or_right_join
            child.iter.src = src_node
   
        
    result = tree, extractors, lr_join

    return result

def check_left_right_join(node):
    leftJoin = node.startswith('left(')
    rightJoin = node.startswith('right(')
    left_or_right_join = None
    if (leftJoin or rightJoin):
        ds = node.replace('(', '_')
        ds = ds.replace(')', '')
        x = ds.split("_")
        node = x[1]
        left_or_right_join = 'left' if leftJoin else 'right'
    
    return node, left_or_right_join   
