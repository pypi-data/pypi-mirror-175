...
For each user (id is wallet) and _type, system push notifications to the user.
...
async def push(wallet: str, _type: str, message: str, timestamp: Any = None, ttl: int = 3600000):
    """TTL in milli-seconds
    """
    if timestamp is None:
        timestamp = int(time())
    await redis_client.setex(
        f"{wallet}:{_type}:{timestamp}", ttl, message
    )


async def pop(wallet: str, _type: str, delete:bool=True) -> List[Dict[str, str]]:
    """Return notification for this user, ordered by timestamp
    If not exist, return empty list.

    when _type is set to None, it will get all notifications for a wallet.
    """
    search_key = f"{wallet}:"
    ...
    if not keys:
        ...
        for obj_key in keys:
            if delete:
                result.append((await redis_client.getdel(obj_key)).decode('utf8'))
            else:
                result.append((await redis_client.get(obj_key)).decode('utf8'))
        return result
...
async def _local_test():
    ...
    assert ['hello222', 'hello231', 'hello221', 'hello232']==(await pop('3', None, delete=False)) # order by type first.
