    ...
    if create_directories:
        with temp_directory(create_directories, basepath=directory):
            with _write_or_touch(
                file_path, contents=contents, strip_first_newline=strip_first_newline
            ):
                yield file_path
    else:
        with _write_or_touch(
...
def _cast_to_list_of_existing_pathlib_paths(
    ...
    return cast(List[pathlib.Path], paths)
...
class SaltEnv:
    ...
    name = attr.ib()
    paths = attr.ib(
        default=attr.Factory(list), converter=_cast_to_list_of_existing_pathlib_paths
    )
...
def _cast_to_saltenv(
    envs: Dict[str, List[Union[str, pathlib.Path]]]
) -> Dict[str, SaltEnv]:
    _envs = {}
    for envname, envtree in envs.items():
        if not isinstance(envtree, SaltEnv):
            if isinstance(envtree, str):
                envtree = [envtree]
            _envs[envname] = SaltEnv(name=envname, paths=envtree)
    return _envs
...
class SaltEnvs:
    ...
    envs = attr.ib(converter=_cast_to_saltenv)

    def __attrs_post_init__(self) -> None:
        """
        Post attrs initialization routines.
        """
        for envname, saltenv in self.envs.items():
            setattr(self, envname, saltenv)
