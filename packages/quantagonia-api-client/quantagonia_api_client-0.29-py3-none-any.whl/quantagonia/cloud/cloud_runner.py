...
class CloudRunner(Runner):
    ...
    def _solveParseArgs(self, batch_size : int, **kwargs):
        ...
        return poll_frequency, timeout, solver_logs, new_incumbent_callback

    def waitForJob(self, jobid: int, poll_frequency: float, timeout: float, solver_logs: list, batch_size : int, new_incumbent_callback = None) -> JobStatus:
    ...
    async def waitForJobAsync(self, jobid: int, poll_frequency: float, timeout: float, solver_logs: list, batch_size : int, new_incumbent_callback = None) -> JobStatus:
        ...
        for t in range(0,int(timeout/poll_frequency)):
            ...
            if status == JobStatus.finished:
                ...
                if new_incumbent_callback is not None:
                    try:
                        batch_solutions = await self.https_client.getCurrentSolution(jobid=jobid)
                    except RuntimeError as runtime_e:
                        sys.exit(f"{self._error_symbol}: " + str(runtime_e))
    ...
    def solveBatched(self, problem_files: list, specs: list, **kwargs):
        ...
        poll_frequency, timeout, solver_logs, new_incumbent_callback = self._solveParseArgs(batch_size, **kwargs)
