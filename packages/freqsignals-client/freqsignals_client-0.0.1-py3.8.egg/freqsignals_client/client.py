...
class FreqSignalsClient:
    ...
    def make_request(self, url, method='get', data=None, headers=None, remaining_attempts=1):
        ...
        while data_response is None and remaining_attempts > 0:
            try:
                ...
                if method == 'get':
                    response = requests.get(self.get_full_url(url), timeout=self._request_timeout, headers=headers, verify=self._https)
                elif method == 'post':
                    response = requests.post(self.get_full_url(url), timeout=self._request_timeout, json=data, headers=headers, verify=self._https)
                else:
                ...
                if response.status_code != 200:
                    ...
                    if response.text:
                        raise FreqSignalsError("bad return status code: {} - {}".format(response.status_code, response.text))
                    else:
    ...
    def get(self, url):
        return self.make_request("get", url, headers=self.get_headers())

    def post(self, url, data):
        return self.make_request("post", url, data=data, headers=self.get_headers())
