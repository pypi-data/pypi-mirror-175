...
def __save_database_profile(profile_js, layer):
    if layer == "bronze":
    ...
    fields = profile_js["profilling"]["descriptiveStatistics"]["fields"]
    df_default = spark.createDataFrame(pd.json_normalize(fields))
    df_default.write.option("header", "true").format('delta').mode('append').option("mergeSchema","true").saveAsTable(table)
...
def save_database_profile_history(quality_js, profiling_s3_bucket):
    ...
    df = spark.createDataFrame(pd.json_normalize(fields))
