...
exports.demoStyles = lit_element_1.css `
  ...
  svg {
   ...
   mwc-icon-button,
    ...
    mwc-icon-button-toggle[selected] {
        color: black;
    }
  .icons {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.arraysMatch = exports.distinctColors = exports.getDensePolysExtrema = exports.getPixelId = exports.isClockwise = exports.densifyPolygon = exports.calcStraightLine = exports.convertIndexToDict = exports.extremaUnion = exports.getPolygonExtrema = exports.isInside = exports.fuseId = exports.unfuseId = void 0;
const pixi_js_1 = __webpack_require__(/*! pixi.js */ "./node_modules/pixi.js/lib/pixi.es.js");
function unfuseId(fId) {
    const cls = Math.floor(fId / (256 * 256));
    const id2 = Math.floor((fId % (256 * 256)) / 256);
    const id1 = fId - cls * (256 * 256) - id2 * 256;
    return [id1, id2, cls];
}
exports.unfuseId = unfuseId;
/**
...
function getPolygonExtrema(polygon) {
    let xMin = 100000;
    let xMax = 0;
    let yMin = 10000;
    let yMax = 0;
...
exports.getPolygonExtrema = getPolygonExtrema;
;
function extremaUnion(extrema, extrema2) {
...
exports.extremaUnion = extremaUnion;
/**
...
exports.convertIndexToDict = convertIndexToDict;
/**
 ...
 */
function calcStraightLine(p1, p2) {
    ...
    coordinatesArray.push(new pixi_js_1.Point(x1, y1));
    // Main loop
    while (!((x1 === x2) && (y1 === y2))) {
        const e2 = err << 1;
        if (e2 > -dy) {
...
exports.calcStraightLine = calcStraightLine;
/**
 ...
 */
function densifyPolygon(polygon, ctType = 'external') {
    const densePoly = [];
...
exports.densifyPolygon = densifyPolygon;
/**
...
function getPixelId(data, idx) {
    const id1 = data.data[idx * 4];
    const id2 = data.data[idx * 4 + 1];
    const cls = data.data[idx * 4 + 2];
    return [id1, id2, cls];
}
exports.getPixelId = getPixelId;
function getDensePolysExtrema(densePolygons) {
    let xMin = 1000000;
    let xMax = 0;
    let yMin = 1000000;
    let yMax = 0;
...
exports.getDensePolysExtrema = getDensePolysExtrema;
exports.distinctColors = [
    [230, 25, 75],
    [60, 180, 75],
    [255, 225, 25],
    [0, 130, 200],
...
//# sourceMappingURL=utils-mask.js.map
...
const utils_mask_1 = __webpack_require__(/*! ./utils-mask */ "./lib/utils-mask.js");
//import { updateDisplayedSelection, getPolygons } from './controller-mask-utils';
const controller_mask_1 = __webpack_require__(/*! @pixano/graphics-2d/lib/controller-mask */ "./node_modules/@pixano/graphics-2d/lib/controller-mask.js");
    ...
    <main>
      <div class="right-panel">
		...
		<mwc-icon-button icon="tonality"
						   ...
						   @click="${() => this.pxnElement.toggleLabels()}">
          </mwc-icon-button>
          ${this.pixano_element_type === 'pxn-segmentation' || this.pixano_element_type === 'pxn-smart-segmentation' || this.pixano_element_type === 'pxn-smart-segmentation-python'
            ? lit_element_1.html `
              <p class="icon" title="Brush"                @click=${() => this.brush_func()} >${style_1.brush}</p>
                ${this.pixano_element_type === 'pxn-smart-segmentation' || this.pixano_element_type === 'pxn-smart-segmentation-python'
...
class PixanoModel extends base_1.DOMWidgetModel {
    defaults() {
        return Object.assign(Object.assign({}, super.defaults()), { _model_name: PixanoModel.model_name, _model_module: PixanoModel.model_module, _model_module_version: PixanoModel.model_module_version, _view_name: PixanoModel.view_name, _view_module: PixanoModel.view_module, _view_module_version: PixanoModel.view_module_version, element: 'pxn-rectangle', image_path: '', image_data: '', mode: 'none', img_count: 0, counter: 0, toggle_labels: 0, 
            ...
            // current_category:'',
            // categories_colors:{},
            modified: 0, annotations: {}, annotations_input: [0], selectedIds: {}, selectedIds_input: [], key_event: {}, 
            //targetClass:1,
            //clsMap:{},
            maskVisuMode: 'INSTANCE', prediction: [], selectedCategory: {}, label_schema: {}, image_screenshot: '', screenshot: '', do_screenshot: 0 });
    }
}
...
class PixanoView extends base_1.DOMWidgetView {
    ...
    remove() {
        console.log('remove pixano');
        if (this.pxnElement.input) {
            URL.revokeObjectURL(this.pxnElement.input);
        }
        super.remove();
    }
    render() {
        console.log("pixanoooooo 1.7.00");
        this.counter = 0;
        this.selectedIds = [];
        //this.old_url=''
        this.prediction_received = false;
        let with_python_model = false;
        this.pxn_element_type = this.model.get('element');
        const python_key = "-python";
        if (this.pxn_element_type.search(python_key) !== -1) {
            console.log("with python model");
            this.pxn_element_type = this.pxn_element_type.slice(0, -python_key.length);
            with_python_model = true;
        }
        console.log("create " + this.pxn_element_type);
        this.annotations = [];
        ...
        this.pxnElement = document.createElement(this.pxn_element_type);
        if (this.pxn_element_type === 'pxn-smart-segmentation' && with_python_model) {
            this.pxnElement.python_model = true;
        }
        if (this.old_url !== '') {
            this.pxnElement.input = this.old_url;
        }
        ...
        this.pxnElement.addEventListener('wait_for_result', (e) => { this.callback_mask_prediction(e); });
        this.pxnElement.addEventListener('wait_for_result', (e) => { this.callback_mask_prediction(e); });
        this.pxnElement.addEventListener('keydown', (e) => { this.callback_key_event(e); });
        this.pxnElement.addEventListener('keyup', (e) => { this.callback_key_event(e); });
        ...
        this.right_panel.pixano_element_type = this.pxn_element_type;
        ...
        this.pxnElement.addEventListener('keydown', (event) => {
            console.log('keydown el', event);
            this.counter = this.counter + 1;
            this.model.set("key_event", { 'counteur': this.counter,
                'code': event['code'],
                'altKey': event['altKey'],
        ...
        this.pxnElement.addEventListener('keyup', (event) => {
            console.log('keydown el', event);
            this.counter = this.counter + 1;
            this.model.set("key_event", { 'counteur': this.counter,
                'code': event['code'],
        ...
        this.init_model_interaction();
    }
    init_model_interaction() {
        ...
        this.model.on('change:selectedIds_input', this._onSelectedIds_input, this);
        this.model.on('change:prediction', this._on_prediction_changed, this);
        this.model.on('change:toggle_labels', this._onToggleLabels, this);
        this.model.on('change:label_schema', () => { console.log("change schema", this.model.get("label_schema")); }, this);
        this.model.on('change:do_screenshot', this.screenShot, this);
        if (this.pxn_element_type.search("segmentation") !== -1) {
    ...
    screenShot() {
        const do_screenshot = this.model.get('do_screenshot');
        console.log("do_screenshot", do_screenshot);
        if (do_screenshot > 0) {
            //this.el.appendChild(this.pxnElement.renderer.renderer.plugins.extract.image(this.pxnElement.renderer.stage))
            this.model.set("image_screenshot", this.pxnElement.renderer.renderer.plugins.extract.image(this.pxnElement.renderer.stage).src);
            this.model.set('do_screenshot', 0);
            this.touch();
            // console.log("get screenshot",this.model.get('image_screenshot'))
            // console.log("get do_screenshot",this.model.get('do_screenshot'))
        }
        // else{
        // 	this.model.set('do_screenshot',0)
        // 	this.touch();
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    _onSelectedIds_input() {
        console.log("_onselectedIds_input");
        let selectedIds = this.model.get("selectedIds_input");
        selectedIds = selectedIds[1];
        this.pxnElement.selectedShapeIds = selectedIds;
        this.selectedIds = selectedIds;
    }
    _onToggleLabels() {
        console.log("_onToggleLabels");
        this.pxnElement.toggleLabels();
    }
    _on_prediction_changed() {
        console.log("prediction_changed");
        this.prediction_received = true;
    }
    async onMessage(message, buffers) {
        console.log("command", message);
        const pxn_element_type = this.model.get('element');
        let new_thing = false;
        if ("set_image" in message) {
            var format = message.set_image.format;
            console.log("set_image");
            if (buffers.length > 0) {
                console.log("with image");
                var url = '';
                if (this.old_url != '') {
                    URL.revokeObjectURL(this.old_url);
                }
                if (format !== 'url') {
        ...
        if ("set_annotations" in message) {
            const annotations = message.set_annotations;
            this.setAnnotations(annotations);
            console.log("msg set_annotations", annotations);
            new_thing = true;
        }
        switch (pxn_element_type) {
            ...
            case 'pxn-smart-rectangle':
                const shapes = [...this.annotations].map(shape => {
                    var newShape = shape;
            ...
            // 	let mask:any = this.annotations.find((l:any) => l.id === 0);
            // 	//console.log(mask)
        ...
        if (new_thing) {
            this.model.set('modified', 0);
            this.touch();
        }
    }
    _onImagedataChanged() {
        console.log("_onImageDataChanged with new image data");
        let image_in = this.model.get('image');
        //if (image_in=='') return
        this.pxnElement.input = "data:image/png;base64," + image_in;
        const pxn_element_type = this.model.get('element');
        this.initAnnotations();
        if (pxn_element_type === 'pxn-segmentation' || pxn_element_type === 'pxn-smart-segmentation' || pxn_element_type === 'pxn-smart-segmentation-python') {
        ...
        this.updateModelAnnotations();
        this.model.set('modified', 0);
        //let n=this.model.get("img_count")
        // n=n+1;
        // this.model.set("img_count",n);
        this.touch();
        console.log("fin image in");
    }
    _onModeChanged() {
    ...
    _onAnnotationsInChanged() {
        let annotations_input = this.model.get('annotations_input');
        console.log("_onAnnotations Input ", annotations_input);
        //console.log("shapes_in",shapes_in);
        const pxn_element_type = this.model.get('element');
        if (annotations_input[0] !== '-') {
            annotations_input = annotations_input[1];
            console.log("annotations_input ok,", annotations_input);
            switch (pxn_element_type) {
                case 'pxn-classification':
                    /* nothing to do */
                    break;
                ...
                case 'pxn-segmentation':
                case 'pxn-smart-segmentation':
                case 'pxn-smart-segmentation-python':
                    //console.log("annotations_input",annotations_input)
    ...
    onAttributeChanged() {
        ...
        modified = modified + 1;
        this.model.set('modified', modified);
        this.touch();
    }
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    callback_mask_prediction(evt) {
        console.log("callback_mask_prediction", evt);
        //console.log("this.pxnElement.image",this.pxnElement.renderer.imageBase64);
        // const buff=new ArrayBuffer(this.pxnElement.renderer.imageBase64);
        // buff.
        // console.log(buff)
        //const img64=this.pxnElement.renderer.imageBase64;
        const canvas = document.createElement('canvas');
        const ctx = canvas.getContext('2d');
        canvas.height = this.pxnElement.renderer.htmlImageElement.height;
        canvas.width = this.pxnElement.renderer.htmlImageElement.width;
        ctx.drawImage(this.pxnElement.renderer.htmlImageElement, 0, 0);
        const xmin = evt.detail[0];
        const ymin = evt.detail[1];
        const xmax = evt.detail[2];
        const ymax = evt.detail[3];
        const w = xmax - xmin;
        const h = ymax - ymin;
        const imdata = ctx.getImageData(xmin, ymin, w, h);
        this.send({ predict: { 'roi': evt.detail, 'size': [w, h] } }, [imdata.data]);
        console.log("wait for prediction...", evt.detail);
        ...
        const apply_prediction = async () => {
            await waitFor(() => (this.prediction_received === true));
            console.log('!!!!!!! FINI D ATTENDRE');
            const prediction = this.model.get("prediction")[0];
            // console.log(prediction);
            // const xmin=prediction['roi'][0]
            // const ymin=prediction['roi'][1]
            // const xmax=prediction['roi'][2]
            // const ymax=prediction['roi'][3]
            const res = prediction; //['result']
            console.log("roi", [xmin, ymin, xmax, ymax]);
            const controler = this.pxnElement.modes['smart-create'];
            const newValue = controler.getTargetValue();
            console.log("newValue", newValue);
            controler.gmask.updateByMaskInRoi(res, [xmin, ymin, xmax, ymax], newValue, "add");
            controler.densePolygons = controller_mask_1.getPolygons(controler.gmask, newValue);
            console.log(controler.densePolygons, controler.contours);
            controller_mask_1.updateDisplayedSelection(controler.contours, controler.densePolygons);
            controler.gmask.fusedIds.add(utils_mask_1.fuseId(newValue));
            this.pxnElement.dispatchEvent(new CustomEvent('update', { detail: newValue }));
            this.model.set("prediction", ['-']);
            this.touch();
        };
        apply_prediction();
        return;
