...
class Notebook(Help, Format):
    ...
    def print_cell_execution_count(self):
      ...
      if "execution_count" in self.current_cell.json:
    ...
    def print_cell_output(self,text=""):
      ...
      try:
        if "outputs" in self.current_cell.json:
          if self.current_cell.json["outputs"]:
            outputs=self.current_cell.json["outputs"]
            if outputs:
    ...
    def run_current_cell(self):
        try:
          if self.current_cell.json["cell_type"] == "code":
            ...
            cell_run=ensure_future(self.queue_run_cell())
            #await self.run_cell()
            self.kernel_exe_tm = 0
