...
def setup(filepath=None, headers_raw=None):
    ...
    if not headers_raw:
        ...
        print(
            "Please paste the request headers from Firefox and press "
            + eof
            + " to continue:"
        )
        while True:
    ...
    try:
        ...
        raise Exception(
            "Error parsing your input, please try again. Full error: " + str(e)
        )

    missing_headers = {"cookie", "x-goog-authuser"} - set(
        k.lower() for k in user_headers.keys()
    )
    if missing_headers:
        raise Exception(
