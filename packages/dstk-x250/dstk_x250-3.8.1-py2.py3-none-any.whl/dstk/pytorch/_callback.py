...
from dstk.pytorch import is_main_process
...
class Backup(CallbackInterface, metaclass=SuperclassDocstring):
    ...
    def end_step(
        ...
        **kwargs
        ...
        if is_main_process() and state.n_iter % self.n_iter == 0:
            pkl.dump(
                [
                    model.state_dict(),
        ...
        if is_main_process() and self.nb_backup:
            files = sorted(
                list(
                    filter(
                        lambda x: (
    ...
    def end_train(
        ...
        **kwargs
        ...
        if is_main_process() and self.erase_folder:
            files = list(
                filter(
                    lambda x: (
                        x.startswith(state.id_process)
...
class ProgressBarCallback(CallbackInterface, metaclass=SuperclassDocstring):
    ...
    def begin_epoch(
        ...
        **kwargs
        ...
        if is_main_process() and self.bar_epoch:
            self.epoch_bar = tqdm(
    ...
    def begin_eval(
        ...
        **kwargs
        ...
        if is_main_process() and self.bar_eval:
            self.eval_bar = tqdm(
