...
def run_amiris(options: dict) -> None:
    ...
    properties_path = Path(jar_wd, "log4j.properties")
    fame_setup_path = Path(jar_wd, "fameSetup.yaml")

    log4j_properties = f"-Dlog4j.configuration=file:{properties_path}"
    runner = "de.dlr.gitlab.fame.setup.FameRunner"
    input_pb = fameio_input_config[Options.OUTPUT]

    call = 'java -cp "{}" "{}" {} -f "{}" -s "{}"'.format(
        path_to_jar, log4j_properties, runner, input_pb, fame_setup_path
    )
