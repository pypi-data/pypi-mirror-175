...
class CatalogThread(CatalogBase):
    ...
    last_replies: List[CatalogBase] = Field([])# catalog OP only | JSON representation of the most recent replies to a thread | array of JSON post objects")

# class CatalogTest(BaseModel):
#     no: Optional[int] = Field(None, description="always | The numeric post ID | any positive integer")
#     com: Optional[str] = Field(None, description="OP only, if thread is currently stickied | If the thread is being pinned to the top of the page| `1` or not set")
