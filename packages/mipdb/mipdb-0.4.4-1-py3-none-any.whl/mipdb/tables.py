...
class PrimaryDataTable(Table):
    ...
    def from_cdes(
        ...
        for cde in cdes:
            if '"' in cde.code:
                raise UserInputError(f'CDE code: {cde.code} contains prohibited character double quotes (")')
            columns.append(sql.Column(cde.code, STR2SQLTYPE[json.loads(cde.metadata)["sql_type"]], quote=True))
        columns.insert(
            0,
            sql.Column(
