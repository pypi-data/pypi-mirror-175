...
class Task:
    def __init__(self, callback: Callable, *args, **kwargs):
        ...
        self._args = args
        self._kwargs = kwargs
        _BackgroundAsyncService(self.__class__.__name__)
        _BackgroundSyncService(self.__class__.__name__)
    ...
    def run(self, type_: TaskType):
        if type_ == TaskType.Asynchron:
            _BackgroundAsyncService().schedule_task(self)
        elif type_ == TaskType.Synchron:
            _BackgroundSyncService().schedule_task(self)
        elif type_ == TaskType.Foreground:
            return self()
        else:
