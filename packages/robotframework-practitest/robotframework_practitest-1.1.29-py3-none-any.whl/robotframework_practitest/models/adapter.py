...
class _TestInstancesCountControl:
    def __init__(self):
        self._test_counter = {'instances': 0, 'results': 0}

    def add_instance(self, count=1):
        self._test_counter['instances'] += count

    def add_result(self, count=1):
        self._test_counter['results'] += count

    def wait_for_completed(self):
        logger.console(f"{self}; Waiting to complete all tests")
        while self._test_counter.get('instances') != self._test_counter.get('results'):
            logger.console(f"{self}")
            sleep(0.5)
        logger.console("All tests ended")

    def __str__(self):
        return f"Scheduled {self._test_counter.get('instances')}; Completed: {self._test_counter.get('results')}"
...
class RobotToPractiTest_Adapter(_TestInstancesCountControl):
    ...
    def create_test_instance(self, test: PTestCase, version):
        ...
        self.add_instance(len(related_tests_list))
