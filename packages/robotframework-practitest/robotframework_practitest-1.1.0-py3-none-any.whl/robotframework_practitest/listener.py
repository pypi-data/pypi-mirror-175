...
import robotframework_practitest.models.configuration as config
...
class Listener(adapter.RobotToPractiTest_Adapter):
    ...
    def start_suite(self, suite: ESuite, result: RSuite):
        ...
        try:
            ...
            if len(get_parents_path(suite)) == self.test_set_level:
                ...
                task = Task(self.start_test_set, data_snapshot, self.version, run_id=self.external_run_id,
                            post_process=self.test_set_post_process(suite))
                task() if self._debug else BackgroundSync.put(task)
        except Exception as e:
            f, li = get_error_info()
    ...
    def start_test(self, test: ETest, result: RTest):
        ...
        try:
            bg_logger.log_background_messages()
            task = Task(self.create_test_instance, robot_m.running.PTestCase(test), self.version)
            task() if self._debug else BackgroundSync.put(task)
        except Exception as e:
            f, li = get_error_info()
            logger.warn(f"{e}; File: {f}:{li}")
    ...
    def end_test(self, test: ETest, result: RTest):
        ...
        try:
            bg_logger.log_background_messages()
            task = Task(self.set_test_results, robot_m.running.PTestCase(test), robot_m.running.PTestResult(result))
            task() if self._debug else BackgroundAsync.put(task)
        except Exception as e:
            f, w = get_error_info()
