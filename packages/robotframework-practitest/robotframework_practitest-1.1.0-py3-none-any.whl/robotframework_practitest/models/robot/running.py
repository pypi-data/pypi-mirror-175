...
def extract_variables(*params):
    for pattern in params:
        for var_match in VARIABLE_REGEX.match(pattern).groups()[1]:
            yield BuiltIn().get_variable_value(var_match)


class PKeyword:
    __slots__ = ['name', 'tags', 'parent', 'doc', 'teardown', 'args']

    def __init__(self, item: RKeyword):
        self.name = getattr(item, 'name', item.type)
        self.tags = getattr(item, 'tags', [])
    ...
    def __str__(self):
        args_values = extract_variables(*self.args)
        return f"{self.name}({', '.join(args_values)})"
