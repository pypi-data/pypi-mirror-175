...
def batch_fetch(connection, sql_query, batch_size):
    if connection.connection is not None and callable(getattr(connection.connection, 'execute', None)):
        ...
        result_proxy = connection.connection.execution_options(yield_per=batch_size).execute(sql_query)
    else:
        result_proxy = connection.execution_options(yield_per=batch_size).execute(sql_query)
