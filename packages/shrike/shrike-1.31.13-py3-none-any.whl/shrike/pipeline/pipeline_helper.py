...
class AMLPipelineHelper:
    ...
    def _apply_windows_runsettings(
        ...
        **custom_runtime_arguments,
        ...
        if self.module_loader.is_local(module_name):
            ...
            if (
                "hdi_dc_target" in self.config.compute
                or "synapse_dc_target" in self.config.compute
            ):
    ...
    def _update_dc_configs(self, pipeline):
        ...
        try:
            ...
            log.info("This pipeline doesn't involve Synapse DC.")
        if not hdi_compute and not synapse_compute:
            # No need to update this pipeline
            return pipeline
