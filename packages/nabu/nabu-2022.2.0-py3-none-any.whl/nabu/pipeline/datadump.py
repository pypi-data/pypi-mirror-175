...
from .utils import WriterConfigurator, get_subregion
...
class DataDumpManager:
    ...
    def _configure_dump(self, step_name, force_dump_to_fname=None):
        ...
        file_prefix += str("_%05d" % self.start_index)
        if self._grouped_processing:
            file_prefix += str("_%05d" % self.sub_region[0][0])

        self.data_dump[step_name] = WriterConfigurator(
