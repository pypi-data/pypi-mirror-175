...
class FullFieldReconstructor:
    ...
    def _get_reconstruction_range(self):
        dataset_info = self.process_config.dataset_info
        ...
        self.n_angles = dataset_info.n_angles
        self.n_x, self.n_z = dataset_info.radio_dims
    ...
    def _compute_max_chunk_size(self):
        ...
        if user_chunk_size is not None:
            self.cpu_max_chunk_size = min(self.cpu_max_chunk_size, user_chunk_size)
            if self.backend == "cuda":
                self.gpu_max_chunk_size = min(self.gpu_max_chunk_size, user_chunk_size)
    ...
    def _compute_max_group_size(self):
        """
        Compute the maximum number of FULL radios that can be processed in memory
        """
        ...
        user_max_group_size = self.extra_options.get("max_chunk_size", None)
    ...
    def _get_pipeline_mode(self):
        ...
        if self._margin_v > self.chunk_size//3:
            ...
            if n_stages > 1:
                msg = str(
                    "Margin (%d) is too big for chunk size (%d)"
    ...
    def _compute_margin(self):
        ...
        if user_margin is not None and user_margin > 0:
            ...
            margin_v = max(unsharp_margin[0], phase_margin[0])
            margin_h = max(unsharp_margin[1], phase_margin[1])
            if margin_v > 0:
    ...
    def _compute_phase_margin(self):
        ...
        if phase_options is None:
            return (0, 0)
        if phase_options["method"] != "paganin":
            return (0, 0)
    ...
    def _build_tasks_grouped(self):
        ...
        for segment in segments:
            ...
            tasks.append({"sub_region": sub_region, "margin": None})
        self.tasks = tasks
        # Build tasks for stage 2 (sinogram processing + reconstruction)
        segments = subdivide_into_overlapping_segment(self.delta_z, self.chunk_size, 0)
        self._sino_tasks = []
        for segment in segments:
    ...
    def _build_tasks_chunked(self):
        ...
        if self.chunk_size >= self.delta_z and self.z_min == 0 and self.z_max == self.n_z:
            ...
            return
        if self.max_chunk_size - (2 * margin_v) >= self.delta_z:
    ...
    def merge_histograms(self, output_file=None, force_merge=False):
        ...
        histograms_merger = PartialHistogram(
