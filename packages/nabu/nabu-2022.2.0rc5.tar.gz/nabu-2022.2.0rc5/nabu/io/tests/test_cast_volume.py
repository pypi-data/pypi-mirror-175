...
def test_get_default_output_volume():
    ...
    with pytest.raises(ValueError):
        # test output value
        get_default_output_volume(
            input_volume=EDFVolume(folder="test"), output_type="toto"
        )
    ...
    assert (
        output_volume.data_url.data_path() == f"/volume/{HDF5Volume.DATA_DATASET_NAME}"
    )
    assert output_volume.metadata_url.file_path() == "folder/vol_cast/basename.hdf5"
    assert (
        output_volume.metadata_url.data_path()
        == f"/volume/{HDF5Volume.METADATA_GROUP_NAME}"
    )
...
def test_find_histogram_hdf5_volume(tmp_path):
    ...
    assert find_histogram(
        volume=HDF5Volume(file_path=h5_file, data_path="myentry")
    ) == DataUrl(
        file_path=h5_file,
    ...
    assert (
        find_histogram(volume=HDF5Volume(file_path=h5_file, data_path="entry")) == None
    )
...
@pytest.mark.parametrize(
    "input_dtype", (numpy.float32, numpy.float64, numpy.uint8, numpy.uint16)
)
...
def test_cast_volume(tmp_path):
    ...
    # if percentiles 10 and 90 provided, no data_min and data_max then will take the largest interval possible
    volume_edf.save()
    assert volume_edf.data.dtype == numpy.uint16
    numpy.testing.assert_array_equal(
        volume_edf.data.flatten()[0:10],
        numpy.array([numpy.iinfo(numpy.uint16).min] * 10),
    )
    numpy.testing.assert_array_equal(
        volume_edf.data.flatten()[90:100],
        numpy.array([numpy.iinfo(numpy.uint16).max] * 10),
    )
