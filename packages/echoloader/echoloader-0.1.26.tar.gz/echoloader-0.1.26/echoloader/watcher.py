...
def read_args():
    ...
    if args.upload or args.sync:
        args.auth = Auth(
            args,
            getattr(args, 'username', os.environ.get(f"{OS_VAR_PREFIX}COGNITO_USERNAME")) or input("username: "),
            getattr(args, 'password', os.environ.get(f"{OS_VAR_PREFIX}COGNITO_PASSWORD")) or getpass("password: "),
        )
...
def main():
    ...
    try:
        ...
        if args.pacs:
            ...
            ae.start_server(('0.0.0.0', args.pacs_port), block=True, evt_handlers=handlers)
