...
from aiokafka_commit.util import create_future, create_task, get_running_loop, wait_for

from aiokafka_commit.abc import AbstractTokenProvider
...
async def create_conn(
    host, port, *, client_id='aiokafka_commit',
...
class AIOKafkaConnection:
    ...
    def __init__(self, host, port, *, client_id='aiokafka_commit',
                 request_timeout_ms=40000, api_version=(0, 8, 2),
                 ssl_context=None, security_protocol='PLAINTEXT',
        ...
        if sasl_mechanism == "OAUTHBEARER":
            if sasl_oauth_token_provider is None or \
                ...
                raise ValueError("sasl_oauth_token_provider needs to be \
                    provided implementing aiokafka_commit.abc.AbstractTokenProvider")
            assert callable(
