...
/**$course**/**$semester**/dashboard (route_dash):
    ...
    for that task. As well as pset submission options, the dashboard has
    a course concepts overview that tracks competencies demonstrated in
    psets and via exercises. TODO: Concepts overview.
...
DEFAULT_CONFIG = {
    ...
    "EXERCISE_GROUP_CREDIT_BUMP": 0.1,
...
def route_set_extensions(
    ...
    as a pset, their extensions will overwrite each other!!!
    """
    if not is_admin:
...
def route_starter_zip(course, semester, taskid):
    ...
    max_starter_files = app.config.get("MAX_STARTER_FILES", 100000)
...
def amend_task(course, semester, task_info, psid, task, username, phase):
    ...
    # Add max score info
    max_score = app.config.get("SCORE_BASIS", 100)
    revision_max = app.config.get("REVISION_MAX_SCORE", 100)
    belated_max = app.config.get("BELATED_MAX_SCORE", 85)
    if phase == "revision":
    ...
    if timeliness_override == '':
        ...
        if phase != 'initial':
            flask.flash(
                ...
                ).format(timeliness_override, psid, task["id"], phase)
            )
    ...
    for egroup in use_groups:
        ...
        for ex in egroup["exercises"]:
            ...
            ebest = storage.fetch_best_outcomes(
                ...
                eid,
                deadline,
                app.config.get("LATE_EXERCISE_CREDIT_FRACTION", 0.5)
            )
...
def amend_exercises(course, semester, task_info, outcomes, username):
    ...
    for group in task_info.get("exercises", []):
        ...
        if credit_fraction == 1:
            status = "perfect"
        elif credit_fraction >= app.config.get(
...
def pset_combined_grade(pset):
    ...
    for task in pset["tasks"]:
        ...
        tp = task_timeliness_points(task)
    ...
    score_basis = app.config.get("SCORE_BASIS", 100)
    max_score = score_basis + app.config.get("TIMELINESS_POINTS", 0)
    weighted_score = sum(
        ...
        options.append(
            min(
                ...
                rev_task.get(
                    ...
                    app.config.get(
                        "REVISION_MAX_SCORE",
                        app.config.get("SCORE_BASIS", 100)
                    )
                )
            )
        )
...
@app.template_filter()
def task_timeliness_points(task):
    """
    ...
    """
    earned = 0
    available = app.config.get("TIMELINESS_POINTS", 0)
    ...
    if initial_grade == "?":
        ...
        if initial_grade >= app.config.get(
            "TIMELINESS_ATTEMPT_THRESHOLD",
            app.config.get(
                "EVALUATION_SCORES",
                {}
            ).get("partially complete", 75)
        ):
    ...
    completion_threshold = app.config.get(
        "TIMELINESS_COMPLETE_THRESHOLD",
        app.config.get(
            "EVALUATION_SCORES",
            {}
        ).get("almost complete", 85)
    )
    if (
...
def ex_combined_grade(egroup):
    ...
    bump = app.config.get("EXERCISE_GROUP_CREDIT_BUMP", 0.1)
    if fraction >= app.config.get("EXERCISE_GROUP_PARTIAL_THRESHOLD"):
        fraction += bump
    if fraction >= app.config.get("EXERCISE_GROUP_THRESHOLD"):
        fraction += bump

    basis = app.config.get("SCORE_BASIS", 100)
    # no extra credit
    return min(basis, basis * fraction)
...
@app.template_filter()
def timeliness_string(grade_value):
    """
    ...
    if grade_value is None or not isinstance(grade_value, (int, float)):
        ...
        round_to = app.config["ROUND_SCORES_TO"]
        rounded = round(grade_value, round_to)
        rdenom = round(app.config.get("TIMELINESS_POINTS", 0), round_to)
        if int(rounded) == rounded:
            rounded = int(rounded)
        if int(rdenom) == rdenom:
            rdenom = int(rdenom)
