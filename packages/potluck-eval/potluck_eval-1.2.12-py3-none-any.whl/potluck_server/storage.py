...
def fetch_outcomes(course, semester, username, exercise, category):
    ...
    - "credit": A number indicating how much credit (0-1) was earned for
        this outcome. If the category is 'none', this will always be 0,
        if the category is 'partial' it will be greater than 0 and less
        than 1, and if the category is 'full', it will be 1. This number
        does NOT account for timeliness.
...
def fetch_best_outcomes(
    ...
    - "credit" - A number indicating how much credit this exercise is
        ...
        with the submission (like wrong # of outcomes). This accounts
        for whether the exercise was late or not.
...
def save_outcomes(
    ...
    if credit > 1:
        raise ValueError(
            "Invalid credit value '{}' (must be <= 1).".format(credit)
        )
    elif credit == 1:
        category = 'full'
    elif credit > 0:
