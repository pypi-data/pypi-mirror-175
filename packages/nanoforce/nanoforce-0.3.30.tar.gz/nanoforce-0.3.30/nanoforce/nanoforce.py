...
class AFM:
    ...
    calc_work_adhesion:
        Function to calculate the work of adhesion (J)
    ...
    def save_data(
        ...
        np.savez(
            ...
            work_adhesion = self.work_adhesion,
    ...
    def load_data(
        ...
        self.work_adhesion = data['work_adhesion']
    ...
    def calc_work_adhesion(
        self
    ):
        '''
        Function to calculate work of adhesion (J).
        '''
        work = np.zeros(self.z_approach.shape[1])

        for curve_number in range(0, self.z_approach.shape[1]):
            force = self.retract[:,curve_number]
            distance = self.z_withdraw[:,curve_number]

            distance = distance[~np.isnan(force)]
            force = force[~np.isnan(force)]

            d = np.linspace(distance[0], distance[-1], 10000)
            f = np.interp(d, distance, force)

            d = d[f < 0]
            f = f[f < 0]
            
            work[curve_number] = np.trapz(f, d) * -1

        self.work_adhesion = work
    ...
    def force_map(self,
        ...
        plot_types = ['adhesion', 'modulus', 'topography', 'indent', 'max_force', 'snap_in','snap_dist','work']
        member = plot in plot_types

        if member == False:
            print('Incorrect plot type specified. Available plots are adhesion, modulus, topography, indent, max_force, work, snap_in and snap_dist.')
