...
class InvoiceService:
    def __init__(self):
        ...
        self.clockify = ClockifyAPIClient().build(
            conf.INVOICE_CONFIG.get("clockify_api_key"), conf.CLOCKIFY_API_URL
        )
    ...
    def create_invoice(self) -> str:
        ...
        invoice = Invoice(
            ...
            issued_on=(
                datetime.datetime.now().replace(day=1) - relativedelta(days=1)
            ).date(),
            lines=[
                InvoiceLine(
    ...
    def get_last_month_hour_report(self) -> int:
        ...
        return (
            report_api.get_summary_report(
                ...
                {
                    "dateRangeStart": last_month_start.strftime(
                        "%Y-%m-%dT%H:%M:%S.%fZ"
                    ),
