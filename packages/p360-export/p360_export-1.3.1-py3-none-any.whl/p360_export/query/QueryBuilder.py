import random
import string
from typing import Sequence, Union, List, Dict, Tuple, Callable, Any
...
class QueryBuilder:
    ...
    def __build_select_part(self, config: Dict[str, Any], table_id: str) -> str:
    ...
    def __build_condition(self, segment_definition: List[Dict]) -> str:
        if self.__exporting_all_users(segment_definition):
            return ""

        list_of_product_terms = self.__build_list_of_product_terms(segment_definition[0].get("definition_segment"))
    ...
    def build(self, config: Dict[str, Any]) -> Tuple[str, str]:
        segment_definition = config.get("segments", [])
        table_id = self.__get_table_id()

        condition = self.__build_condition(segment_definition=segment_definition)
        select_part = self.__build_select_part(config=config, table_id=table_id)
