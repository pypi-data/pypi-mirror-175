...
class DingBot:
    ...
    def __data2url(self, result_data, html_path, img_path, oss_img_name):
        # ----- 数据转html文件 -----
    ...
    def send_img_simple(self, pg_config, pg_sql):
        ...
        img_path = '%s/html2jpg%s.jpg' % (self.dir_,''.join([random.choice(string.ascii_lowercase) for i in range(0, 32)]))
        oss_img_name = ''.join([random.choice(string.ascii_lowercase) for i in range(0, 32)])
        if result_data:
            img_url = self.__data2url(result_data, html_path, img_path, oss_img_name)
        elif not self.img_null_is_send:
            return 'data is null'
        else:
            img_url = ''
