...
class LIB:
    ...
    @staticmethod
    def CRYPTOGRAPHY(word: str, key: str, action: str = "D") -> str:
        # D=Decrypt, E=Encrypt
        msg, result = None, None
        try:
            if word is None:
                result = f"""Parametro "word" é obrigatorio!"""
            if key is None:
                result = f"""Parametro "token" é obrigatório"""
            if isinstance(word, str):
                word = word.encode()
            CIPHER = Fernet(key)
            if action == "E":
                if type(word == "str"):
                    result = CIPHER.encrypt(word).decode()
            else:
                result = CIPHER.decrypt(word).decode()
        except Exception as error:
            msg = error.args[0]
            result = msg
        finally:
