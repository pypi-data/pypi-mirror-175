...
def create_tdapi_context(type, bucket_path=None, datastore_path=None, datastore_name=None, **options):
    """
    DESCRIPTION:
    ...
    if type.lower() != "aws" and type.lower() != "azure":
...
def remove_tdapi_context(tdapi_context, delete_byom_models=False, table_name="", schema_name=None):
    ...
    DESCRIPTION:
        ...
        For AWS Sagemaker, it removes any S3 folder created during operations of TDApiClient.
        For Azure ML, it removes all blobs created during operations of TDApiClient.
...
class TDPredictor:
    ...
    def __init__(self, sage_obj, tdapi_context):
        ...
        awu_matrix = []
        awu_matrix.append(["tdapi_context", tdapi_context, False, (_TDApiContext), True])
        awu_matrix.append(["sage_obj", sage_obj, False, (object), True])
        # Validate argument types
        _Validators._validate_function_arguments(awu_matrix)
...
class _TDAzurePredictor:
    """
    This is a wrapper over AciWebservice class. It allows for
    integration with Teradata at the time of scoring using predict method.
    """

    def __init__(self, azure_obj, tdapi_context):
        """
        DESCRIPTION:
            Initliazer for _TDAzurePredictor.

        PARAMETERS:
            azure_obj:
                Required Argument.
                Specifies instance of AciWebservice class.
                Types: AciWebservice Object

            tdapi_context:
                Required Argument.
                Specifies TDAPI Context object holding azure credentials
                information.
                Types: _TDApiContext object

        RETURNS:
            A _TDAzurePredictor instance.
        ...
        EXAMPLES:
            ...
            predictor_object = skLearnObject.deploy(platform="az-webservice", 
                                model=model, model_type="",**model_deploy_kwargs)
            return _TDAzurePredictor(predictor_object, self._tdapi_context)
            # predictor will be of type _TDAzurePredictor.
        """
        awu_matrix = []
        awu_matrix.append(["tdapi_context", tdapi_context, False, (_TDApiContext), True])
        awu_matrix.append(["azure_obj", azure_obj, False, (object), True])
        # Validate argument types
        _Validators._validate_function_arguments(awu_matrix)

        self.azureObj = azure_obj
        self._tdapi_context: _TDApiContext = tdapi_context

    @classmethod
    def _from_predictor(cls, azureml_predictor_obj, tdapi_context):
        """
        DESCRIPTION:
            This method creates _TDAzurePredictor from the AciWebservice
            object to allow for prediction using teradataml DataFrame and
        ...
        EXAMPLES:
            ...
            tdaz_predictor = _TDAzurePredictor._from_predictor(
                skLearnObject, context)
        ...
        return _TDAzurePredictor(azureml_predictor_obj, tdapi_context)

    def predict(self, input: DataFrame, mode, **options):
        """
        DESCRIPTION:
            This method performs prediction using teradataml DataFrame and
            Azure ML endpoint represented by this predictor object.
    ...
    def _run_prediction_at_client(self, data: DataFrame, options):
        ...
        if content_type == "json":
            ...
            return (self.azureObj.run(run_obj))
        else:
            errMsg = Messages.get_message(
                MessageCodes.INVALID_ARG_VALUE, content_type, "content_type",
                "json")
    ...
    def _run_udf(self, data: DataFrame, options: dict):
        ...
        PARAMETERS:
            data:
                Required Argument.
                Specifies teradataml dataframe which holds input data for scoring.
                Types: teradataml DataFrame
        ...
        try:
            ...
            error_code = ErrorInfoCodes.TDSG_RUNTIME_ERROR
            raise TDApiClientException(msg, error_code) from ex
        keys = self.azureObj.get_keys()
        primary_key = keys[0]
        auth_key = primary_key
        endpoint = self.azureObj.scoring_uri
        region = self.azureObj.location
        auth_info_fmt_str = ('{{ "Key": "{}", "Region" : "{}"}}')
        auth_info = auth_info_fmt_str.format(auth_key,
                                             region)
        udf_query = ("SELECT * FROM {}( ON ({}) USING AUTHORIZATION('{}')"
        ...
        logger.debug("key_start_index is {}".format(key_start_index))
        logger.debug("function_name is {}".format(function_name))
        logger.debug("content_format is {}".format(content_format))
        udf_query = udf_query.format(function_name, input_query,
                                     auth_info, endpoint,
                                     content_type, key_start_index, content_format_str)
        try:
            queryDf = DataFrame(query=udf_query)
            return queryDf
        except TeradataMlException as tdml_ex:
    ...
    def __getattr__(self, name):
        if self.azureObj is None:
            return None

        def __az_method_invoker(*c, **kwargs):
            return atrribute_instance(*c, **kwargs)

        atrribute_instance = getattr(self.azureObj, name)
        if callable(atrribute_instance):
            return __az_method_invoker
        return atrribute_instance

class _TDApiAzureClientObjectWrapper:
    """
    ...
    def __init__(self, class_instance, pargs, kwargs, tdapi_context) -> None:
        """
        DESCRIPTION:
            This method is used for creating objects of _TDApiAzureClientObjectWrapper.
        ...
        RETURNS:
            _TDApiAzureClientObjectWrapper
    ...
    def fit(self, content_type='parquet', mount=True, wait=True):
        ...
        self.azureObj = self._class_instance(*self._pargs, **self._kwargs)
        experiment_name = 'tdapi_experiment'
        experiment = Experiment(workspace=workspace, name=experiment_name)

        run = experiment.submit(config=self.azureObj)
        if wait:
...
class _BYOMPredictor:
    ...
    obtained in _TDApiAzureClientObjectWrapper.deploy.
    """
...
class _TDApiClientObjectWrapper:
    ...
    def _export_df_to_s3(self, input_data_frame: DataFrame, content_type):
        ...
        try:
            from teradataml import WriteNOS
            obj = WriteNOS(data=input_data_frame, location=s3_address,
        ...
        def __azClassFinder(*c, **kwargs):
            ...
            for module in azureml_module_list:
                ...
                try:
                    ...
                    continue
                return _TDApiAzureClientObjectWrapper(class_instance, c, kwargs, self._tdapi_context)
