    ...
    def __init__(
        ...
        if not isinstance(data, (pl.LazyFrame, pl.DataFrame)):
            ...
            if isinstance(data, dict):
                # Convert the columns to root fqn strings
                # str(Feature) and str(FeatureWrapper) return the root fqns
                new_data_dict: Dict[str, Sequence[Any]] = {}
                for (k, v) in data.items():
                    ...
                    for i, element in enumerate(v):
                        ...
                        if isinstance(element, datetime.datetime) and element.tzinfo is not None:
                            v[i] = element.astimezone(datetime.timezone.utc)
                    new_data_dict[str(k)] = v
                data = new_data_dict
        ...
        if convert_dtypes:
            for root_fqn in underlying.columns:
                ...
                if actual_dtype != expected_dtype:
                    if isinstance(expected_dtype, pl.Struct) or (
                        isinstance(expected_dtype, type) and issubclass(expected_dtype, pl.Struct)
                    ):
                        # Skipping casting of structs, as it can double-nest them
                        pass
