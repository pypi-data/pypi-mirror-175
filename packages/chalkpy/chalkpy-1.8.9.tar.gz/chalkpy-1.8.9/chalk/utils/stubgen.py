...
import warnings
import typing
from typing import Dict, List, NamedTuple, Optional, Tuple, Type, Union, get_args, get_origin
...
class StubGenerator:
    ...
    def _register_imports_for_annotation(self, annotation: type) -> str:
    ...
    def register_features_class(self, features_cls: Type[Features]):
        ...
        for f in features_cls.features:
            ...
            if f.typ.is_dataframe:
                ...
                annotations[f.attribute_name] = "DataFrame"
            else:
                # Make a best-effort to figure out imports.
                # Type
                annotations[f.attribute_name] = self._register_imports_for_annotation(f.typ.parsed_annotation)
    ...
    def generate_features_decorator_stub_file(self):
        ...
        parsed_feature_classes = sorted(
            self._parsed_feature_classes.items(), key=lambda x: len(x[1].annotations), reverse=True
        )
...
class {feature_cls_name}Metaclass(FeaturesMeta):"""
            ...
            for attribute, annotation in features_cls.annotations.items():
                lines.append(
                    f"""\
    @property
    def {attribute}(self) -> Type[{annotation}]: ...
"""
                )
