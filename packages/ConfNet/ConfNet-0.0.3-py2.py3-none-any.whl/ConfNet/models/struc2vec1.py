...
class Struc2Vec1():
    def __init__(self, graph, walk_length=10, num_walks=100, workers=1, verbose=0, stay_prob=0.3, opt1_reduce_len=True, opt2_reduce_sim_calc=True, opt3_num_layers=None, temp_path='./temp_struc2vec/', reuse=False):
        self.graph = graph
        """
        :param opt_reduce_sim_calc 选择是否使用优化技巧，是计算所有顶点对之间的距离，还是只计算度数相近的顶点对之间的距离
    ...
    """
    注意，我主要改的就是这里。不再是度数，而是角度。不再计算频次
    ...
    def _get_order_degreelist_node(self, root, max_num_layers=None):
        if max_num_layers is None:
            max_num_layers = float('inf') # 正无穷

        # ordered_degree_sequence_dict = {}
        ordered_angle_sequence_dict = {}

        visited = [False] * len(self.graph.nodes())
        queue = deque()
        level = 0

        # 在上一级是个遍历，以每个节点编号为root，1，2，3...
        queue.append(root)
        visited[root] = True

        while (len(queue) > 0 and level <= max_num_layers):

            # queue是把下一层的节点写进来，所以只要有下一层就可以一直循环
            count = len(queue)

            # if self.opt1_reduce_len:
            #     # opt1_reduce_len是个参数，用opt1方法，用度数-频率对，来减少有序度序列的长度，默认是True
            #     degree_list = {}
            #
            # else:
            #     degree_list = []
            angle_list = []  # 因为我这里不采用"对"的形式，所以跟未选用优化方法的数据结构一样

            while (count > 0):
                # level向外逐渐遍历，每一次循环，获得相邻节点列表，放到queue里。
                top = queue.popleft()
                node = self.idx2node[top]
                '''
                NetworkX教程
                https://blog.csdn.net/PolarisRisingWar/article/details/117372435
                G[node_id] 是一种访问节点邻居的方法，返回{'20':{}, '23':{}} 后面的括号是属性
                则degree = len(graph[node])就是这个节点的度
                '''
                # degree = len(self.graph[node])
                neighbour_list = self.graph[node]

                # if self.opt1_reduce_len:
                #     # opt1_reduce_len 是个参数，默认是true
                #     degree_list[degree] = degree_list.get(degree, 0) + 1
                # else:
                #     degree_list.append(degree)
                # 同上，数据结构跟未采用优化一样
                for neighbour in neighbour_list:
                    angle_list.append(neighbour.intersection_angle()) # 夹角需要实现一下

                # 遍历node这个节点的相邻节点
                for nei in self.graph[node]:  # nei是节点的名字（不是0起头的编号）
                    nei_idx = self.node2idx[nei]
                    if not visited[nei_idx]:
                        visited[nei_idx] = True
                        # 把相邻的节点放到queue里，带到下一个循环
                        queue.append(nei_idx)
                count -= 1

            # 排序
            # if self.opt1_reduce_len:
            #     # opt1_reduce_len是个参数，默认是True
            ...
            # 同上，跟未采用opt1优化的数据结构一样
            ordered_angle_list = sorted(angle_list)

            # orderd_degree_list格式：[(度数，出现次数),(度数，出现次数), ...]，比如：[(4,1), (3,2), (2, 5)]
            # ordered_degree_sequence_dict格式：{层数：orderd_degree_list , ...]}，比如{0: [(2, 1)], 1: [(1, 2)], 2: [(1, 1), (2, 1)], 3: [(1, 1), (2, 2)], 4: [(1, 3), (2, 1)]}
            # 生成这样的序列是第一个优化技巧
            # ordered_degree_sequence_dict[level] = orderd_degree_list
            ordered_angle_sequence_dict[level] = ordered_angle_list
            level += 1
        ...
        return ordered_angle_sequence_dict
    ...
    def _compute_structural_distance(self, max_num_layers, workers=1, verbose=0,):
        ...
        if os.path.exists(self.temp_path+'structural_dist.pkl'):
            ...
            if self.opt2_reduce_sim_calc:
                # opt2_reduce_sim_calc是参数，默认是True。是否选用优化技巧
