...
# -------------------------------------------------- PUBLIC METHODS -------------------------------------------------- #
    ...
    def nlm_geometric_mean(df:pd.DataFrame, mol_col_name:str, inchi_key_col_name:str, value_col_name:str,
        ...
        """
        if assay_type not in Bleach.assay_types:
            raise ValueError('units_type must be "vitro" or "vivo"')
...
# ---------------------------------------------------- MAIN STEPS ---------------------------------------------------- #
    ...
    def _instance_final_unit_conversion(self):
        ...
        if self._average_flag:
            ...
            self._df, self._target_col = Bleach.final_unit_conversion(df=self._df, input_units=self.model['input_units'],
