...
from databricks.feature_store.utils.converter_utils import return_if_none, return_if_nan
...
class PandasStatelessConverter(ABC):
    @staticmethod
    @abstractmethod
...
class PandasStatefulConverter(ABC):
...
class PandasIsoTimestampTypeUsConverter(PandasStatelessConverter):
    @staticmethod
    def to_online_store(value: str) -> str:
        # Pandas Timestamp does not support forcing microsecond precision, which is required for lookup.
        # e.g. pd.Timestamp("2022-06-20T15:30:45.000000").isoformat() outputs '2022-06-20T15:30:45'
        # TODO (ML-23163): Consider using `datetime.fromisoformat` when Python 3.7+ is available
        # Python's datetime.fromisoformat` is only supported in 3.7+, but the Runbot is currently on 3.6
