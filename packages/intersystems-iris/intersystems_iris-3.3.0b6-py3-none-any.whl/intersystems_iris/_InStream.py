import hashlib
import threading
import intersystems_iris._BufferReader
...
class _InStream(object):
    ...
    def _read_message_gateway(self, expected_message_id = -1):
        is_for_gateway = self.__read_message_internal(expected_message_id, -1, intersystems_iris._InStream._InStream.GATEWAY)
    ...
    def _read_message_sysio(self, expected_message_id, allowedErrors):
        while True:
            is_for_gateway = self.__read_message_internal(expected_message_id, -1, intersystems_iris._InStream._InStream.IRISNATIVE)
            if is_for_gateway:
    ...
    def __read_message_internal(self, expected_message_id, expected_statement_id, call_type):
        ...
        header = intersystems_iris._MessageHeader._MessageHeader()
        final_buffer = bytearray()
        while high_bit != 0:
            self.__read_buffer(header.buffer, 0, intersystems_iris._MessageHeader._MessageHeader.HEADER_SIZE)
            if self._log_stream is not None:
                self._log_stream._dump_header(header.buffer, intersystems_iris._LogFileStream._LogFileStream.LOG_RECEIVED, self._connection)
            ...
            if not high_bit and len(final_buffer) == 0:
                self.wire = intersystems_iris._BufferReader._BufferReader(header, buffer, self._connection._connection_info._locale)
                return is_for_gateway

            final_buffer.extend(buffer)
        self.wire = intersystems_iris._BufferReader._BufferReader(header, final_buffer, self._connection._connection_info._locale)
        return is_for_gateway

    def _check_sheader(self, hash, sock):
        sheader = intersystems_iris._MessageHeader._MessageHeader()
        try:
            timeout = sock.gettimeout()
            sock.settimeout(5)
            self.__read_buffer(sheader.buffer, 0, intersystems_iris._MessageHeader._MessageHeader.HEADER_SIZE)
            sock.settimeout(timeout)
        except Exception as e:
            return False
        if self._log_stream is not None:
            self._log_stream._dump_header(sheader.buffer, intersystems_iris._LogFileStream._LogFileStream.LOG_RECEIVED, self._connection)
    ...
    def __is_header_initizted_from_iris(self, header, call_type):
        ...
        if (call_type == intersystems_iris._InStream._InStream.GATEWAY or message_id > 0) and message_id%2 == 0:
            return True
        ...
        if call_type == intersystems_iris._InStream._InStream.GATEWAY and data_length == 0 and function_code == 0:
            return True
