...
import intersystems_iris.dbapi._Message
...
class Cursor():
        ...
        if self._statementType == StatementType.CALL or self._statementType == StatementType.CALLWITHRESULT:
            ...
            if (not self._multiple_result_sets and 
                ...
                if self.statementFeatureOption & Feature.optionFastSelect == Feature.optionFastSelect:
                    self._rsrow = intersystems_iris.dbapi._ResultSetRow._ResultSetRow(self._columns, cached_statement.maxRowItemCount, self._connection._connection_info._locale)
                else:
                    self._rsrow = intersystems_iris.dbapi._ResultSetRow._ResultSetRow(self._columns, 0, self._connection._connection_info._locale)
    ...
    def _prepare_new(self):
        ...
        with self._connection._lock:
            ...
            self._out_message.wire._write_header(intersystems_iris.dbapi._Message.PREPARE)
    ...
    def _get_error_info(self, sqlcode):
        with self._connection._lock:
            self._out_message.wire._write_header(intersystems_iris.dbapi._Message.GET_SERVER_ERROR)
    ...
    def _execute_query(self):
        ...
        if self._cursor_type == CursorType.DEFAULT:
            ...
            if self.statementFeatureOption & Feature.optionFastSelect == Feature.optionFastSelect:
                self._rsrow = intersystems_iris.dbapi._ResultSetRow._ResultSetRow(self._columns, self.maxRowItemCount, self._connection._connection_info._locale)
            else:
                self._rsrow = intersystems_iris.dbapi._ResultSetRow._ResultSetRow(self._columns, 0, self._connection._connection_info._locale)
    ...
    def _prepared_query_execute(self):
        ...
        with self._connection._lock:
            # message header
            self._out_message.wire._write_header(intersystems_iris.dbapi._Message.PREPARED_QUERY_EXECUTE)
            intersystems_iris._MessageHeader._MessageHeader._set_statement_id(self._out_message.wire.buffer, self._statement_id)
    ...
    def _send_direct_query_request(self):
        ...
        with self._connection._lock:
            # message header
            self._statement_id = self._connection._get_new_statement_id()
            self._out_message.wire._write_header(intersystems_iris.dbapi._Message.DIRECT_QUERY)
            intersystems_iris._MessageHeader._MessageHeader._set_statement_id(self._out_message.wire.buffer, self._statement_id)
    ...
    def _prepared_update_execute(self):
        ...
        with self._connection._lock:
            # message header
            self._out_message.wire._write_header(intersystems_iris.dbapi._Message.PREPARED_UPDATE_EXECUTE)
            intersystems_iris._MessageHeader._MessageHeader._set_statement_id(self._out_message.wire.buffer, self._statement_id)
    ...
    def _send_direct_update_request(self):
        ...
        with self._connection._lock:
            # message header
            self._out_message.wire._write_header(intersystems_iris.dbapi._Message.DIRECT_UPDATE)
            intersystems_iris._MessageHeader._MessageHeader._set_statement_id(self._out_message.wire.buffer, self._statement_id)
    ...
    def _prepare_stored_procedure(self):
        with self._connection._lock:
            # message header
            self._statement_id = self._connection._get_new_statement_id()
            self._out_message.wire._write_header(intersystems_iris.dbapi._Message.PREPARE_STORED_PROCEDURE)
            intersystems_iris._MessageHeader._MessageHeader._set_statement_id(self._out_message.wire.buffer, self._statement_id)
    ...
    def _stored_procedure_update(self):
        with self._connection._lock:
            # message header
            self._out_message.wire._write_header(intersystems_iris.dbapi._Message.STORED_PROCEDURE_UPDATE_EXECUTE)
            intersystems_iris._MessageHeader._MessageHeader._set_statement_id(self._out_message.wire.buffer, self._statement_id)
    ...
    def _stored_procedure_query(self):
        ...
        with self._connection._lock:
            # message header
            self._out_message.wire._write_header(intersystems_iris.dbapi._Message.STORED_PROCEDURE_QUERY_EXECUTE)
            intersystems_iris._MessageHeader._MessageHeader._set_statement_id(self._out_message.wire.buffer, self._statement_id)
    ...
    def _execute_multiple_result_sets(self, validate):
        ...
        if validate:
            self._validate_parameters()
        with self._connection._lock:
            self._out_message.wire._write_header(intersystems_iris.dbapi._Message.EXECUTE_MULTIPLE_RESULT_SETS)
    ...
    def nextset(self):
        if len(self._stored_results) == 0:
            ...
            if (self._mrs_done or not self._multiple_result_sets or 
                ...
                return False
            with self._connection._lock:
                self._out_message.wire._write_header(intersystems_iris.dbapi._Message.GET_MORE_RESULTS)
    ...
    def _get_result_set(self, oref):
        ...
        with self._connection._lock:
            self._out_message.wire._write_header(intersystems_iris.dbapi._Message.GET_RESULT_SET_OBJECT)
    ...
    def lastrowid(self):
        ...
        with self._connection._lock:
            self._out_message.wire._write_header(intersystems_iris.dbapi._Message.GET_AUTO_GENERATED_KEYS)
            sequence_number = self._connection._get_new_sequence_number()
            self._out_message._send(sequence_number)
            self._in_message._read_message_sql(sequence_number)
            self._sqlcode = self._in_message.wire.header._get_function_code()
    ...
    def _switch_buffer(self):
        if self._sqlcode == 0:
            with self._connection._lock:
                self._out_message.wire._write_header(intersystems_iris.dbapi._Message.FETCH_DATA)
                intersystems_iris._MessageHeader._MessageHeader._set_statement_id(self._out_message.wire.buffer, self._statement_id)
    ...
    def fetchone(self):
        ...
        row = []
        self._columns: list[intersystems_iris.dbapi._Column._Column]
    ...
    def _fix_type(self, value, sql_type: SQLType):
        if value is None:
            return value
