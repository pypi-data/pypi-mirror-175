...
def get_agent():
    ...
    if IN_COLAB:
        return f'python-{pip_version}-colab'
    else:
        return f'python-{pip_version}-local'
class HUBLogger:
    ...
    def __init__(self, model_id, auth):
        ...
        self.rate_limits = {'metrics': 1.0, 'ckpt': 900.0, 'heartbeat': 300.0}  # rate limits (seconds)
    ...
    def _upload_model(self, epoch, weights, is_best=False, map=0.0, final=False):
        ...
        if final:
            ...
            smart_request(f'{self.api_url}/upload', data=data, files={"best.pt": file}, headers=self.auth_header, retry=10, timeout=3600, code=4)
        else:
            data = {"epoch": epoch, "type": "epoch", "isBest": bool(is_best)}
            smart_request(f'{self.api_url}/upload', data=data, headers=self.auth_header, files={"last.pt": file}, code=3)

    @threaded
    def _heartbeats(self):
        payload = {"agent": get_agent()}
        while True:
            smart_request(f'{HUB_API_ROOT}/agent/heartbeat/models/{self.model_id}', json=payload, headers=self.auth_header, retry=0, code=5)
