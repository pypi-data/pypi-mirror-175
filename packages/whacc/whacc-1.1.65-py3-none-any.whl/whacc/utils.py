...
def download_resnet_model(overwrite_model=False):
    url = 'https://www.dropbox.com/s/k0398tchgk20lkv/final_resnet50V2_full_model.zip?dl=1'
    ...
    if os.path.isdir(dst):
        if overwrite_model:
            ...
            print('data is already downloaded, if you want to overwrite it set overwrite to True')
            return
...
def equal_distance_pole_sample(h5, numElems=8, equal_x=True, inds_to_bad_trials=None):
    ...
    idx = np.round(np.linspace(0, len(arg_xpoles) - 1, numElems)).astype(int)
...
def make_transfer_learning_data(h5, start_frame, end_frame, equal_x_trial_inds, vid_dir=None, base_dir=None,
    ...
    smooth_by = 5
    if end_frame - start_frame >= smooth_by:
        smooth_by = end_frame - start_frame
        warnings.warn(
            'length of sequential frames selected is larger than the default smoothing kernel of 5, setting smoothing kernel to ' + str(
                smooth_by))
    ...
    with h5py.File(h5, 'r') as h_read:
        for i, (k1, k2) in enumerate(zip(loop_segs[0], loop_segs[1])):
            ...
            pred_labels.append(smooth(mod.predict(x), smooth_by))
    if 'images' in keys:
