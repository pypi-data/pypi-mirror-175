...
bamCoverage -b "$BASE_PATH.with_duplicates.bam" -o "$BASE_PATH.with_duplicates.cpm.bigwig" -bs "$BIGWIG_BIN_SIZE" -e 150 --normalizeUsing BPM -p "$CPU_CORES"
...
mv "$BASE_PATH.with_duplicates.bam" "$ORIGINAL_BASE_PATH.with_duplicates.bam" &
mv "$BASE_PATH.with_duplicates.bam.bai" "$ORIGINAL_BASE_PATH.with_duplicates.bam.bai" &
mv "$BASE_PATH.cpm.bigwig" "$ORIGINAL_BASE_PATH.cpm.bigwig" &
mv "$BASE_PATH.cpm.bigwig" "$ORIGINAL_BASE_PATH.with_duplicates.cpm.bigwig" &
wait
