...
@click.option(
    ...
    required=True,
    help="""
    The path to the image .tar/.tar.bz2/.tar.gz/.tar.xz as produced by Yocto's
    do_image_*. The file is expected to reside in the location where Yocto
    produced the file (tmp/deploy/images/${PACKAGE_ARCH}/images). Additionally,
    an image with the same name but ending in -dbg.tar/.tar.bz2/.tar.gz is
    expected to reside next to the main image. To generate it alongside the
    required elfutils-native, add the following to your main image (note that
    the main image will not be affected except for its build dependencies):
    ...
    **kwargs,
    ...
    Example Yocto Symbol Upload:
        ...
        $ memfault --org-token $ORG_TOKEN \\
            ...
            upload-yocto-symbols --image build/tmp/deploy/images/raspberrypi3/my-image-raspberrypi3.tar.bz2
    """
    ctx.obj.update(**kwargs)
    ...
    process_and_upload_yocto_symbols(ctx, image, build_env=build_env)
