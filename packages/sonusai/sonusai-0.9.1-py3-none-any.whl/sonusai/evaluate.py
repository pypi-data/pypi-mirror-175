...
usage: evaluate [-hv] (d- MIXDB) [-i MIXID] (-f FEATURE) (-p PREDICT) [-t PTHR]
...
options:
    ...
    -f FEATURE, --feature FEATURE   A directory containing genft data.
...
Inputs:
    ...
    FEATURE     A SonusAI genft data directory. Contains:
...
def main():
    ...
    with h5py.File(feature_name, 'r') as f:
        mixdb = convert_json_to_mixdb(f.attrs['mixdb'])
