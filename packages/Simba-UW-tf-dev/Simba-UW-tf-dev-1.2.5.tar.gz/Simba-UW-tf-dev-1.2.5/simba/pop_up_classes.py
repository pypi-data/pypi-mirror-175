...
class MakePathPlotPopUp(object):
    def __init__(self):
        ...
        video_path = FileSelect(settings_frm, 'VIDEO PATH: ', lblwidth='10')
        body_part = Entry_Box(settings_frm, 'BODY PART: ', '10')
        csv_path = FileSelect(settings_frm, 'CSV PATH: ', lblwidth='10')
        color_lst = ['White',
                     'Black',
                     'Grey',
                     'Red',
        ...
        run_btn = Button(settings_frm,text='CREATE PATH PLOT VIDEO',command =lambda: draw_line_plot_tools(video_path.file_path, csv_path.file_path, body_part.entry_get))
        settings_frm.grid(row=0,sticky=W)
        video_path.grid(row=0,sticky=W)
        csv_path.grid(row=1,sticky=W)
