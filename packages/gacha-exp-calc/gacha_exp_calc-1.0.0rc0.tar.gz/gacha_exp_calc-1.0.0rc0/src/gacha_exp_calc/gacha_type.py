...
from gacha_exp_calc.gacha_func import judge_exp

class step():
    def __init__(self,p:float,p_up:float|list,ups:int,thres:int,most:float,mg:int,mg_type:str='default',*args,**kwargs) -> None:
        '''
        p:最高稀有度出率
        p_up:up占最高稀有度比例，或up出率列表
        ups:up角色数量, 如果p_up为up出率列表则为None
        ...
        if ups==None:
            ...
            try:
                self.p_up=[(_+1)*p*p_up for _ in range(ups)]
            except:
                raise TypeError('mismatch between p_up and ups: should be float-int or list-None pair')
        step.ups=ups
        step.thres=thres
    ...
    def smlt(self,n:int,e:list,t:int=0,times:int=100000,detail:bool=False) -> float:
        '''
        n为总抽数
        e为期望抽卡结果（数组形式表示）
        t为未出最高稀有度角色抽数
    ...
    def clmp_n(self,e:list,target:float=0.95,lower=0,upper=None,fdis=None) -> int:
        ...
        '''
        if upper==None:
            upper=sum(e)*int((self.most if self.most>1 else self.thres+self.p*round((1-self.p)/self.most))/self.p_up[-1])
        if fdis==None:
            fdis=upper-lower
    ...
    def calc(self,n:int,e:list,t:int=0,rel_exp:int=6) -> float:
        ...
        if (self.mg_type=="mg" and self.most and (n>=(self.mg+1)*(self.most if self.most>1 else self.thres+round((1-self.p)/self.most))*sum(e)))\
...
class fixed():
    def __init__(self,p:float,p_up:list,most:int=0,*args,**kwargs):
        '''
        p:最高稀有度出率
        p_up:up角色出率
        most:保底抽数, 0为无保底机制
    ...
    def smlt(self,n:int,e:list,times=100000) -> float:
        result=0
        up=dict()
        for i in range(times):
            count=[0 for _ in range(len(self.p_up))]
            for j in range(n):
                cache=random.random()
                if cache<self.p:
                    for k in range(len(self.p_up)):
                        if cache<self.p_up[k]:
                            count[k]+=1
                            break
            up[tuple(count)]=up.get(tuple(count),0)+1
        for c in up.keys():
            if judge_exp(c,e):
                result+=up[c]
        return eval('%.4f'%(result/times))

    def prob(self,n:int,e:list) -> float:
