...
def cli_install_one_shot_task(task: Task, verbose=False) -> Result:
    ...
    # Run the Nornir Netmiko task netmiko_send_command
    # As the command is really long the task stop when the expect_string is seen. When the package activating
    ...
    if "install_add_activate_commit: Activating PACKAGE" in task_result.result:
        result_summary = f"'{task.name}' -> CliResponse <Success: True>"
        # Define the custom_result variable for print_result
        custom_result = result_summary + "\n\n" + task_result.result if verbose else result_summary

        # Return the custom Nornir result as success
        return Result(host=task.host, custom_result=custom_result)

    # Else the one-shot install failed
    result_summary = f"'{task.name}' -> CliResponse <Success: False>\n\n" f"{task_result.result}"
