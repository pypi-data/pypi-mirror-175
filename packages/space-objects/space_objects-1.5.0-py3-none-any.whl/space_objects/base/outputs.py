...
class OrbitObject:
    ...
    def _get_parts_min_max(self):
        elems = []
        if self.part_one:
            elems = [self.part_one[0], self.part_one[-1]]
        if self.part_two:
            elems = [*elems, self.part_two[0], self.part_two[-1]]
        return elems

    @property
    def date_end(self):
        _ = max(self._get_parts_min_max(), key=lambda e: e.timeposition_utc)
        return _.timeposition_utc

    @property
    def date_start(self):
        _ = min(self._get_parts_min_max(), key=lambda e: e.timeposition_utc)
        return _.timeposition_utc
