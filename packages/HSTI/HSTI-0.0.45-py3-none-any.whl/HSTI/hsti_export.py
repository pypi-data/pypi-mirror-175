...
def export_data_cube(data_cube, directory, rotate = False, generate_folders = False, normalize_each_band = False):
    """
    ########## HSTI_export ##########
    ...
    if normalize_each_band:
        for i in range(data_cube.shape[2]):
            data_cube[:,:,i] = 65535.9*(data_cube[:,:,i] - np.min(data_cube[:,:,i]))/(np.max(data_cube[:,:,i]) - np.min(data_cube[:,:,i]))
    else:
        data_cube = 65535.9*(data_cube - np.min(data_cube))/(np.max(data_cube) - np.min(data_cube))
