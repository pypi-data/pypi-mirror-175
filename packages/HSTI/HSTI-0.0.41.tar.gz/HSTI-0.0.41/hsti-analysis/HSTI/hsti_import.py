...
def import_data_cube(path, file_format = '.ppm', verbose = False):
    """
    ########## import_data_cube ##########
This function imports images of the stated file type (.ppm by default) from the
provided path. The functions first looks for images at the provided distination,
...
parameter is set.
    ...
    try:
        if (os.path.exists(path)) and (sum([ file_format in s for s in os.listdir(path)]) > 0):
            new_path = path
        elif (os.path.exists(f"{path}/capture/")) and (sum([ file_format in s for s in os.listdir(f"{path}/capture/")]) > 0):
            new_path = f"{path}/capture/"
        elif (os.path.exists(f"{path}/images/capture/")) and (sum([ file_format in s for s in os.listdir(f"{path}/images/capture/")]) > 0):
            new_path = f"{path}/images/capture/"

        file_lst = [elem for elem in os.listdir(new_path) if file_format in elem]
        file_lst = natsorted(file_lst)
        imgs = []
        for img in file_lst:
            imgs.append(np.rot90(cv.imread(f'{new_path}{img}',cv.IMREAD_ANYDEPTH)))
        ...
        print('Chosen path either does not exist or contains no images of provided file type')
