...
class GeneralisedDebyeModel(Model):
    ...
    def write_fit_params(models: list[Model], file_name: str):
        ...
        print("\nFit information has been written to {}\mn".format(
            file_name
        ))
        ...
        with open(file, 'r') as f:
            for it, line in enumerate(f):
                if data_header in line:
                    header_line = it + 1
        # If data_pointer is not present, raise and error.
        try:
            header_line
        except UnboundLocalError:
            print(
                    '\n    ***Error*** \n',
                    '    --data_pointer "{}"'.format(data_header),
                    'is not present in {}\n'.format(file)
                    )
            exit()
...
class Experiment():
    ...
    def from_measurements(cls,
                          ...
                          field_thresh: float = 1e-8) -> list[list['Experiment']]: # noqa
        """
