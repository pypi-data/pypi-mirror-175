...
class DataProcessor:
    ...
    def handle_outliers(self, data_frame):
    ...
    def handle_missing(self, data_frame):
        df = data_frame
    ...
    def process_data(self):
        ...
        if self.config.handle_missing:
            self.data_frame = self.handle_missing(self.data_frame)
        if self.config.label_encode or self.config.one_hot_encode:
            self.data_frame = self.encode(cat_cols)
        if self.config.remove_outliers:
            self.data_frame = self.handle_outliers(self.data_frame)
    ...
    def process_as_previous(self, df):
        ...
        if self.config.remove_outliers:
            self.data_frame = self.handle_outliers(df)
