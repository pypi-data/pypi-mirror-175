...
class Operator:
    ...
    def draw_auto_exit_pic(self, picture):
        ...
        # full = (0,0,2592,1944)
        im_r = img.crop((int(2592/2-600), int(1944/2-400), int(2592/2+600), int(1944/2+200)))
        img_byte_arr = io.BytesIO()
        im_r.save(img_byte_arr, format='PNG')
        img_byte_arr = img_byte_arr.getvalue()
        self.photoimg = ImageTk.PhotoImage(Image.open(io.BytesIO(img_byte_arr)))
        self.currentPage.can.create_image(self.terminal.w/2, self.terminal.h/3,
                                          image=self.photoimg,
                                          tags=('orupentry',))
