...
def _convert_project(app: str = "source/app.py", dest_path: str = "build", requirements="auto"):
    ...
    if not ((build_dir / "app.html").exists() and (build_dir / "app.js").exists()):
        temp_out = StringIO()
        sys.stdout = temp_out
        convert_app(
            app=Path(app), # type: ignore
    ...
    if (build_dir / "app.html").exists() and (build_dir / "app.js").exists():
        return

    if out.startswith("Failed to convert"):
        out = """\n\nFailed to convert. Make sure you are 1) marking items .servable() and 2) only
importing packages available on the server."""

    app_html_path = Path(dest_path) / "app.html"
    app_html_path.parent.mkdir(parents=True, exist_ok=True)
    app_html_path.write_text(
        f"""
{ALERT_STYLE}<div class="bk alert alert-danger">{out}</div>
"""
    )
