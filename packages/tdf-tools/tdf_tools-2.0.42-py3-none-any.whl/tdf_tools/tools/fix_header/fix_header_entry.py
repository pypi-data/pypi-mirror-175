import os
from tdf_tools.tools.translate.ios.tools.batch_pod_tools import (
    BatchPodTools,
)
from tdf_tools.tools.cmd import Cmd
from tdf_tools.tools.fix_header.fix_header_lint_tool import FixHeaderLintTool
from tdf_tools.tools.fix_header.fix_header_replace_tool import FixHeaderReplaceTool
...
class FixHeaderEntry:
    ...
    def start(self):
        ...
        while True:
            targetModuleName = input("请输入需要执行修复头文件脚本的模块名(input ! 退出，all 所有模块执行)：")
            if targetModuleName == "!" or targetModuleName == "！":
                exit(0)
            elif targetModuleName == "all":
                if self.fix_type == FixHeaderType.REPLACE:
                    path = os.getcwd()
                    BatchPodTools.generate_pod_source_files(path, batchPodList)
    ...
    def lint_path(self, path: str):
        ...
        if podpsec_path != None:
            self.__lint_header(path, podpsec_path)
        else:
            Print.error(path + "路径下不存在podspec文件")

    def __lint_header(self, path: str, podspec_path: str):
        lint_header = FixHeaderLintTool()
        lint_header.lint(path, podspec_path)
        Print.stage("{0} 路径下头文件 lint 成功".format(path))
