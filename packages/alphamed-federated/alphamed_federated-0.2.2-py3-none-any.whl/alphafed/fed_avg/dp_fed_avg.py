...
class DPFedAvgScheduler(FedAvgScheduler, metaclass=ABCMeta):
    ...
    def _setup_context(self, id: str, task_id: str, is_initiator: bool = False):
        ...
        if self.involve_aggregator:
            loader = self.make_train_dataloader()
            self._w_ks[self.id] = self._calc_w_k(len(loader.dataset))
    ...
    def _check_in_task(self) -> bool:
        ...
        nonce = self.contractor.checkin(peer_id=self.id, n_k=len(train_loader.dataset))
    ...
    def _process_aggregation(self):
        ...
        with torch.no_grad():
            ...
            _model = self.make_model()
            _model.load_state_dict(state_dict_0)
            self._theta_0 = deepcopy(tuple(_model.parameters()))
            ...
            for _peer_id, _state_dict in state_dicts.items():
                ...
                for _param, _param_0 in zip(_model.parameters(), self._theta_0):
                    if not _param.requires_grad:
                        continue
                    _param.sub_(_param_0)
                for _delta_param, _accum_delta_param in zip(_model.parameters(), accum_delta):
                    _accum_delta_param.add_(_delta_param * w_k / self.q / self.W)

            sigme = self.z * self.S / self.q / self.W
            _model.load_state_dict(state_dict_0)
            for _param, _delta in zip(_model.parameters(), accum_delta):
                if not _param.requires_grad:
                    continue
                _param.add_(_delta)
                _param.add_(torch.normal(mean=0.0, std=sigme ** 2, size=_param.size()))

            new_state_dict = _model.state_dict()
