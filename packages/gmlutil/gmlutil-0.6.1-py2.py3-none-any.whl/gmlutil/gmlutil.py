...
benchmark_geo_str = 's3://ejgallo-lake-sales-dev/prophet/Data_Acquisition/source_data/Archive/2021/benchmark/data/geo_codes.csv'
...
class data_extraction:
    ...
    def lc_reading_in_from_s3(self, bucket_name, file_name, dtypes = None):
        ...
        df = pd.read_csv(io.BytesIO(obj['Body'].read()) , encoding='ISO-8859-1', low_memory=False, dtype = dtypes) # , on_bad_lines='skip')
        return df
...
class data_conversion:
    ...
    def data_conversion_num(self, df, bucket_name, directory_name, label_name, scaler_type = "minmax"):
        ...
        if self.numerical_list != []:
            print("Detected numerical list for sustainable growth...")
            if label_dict == {}:
                ...
                #     label_dict[variable] = scaler
                self.save_labels(scaler,  bucket_name, directory_name, label_name)
            else:
                # for variable in self.numerical_list:
                #     scaler = label_dict[variable]
                #     df_num.loc[:,variable] = scaler.transform(df_num.loc[:,variable])
                scaler = label_dict
