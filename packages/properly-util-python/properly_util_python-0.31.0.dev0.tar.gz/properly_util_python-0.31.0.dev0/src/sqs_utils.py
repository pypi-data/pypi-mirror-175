...
def add_to_queue(queue_name, message_attributes: dict, message_body=""):
    ...
    try:
        if not queue_exists(queue_name):
            ...
            response = sqs.create_queue(QueueName=queue_name, Attributes={"MessageRetentionPeriod": "345600"})  # 4 days
        else:
            response = sqs.get_queue_url(QueueName=queue_name)
        ...
        response = sqs.send_message(
            QueueUrl=queue_url, DelaySeconds=10, MessageAttributes=sqs_message_attributes, MessageBody=message_body
        )
    except Exception as e:
        print("ClientError e:", e)
        print("traceback", traceback.format_exc())
...
def delete_from_queue(queue_name, receipt_handle):
    response = sqs.get_queue_url(QueueName=queue_name)
    sqs.delete_message(QueueUrl=response["QueueUrl"], ReceiptHandle=receipt_handle)
