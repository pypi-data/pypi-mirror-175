...
from src.dynamo_helper import DynamoHelperBase, EnvironmentControlledDynamoHelper
from src.environments import get_properly_stage_for_resource
from src.helper_utils import dict_to_dynamo, dynamo_to_dict
from src.properly_logging import ProperLogger

logger = ProperLogger("src.dynamo_utils")
...
def save_to_dynamo(
    table_name, item: dict, dynamodb_helper: DynamoHelperBase = EnvironmentControlledDynamoHelper(), *args, **kwargs
):
    ...
    response = table.get_item(TableName=table_name, Key={"id": item["id"]})
...
def get_dynamo_item(
    ...
    resp = deal_table.get_item(TableName=table_name, Key={item_key: item_value})
    item = resp.get("Item")
...
def query_table(table_name, filter_key=None, filter_value=None, filter_type="eq"):
    ...
    if filter_type == "not_exists":
        table_filter = {"FilterExpression": Attr(filter_key).not_exists()}
    elif filter_type == "lt":
        table_filter = {"FilterExpression": Attr(filter_key).lt(filter_value)}
    elif filter_type == "contains":
        table_filter = {"FilterExpression": Attr(filter_key).contains(filter_value)}

    else:
        table_filter = {"FilterExpression": Attr(filter_key).eq(filter_value)}
...
def get_last_key(
    config_id, table_name, META_KEY_MAP, dynamodb_helper: DynamoHelperBase = EnvironmentControlledDynamoHelper()
):
...
#
# from src.distributed_property import DistributedPropertyTuple
#
#  ...
