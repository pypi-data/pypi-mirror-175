...
async def _get_nft_items_by_owner_address(client: TonlibClient, addr: str):
    ...
    client = await _get_client(client.ls_index)

    async def proc_tr(tr):

        if tr['type'] == 'in':
            if 'EQBdHpUa1u3IVs1b3xfjpSlXVD5lkNsfIcJMqtXu1IJ0rLHK' == tr['from'] or 'EQBqkkKeF7cwbX-erNK73qs4fqpwkXNQd_yqS4HXzp3PaCSX' == tr['from']:
                print(tr)
...
async def _get_collection_items(client: TonlibClient, addr: str):
    ...
    async def get_item_addr(index, result):
        ...
        result.append(item['address'].to_string()) # raw form
    ...
    if len(result) < items_amount:
        raise Exception('GOT NOT ALL ITEMS')
    return result
