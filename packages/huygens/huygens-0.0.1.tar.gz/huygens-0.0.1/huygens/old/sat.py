...
class Add(Expr):
    ...
    def get_affine(self):
        lin, a = {}, 0.
        for item in self.items:
            _lin, _a = item.get_affine()
            a += _a
            for k,v in _lin.items():
                lin[k] = lin.get(k, 0.) + v
        return lin, a
...
class Matrix(object):
    def __init__(self, rows, cols):
        self.rows = list(rows)
        self.cols = list(cols)
        self.rowlookup = dict((row, idx) for (idx, row) in enumerate(rows))
        self.collookup = dict((col, jdx) for (jdx, col) in enumerate(cols))
        self.data = {}
        self.shape = (len(rows), len(cols))

    def __getitem__(self, key):
        row, col = key
        idx, jdx = self.rowlookup[row], self.collookup[col]
        key = (idx, jdx)
    ...
    def swap(self, i, j):
        ...
        rowlookup[i], rowlookup[j] = rowlookup[j], rowlookup[i]

    def __str__(self):
        data = self.data
        rows = [[str(self[i,j] or  '.') for j in self.cols] for i in self.rows]
...
def gaussian_eliminate(A, b):
    ...
    idx = 0
    jdx = 0
    while idx < len(rows) and jdx < len(cols):
        col = cols[jdx]
...
def simplify(solver):
    items = solver.items
    all_vars = solver.all_vars
    print("gaussian_eliminate", len(items), len(all_vars))
