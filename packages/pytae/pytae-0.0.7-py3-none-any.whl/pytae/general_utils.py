...
class ProcessTar(object):
    ...
    def return_df_orig(self,k,cols=None): 
        self.cols=cols
        self.k=k
            ...
            for item in tar.getnames():
                ...
                if self.k==file:
                    if cols==None:
                        ...
                        df= pd.read_csv(tar.extractfile(item),low_memory=False,usecols=self.cols)
    ...
    def return_df(self,f=None,cols=None):
        self.f=f
        self.cols=cols
                ...
                with tarfile.open(self.tar_path, "r:*") as tar:
                    for item in tar.getnames(): 
                        if cols==None:
                            ...
                            df=d.read_csv(tar.extractfile(item),low_memory=False,usecols=self.cols)
                        return df
            else:
