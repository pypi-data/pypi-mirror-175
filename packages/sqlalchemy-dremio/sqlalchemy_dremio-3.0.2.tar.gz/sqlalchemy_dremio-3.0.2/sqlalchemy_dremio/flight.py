...
class DremioDialect_flight(default.DefaultDialect):
    ...
    def create_connect_args(self, url):
        ...
        # Clone url.query but make the keys lower-case.
        url_extras_lc = {k.lower():v for k,v in url.query.items()}    

        def add_property(url_extras_lc, property_name, connectors):
            if property_name.lower() in url_extras_lc:
                connectors.append('{0}={1}'.format(property_name, url_extras_lc[property_name.lower()]))
        
        add_property(url_extras_lc, 'UseEncryption', connectors)
        add_property(url_extras_lc, 'DisableCertificateVerification', connectors)
        add_property(url_extras_lc, 'TrustedCerts', connectors)
        add_property(url_extras_lc, 'routing_queue', connectors)
        add_property(url_extras_lc, 'routing_tag', connectors)
        add_property(url_extras_lc, 'quoting', connectors)
        add_property(url_extras_lc, 'Token', connectors)
