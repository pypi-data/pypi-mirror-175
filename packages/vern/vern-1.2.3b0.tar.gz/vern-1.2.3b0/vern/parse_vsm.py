...
def read_files(input_path, csv_path, plot_path, hist_path, interactive=False):
    ...
    df = pd_read_csv(
        ...
        names_old=["magnetic field (Oe)", r"magnetization ($\textrm{erg}/\textrm{cm}^{3}$)"], names_new=["magnetic field (Oe)", r"magnetization ($\textrm{erg}/\textrm{cm}^{3}$)"], unit_conversion_coefficients=[1, 1]
    )
