...
class downloader(Thread):
    ...
    def merge(self,path="./result.tif")
        print("# ---------------------- Merge tiles to whole tiff file ---------------------- #")
        ...
        output_array=np.array(output_array.convert("RGB"))
        print("# ----- image size",output_array.shape)
        image=Image.fromarray(output_array)
        channels = image.split()
        gt=(
            self.extent['LT'][0],
            (self.extent['RB'][0] - self.extent['LT'][0]) / channels[0].shape[1],
            0,
            self.extent['LT'][1],
            0,
            (self.extent['RB'][1] - self.extent['LT'][1]) / channels[0].shape[0]
        )
        if len(channels)==3:
            print("# ----- 3 channel output")
            saveTiff(channels[0],channels[1],channels[2],gt,path)
        else:
            print("# ----- not 3 channels , only support single channel output")
            saveTiff(channels[0],channels[0],channels[0],gt,path)
