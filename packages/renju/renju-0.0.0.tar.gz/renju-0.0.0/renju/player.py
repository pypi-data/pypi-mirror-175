...
class Player:
    ...
    def __init__(self, name, config):
        ...
        self.w2 = (math.sqrt(config['w2']) * math.log(10.) /
                   400.)**2.  # Convert from elo^2 to r^2
    ...
    def log_likelihood(self):
        ...
        for i in range(n):
            ...
            if i < n - 1:
                ...
                prior += (1. /
                          (math.sqrt(2 * math.pi * sigma2[i]))) * math.exp(
                              -(rd**2) / 2. / sigma2[i])
            if i > 0:
                rd = self.days[i].r - self.days[i - 1].r
                prior += (1. /
                          (math.sqrt(2 * math.pi * sigma2[i - 1]))) * math.exp(
                              -(rd**2) / 2. / sigma2[i - 1])
            if prior == 0:
                sum_ += self.days[i].log_likelihood()
            else:
                if math.isinf(self.days[i].log_likelihood()) or math.isinf(
                        math.log(prior)):
                    print("Infinity at %s: %f + %f: prior = %f, days = %s" %
                          (self.inspect(), self.days[i].log_likelihood(),
                           math.log(prior), prior,
                           map(lambda d: d.inspect(), self.days)))
                    exit(0)
    ...
    def gradient(self, r, days, sigma2):
        ...
        for idx in range(n):
            ...
            if self.debug:
                print("g[%d] = %f + %f" %
                      (idx, day.log_likelihood_derivative(), prior))
    ...
    def update_by_ndim_newton(self):
        ...
        if self.debug:
            ...
            for day in self.days:
                ...
                print("day[%d] win terms = %s" %
                      (day.day, day.won_game_terms()))
                print("day[%d] win games = %s" % (day.day, day.won_games))
                print("day[%d] draw terms = %s" %
                      (day.day, day.draw_game_terms()))
                print("day[%d] draw games = %s" % (day.day, day.draw_games))
                print("day[%d] lost terms = %s" %
                      (day.day, day.lost_game_terms()))
                print("day[%d] lost games = %s" % (day.day, day.lost_games))
                print("day[%d] log(p) = %f" % (day.day, day.log_likelihood()))
                print("day[%d] dlp = %f" %
                      (day.day, day.log_likelihood_derivative()))
                print("day[%d] dlp2 = %f" %
                      (day.day, day.log_likelihood_second_derivative()))
        ...
        for r_ in new_r:
            if r_ > 650:
                raise UnstableRatingException("Unstable r (%s) on player %s" %
                                              (new_r, self.inspect()))
