...
class PlayerDay:
    ...
    def won_game_terms(self):
        if self.won_game_terms_cache == None:
            ...
            for g in self.won_games:
                ...
                if other_gamma == 0. or math.isnan(other_gamma) or math.isinf(
                        other_gamma):
                    print("other_gamma (%s) = %s" %
                          (g.opponent(self.player).inspect(), other_gamma))
        ...
        if self.draw_game_terms_cache == None:
            ...
            for g in self.draw_games:
                ...
                if other_gamma == 0. or math.isnan(other_gamma) or math.isinf(
                    ...
                    print("other_gamma (%s) = %s" %
                          (g.opponent(self.player).inspect(), other_gamma))
                self.draw_game_terms_cache.append(
                    [0.5, 0.5 * other_gamma, 1.0, other_gamma])
            if self.is_first_day:
                for _ in range(self.virtual_games):
                    self.draw_game_terms_cache.append([
                        0.5, 0.5, 1.0, 1.0
                    ])  # draw with virtual player ranked with gamma = 1.0
        ...
        if self.lost_game_terms_cache == None:
            ...
            for g in self.lost_games:
                ...
                if other_gamma == 0. or math.isnan(other_gamma) or math.isinf(
                    ...
                    print("other_gamma (%s) = %s" %
                          (g.opponent(self.player).inspect(), other_gamma))
                self.lost_game_terms_cache.append(
    ...
    def log_likelihood_second_derivative(self):
        sum_ = 0.0
        gamma = self.gamma()
        for _, _, c, d in (self.won_game_terms() + self.draw_game_terms() +
    ...
    def log_likelihood_derivative(self):
        ...
        for _, _, c, d in (self.won_game_terms() + self.draw_game_terms() +
                           self.lost_game_terms()):
            tally += c / (c * gamma + d)
        return len(self.won_game_terms()) + 0.5 * len(
            self.draw_game_terms()) - gamma * tally
