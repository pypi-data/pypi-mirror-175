...
from libsan.host.cmdline import exists
...
def if_down(nic_or_mac):
    ...
    if not exists('ifdown') and libsan.host.linux.is_installed('NetworkManager'):
...
def if_up(nic_or_mac):
    ...
    # ifup/ifdown scripts are not shipped by default on RHEL-8+
    if not exists('ifup') and libsan.host.linux.is_installed('NetworkManager'):
        if not libsan.host.linux.install_package('NetworkManager-initscripts-updown'):
...
def set_ifcfg(nic_or_mac, parameters):
    ...
    libsan.host.linux.aug_set_config('/etc/sysconfig/network-scripts/ifcfg-' + nic, parameters)
...
def set_iface(nic_or_mac, **kwargs):
    ...
    Parameters:
    \t nic_or_mac: interface name or mac address
    \t key=value: e.g. IPADDR='10.37.151.7'
    """
    if is_mac(nic_or_mac):
        nic = get_nic_of_mac(nic_or_mac)
        if not nic:
...
def nm_add_conn(name=None, nic_or_mac=None, nic_type='ethernet'):
    ...
    cmd = "nmcli conn add type %s con-name %s" % (nic_type, name)
    if nic:
        cmd += " ifname %s" % nic
    if '_' in cmd:
        cmd = cmd.replace("_", "-")
...
def nm_conn_mod(conn, key, value):
    ...
    retcode, output = run(cmd, return_output=True, verbose=False)
    if retcode != 0:
        print("FAIL: Unable to modify conn %s with cmd %s " % (conn, cmd))
        print(output)
        return False
    return True
...
def nm_dev_mod(dev, key, value):
    ...
    retcode, output = run(cmd, return_output=True, verbose=False)
    if retcode != 0:
        print("FAIL: Unable to modify dev %s with cmd %s " % (dev, cmd))
        print(output)
        return False
    return True
