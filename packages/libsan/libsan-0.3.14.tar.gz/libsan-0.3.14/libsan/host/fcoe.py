...
def configure_intel_fcoe(nic, mode):
    ...
    retcode, output = run(r"sed -i -e 's/\(MODE=.*\)$/MODE=\"%s\"/' /etc/fcoe/cfg-%s" % (mode, nic), return_output=True)
    if retcode != 0:
        print(output)
        return False
...
def configure_bnx2fc_fcoe(nic, mode):
    ...
    retcode, output = run(r"sed -i -e 's/\(DCB_REQUIRED=.*\)$/DCB_REQUIRED=\"no\"/' /etc/fcoe/cfg-%s" % nic,
                          return_output=True)
    if retcode != 0:
        print(output)
        return False
