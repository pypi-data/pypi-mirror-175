        ...
        if i > 0:
            ...
            if time_diff.delta / 1e9 != timestep:
                ...
                if print_all or (msg_counter <= print_max - 1):
                    if verbose:
                        print(
                            f"Warning: Gap in time series found between {dataframe.index[i - 1]} and {dataframe.index[i]}"
                        )
                    msg_counter += 1
                if msg_counter == print_max and verbose and not warning_printed:
                    print(
                        "Warning: Only the first {msg_counter} gaps have been listed. Try to increase print_max parameter to show more details."
                    )
                    warning_printed = True
    if verbose:
        print("{gcount} gaps found in total.")
    return gcount
