"""
Tools for reading data and accompanying metadata.
"""
...
# TODO: Generalize this beyond file-based datasets.
...
class Interface(iterables.ReprStrMixin):
    """A format-agnostic view of a generic dataset."""

    def __init__(self, __source) -> None:
    ...
    @property
    def viewers(self):
        """The available dataset viewers."""
        if self._viewers is None:
            self._viewers = ViewerFactory(self._source)
...
class File(Interface, metaclass=iotools.PathSet):
    """A format-agnostic view of a file-based dataset.
    ...
    def __init__(self, __source: iotools.PathLike) -> None:
        super().__init__(__source)

    @property
    def path(self):
        """The path to this dataset."""
        iotools.ReadOnlyPath(self._source)


class Set(abc.ABC, typing.Generic[T]):
    """Abstract interface to a generic dataset."""
