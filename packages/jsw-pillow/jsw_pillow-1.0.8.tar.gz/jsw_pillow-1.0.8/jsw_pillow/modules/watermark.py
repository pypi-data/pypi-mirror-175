...
angle: 旋转角度
opacity: 透明度
save_mode: 保存模式，file 为保存到文件，b64 为返回 base64 编码, buffer 为返回 io.BytesIO 对象
"""
...
class Watermark:
    ...
    def _postprocess(cls, **kwargs):
        ...
        if save_mode == 'buffer' or save_mode == 'b64':
            buffer = io.BytesIO()
            combined_image.save(buffer)

            if save_mode == 'b64':
                buffer.seek(0)
                buffer = base64.b64encode(buffer.read())

        return combined_image, target, buffer
