...
class Watermark:
    ...
    def _preprocess(cls, **kwargs):
        # 参数处理
        source, is_str = cls._normalize_source(kwargs.get('source'))
        save_mode = kwargs.get('save_mode', 'file')  # FILE or BUFFER
        opacity = kwargs.get('opacity', 0.5)
        if save_mode == 'file':
            filename, ext = os.path.splitext(source) if is_str else ('tmp', '.jpg')
    ...
    def _postprocess(cls, **kwargs):
        ...
        if save_mode == 'buffer':
            target = io.BytesIO()
            combined_image.save(target, format='PNG')
    ...
    @classmethod
    def _normalize_source(cls, target):
