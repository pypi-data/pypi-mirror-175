...
class RacingDataAnalyzer:
    ...
    def reports_api(self, direction: bool) -> List[Racer]:
        """
        Return report that shows the 15 racers(asc or desc) and the rest after underline.
        Parameters
        :param direction:
              Sort list of drivers and order by [--asc | --desc].
        """
        sorted_data = sorted(self.racer_data, key=lambda time: time.lap_time, reverse=direction)
        return list(enumerate(sorted_data, start=1))

    def single_racer_api(self, driver_code: str) -> List[Racer]:
        """
        Return driver info.
        Parameters
        :param driver_code
