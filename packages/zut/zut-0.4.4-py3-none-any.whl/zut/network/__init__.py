...
def check_urllib_connectivity(url: str, expected_regex: re.Pattern|str, label=None, timeout: float = None) -> str:
    """
...
def check_connectivity(url: str, expected_regex: re.Pattern|str, label=None, timeout: float = None) -> str:
    """
    ...
    Return a failure message (None on success).
    """ 
    failure = check_urllib_connectivity(url, expected_regex, label=label, timeout=timeout)

    if sys.platform == "win32":
        from .windows import check_winhttp_connectivity
        winhttp_failure = check_winhttp_connectivity(url, expected_regex, label=label, timeout=timeout)
        if winhttp_failure:
            failure = (failure + '\n' if failure else '') + winhttp_failure
