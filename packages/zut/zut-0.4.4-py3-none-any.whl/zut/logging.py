...
def configure_logging(level=None, default_level=None, format=None, filename=None, filelevel=None, fileformat=None, nocount=False, nocolor=False):
    if level is None:
        level = os.environ.get("LOGLEVEL", None)
        if not level:
            level = default_level if default_level else logging.INFO
