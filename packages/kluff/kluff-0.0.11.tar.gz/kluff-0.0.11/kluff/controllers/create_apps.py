...
class AppSetup(Controller):
    ...
    def run(self):
        try:
            ...
            self.backend_image_tag = f'{plugin_name}:backend'
            self.frontend_image_tag = f'{plugin_name}:frontend'

            os.system(f'cd {self.backend_path} && docker build -t {self.backend_image_tag} .')
            os.system(f'docker run -it -p 5000:5000 -d {self.backend_image_tag}')

            os.system(f'cd {self.frontend_path} && docker build -t {self.frontend_image_tag} .')
            os.system(f'docker run -it -p 3000:3000 -d {self.frontend_image_tag}')
    ...
    def deploy(self):
        ...
        try:
            ...
            os.system(f'docker build -t {self.backend_image_tag} .')
            os.system(f'docker tag {self.backend_image_tag} {self.container_registry}/{self.backend_image_tag}')
            os.system(f'docker push {self.container_registry}/{self.backend_image_tag}')
        except OSError as e:
            raise CommandError(e)
