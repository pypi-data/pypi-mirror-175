...
def get_value_from_datagroups_dict(item, key):
    value = item.get(key, None)  # str(item[key])
    value = str(value)
    return value


def get_titles_and_ids_for_selection(json_content):
    new_list = []
    for item in json_content:
        title = get_value_from_datagroups_dict(item, 'DATAGROUP_NAME_ENG')
        code = get_value_from_datagroups_dict(item, 'DATAGROUP_CODE')
        new_list.append((code, title))
    return new_list


def show_datagroups_summary(json_content):
    keys = get_keys()
    for item in json_content:
        new_list = []
        for key in keys:
...
def json_to_df(json_content: list, code: int):
...
def get_and_process_datagroups_with_code(code=1):
    json_content: list = get_datagroups_with_code(code)
