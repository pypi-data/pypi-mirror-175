...
class Base(Controller):
    ...
    def cai(self):
        ...
        if self.app.pargs.text=='ls':
            ...
            if self.app.pargs.model != 'all':
                # check if each row contains the model string
                if self.app.pargs.model:
                    df = df[df['root'].str.contains(self.app.pargs.model)]
            ...
            suggestions = get_cai_suggestions(self.app.pargs.text, self.app.pargs.n_suggestions, self.app.pargs.model)
            try:
