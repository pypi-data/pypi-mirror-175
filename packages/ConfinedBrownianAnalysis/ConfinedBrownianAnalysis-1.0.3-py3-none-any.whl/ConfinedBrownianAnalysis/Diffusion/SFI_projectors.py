...
class TrajectoryProjectors(object):
    ...
    def __init__(self, inner_product, functions, is_interacting):
        ...
        try:
            ...
            print(
                "Warning, singular normalization matrix (redundant basis or dataset too small), using pseudo-inverse."
            )
    ...
    def projector_combination(self, c_coefficients):
        ...
        return function, 1.0 * b_coefficients
    ...
    def truncated_projector_combination(self, truncated_c_coefficients):
        ...
        b_coefficients = np.einsum(
            "...a,ba->...b",
            truncated_c_coefficients,
            self.H[:truncation_order, :truncation_order],
        )
        function = lambda x: np.einsum(
            "ia,...a->i...", self.b(x)[:, :truncation_order], b_coefficients
        )
        return function, 1.0 * b_coefficients
    ...
    def grad_b(self, x, epsilon=1e-6):
        ...
        dx = [
            [
                np.array(
                    [
                        [0 if (i, m) != (ind, mu) else epsilon for m in range(d)]
        ...
        return np.array(
            [
                [
                    (self.b(x + dx[ind][mu]) - self.b(x - dx[ind][mu])) / (2 * epsilon)
                    for mu in range(d)
                ]
                for ind in range(Nparticles)
            ]
    ...
    def grad_b_noninteracting(self, x, epsilon=1e-6):
        ...
        dx = [
            np.array(
                [
                    [0 if m != mu else epsilon for m in range(d)]
                    for i in range(Nparticles)
                ]
            )
            for mu in range(d)
        ...
        return np.array(
            [
                (self.b(x + dx[mu]) - self.b(x - dx[mu])) / (2 * epsilon)
                for mu in range(d)
            ]
        )
