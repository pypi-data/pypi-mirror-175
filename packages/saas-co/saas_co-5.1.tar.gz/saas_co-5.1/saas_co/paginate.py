...
class paginate():
    def __init__(self, session, **kargs):
        self.json = []
        pagination_args = {k:v for k,v in args.items() if k not in ['ep', 'service', 'fields', 'filter'] if kargs}
        for page in session.client(kargs.get('service')).get_paginator(kargs.get('ep')).paginate(**pagination_args):
