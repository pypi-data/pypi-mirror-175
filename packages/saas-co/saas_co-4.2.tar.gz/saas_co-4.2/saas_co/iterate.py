...
def get_session(args):
    ...
    if not s:
        try:     ss[aid][reg] = assume_role(access_key_id = arg.get('access_key'), 
                                            secret_access_key = arg.get('secret'), 
                                            accountId = aid, 
                                            roleName = arg.get('role'), 
                                            region=reg)
        except:  pass
    return {'accounts':accounts, 'session':ss}

def update_dataframe_with_func(**kargs):
    accounts = kargs.get('cur',[])
    session = kargs.get('session')
    data = {'accounts':accounts, 'session':session}
    results = [run_func(a, data, **kargs) for a in accounts]
