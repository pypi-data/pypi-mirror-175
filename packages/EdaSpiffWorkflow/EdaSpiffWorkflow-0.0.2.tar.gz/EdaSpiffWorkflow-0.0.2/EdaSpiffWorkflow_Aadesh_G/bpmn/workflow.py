...
from EdaSpiffWorkflow_Aadesh_G.bpmn.specs.ScriptTask import ScriptTask
...
class BpmnWorkflow(Workflow):
    ...
    def catch(self, event_definition):
        ...
        tasks = [t for t in self.get_catching_tasks(
        ) if t.task_spec.catches(t, event_definition)]
        for task in tasks:
    ...
    def do_engine_steps(self, exit_at=None):
        ...
        for task in engine_steps:
            # Change for picking out inline script task.
            if isinstance(task.task_spec, ScriptTask):
                task.task_spec.script = task.task_spec.script + "\nprint('\a')"
        while engine_steps:
            for task in engine_steps:
                task.complete()
