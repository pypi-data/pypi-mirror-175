...
class TaskParser(object):
        ...
        if isMultiInstance or isLoop:
            ...
            if isMultiInstance:
                ...
                loopCardinality = self.process_xpath(
                    './/*[@id="%s"]/bpmn:multiInstanceLoopCharacteristics/bpmn:loopCardinality'
                    % self.get_id()
                )
                ...
                completionCondition = self.process_xpath(
                    './/*[@id="%s"]/bpmn:multiInstanceLoopCharacteristics/bpmn:completionCondition'
                    % self.get_id()
                )
                ...
                isSequential = True
                loopcount = (
                    STANDARDLOOPCOUNT  # here we default to a sane numer of loops
                )
            ...
            # test classes!
        if multiinstance and isinstance(
            self.task,
            (UserTask, BusinessRuleTask, ScriptTask, CallActivity, SubWorkflow),
        ):
            ...
            self.task.completioncondition = (
                completecondition  # we need to define what this is
            )
            ...
            self.task.prevtaskclass = (
                self.task.__module__ + "." + self.task.__class__.__name__
            )
    ...
    def parse_node(self):
        ...
        try:
            ...
            self.task.extensions = self.parser.parse_extensions(
                self.node, xpath=self.xpath
            )
            # process lane's extensions
            for eachLane in self.process_parser.lanes:
                if self.task.lane == eachLane.get("name"):
                    lane_extensions = self.parser.parse_extensions(eachLane)
                    for ext_name, ext_value in lane_extensions.items():
                        self.task.extensions[ext_name] = ext_value

            self.task.documentation = self.parser.parse_documentation(
                self.node, xpath=self.xpath
            )
            ...
            if boundary_event_nodes:
                parent_task = _BoundaryEventParent(
                    self.spec,
                    "%s.BoundaryEventParent" % self.get_id(),
                    self.task,
                    lane=self.task.lane,
                )
                ...
                for boundary_event in boundary_event_nodes:
                    ...
                    if isinstance(
                        b.event_definition, CancelEventDefinition
                    ...
                    parent_task.connect_outgoing(
                        b,
                        "%s.FromBoundaryEventParent" % boundary_event.get("id"),
                        None,
                        None,
                    )
            else:
            ...
            if children:
                ...
                if not default_outgoing:
                    if len(children) == 1 or not isinstance(
                        self.task, ExclusiveGateway
                    ):
