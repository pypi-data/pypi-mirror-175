...
def __remove(__path:str,__where:Optional[FilePredicate],abs_paths:bool):
    files = __dirfiles(__path,__where,abs_paths=abs_paths)
    for file in files:
        __remove_path(file)


def remove(__path,where:Optional[FilePredicate],abs_paths:bool=True):
    return __remove(__path,where,abs_paths=abs_paths)
...
def scan(__path: str, where: Optional[FilePredicate] = None):
    return __scan(__path, where)


def scan_oldest_to_mostrecent(
    __path: str, where: Optional[FilePredicate] = None, abs_paths: bool = True
):
    return __scan_oldest_to_mostrecent(__path, where, abs_paths=abs_paths)


def scan_mostrecent_to_oldest(
    __path: str, where: Optional[FilePredicate] = None, abs_paths: bool = True
):
    return __scan_mostrecent_to_oldest(__path, where, abs_paths=abs_paths)
