...
def run_deployed_model(
    ...
    kubeflow_model_url: str = None,
    header: Dict[Any, Any] = None,
    cdf_external_id: str = None,
    **kwargs,
) -> pd.DataFrame:
    """
    ...
    Args:
        ...
        kubeflow_model_url (str): The URL to the deployed model
    ...
    elif kubeflow_model_url is not None:
        if header is not None:
            predictions = run_kubeflow_model(
                in_data, kubeflow_model_url, header=header, **kwargs
            )
        else:
            predictions = run_kubeflow_model(in_data, kubeflow_model_url, **kwargs)
    else:
        raise ValueError(
            "Neither a Cognite Data Fusion function nor a Kubeflow model URL was provided!"
        )
...
def run_kubeflow_model(
    ...
    **kwargs,
    ...
    """
    Helper function to send a http(s) request to a model hosted on a (kubeflow) cluster.
    Note: this function assumes requests are sent to url on the form
    'http(s)://(...)/model_name/endpoint',
    where 'endpoint' can be either 'metadata' or 'predict'.

        Args:
            in_data: The payload to send to the kubeflow model
            model_url (str): The URL to the kubeflow model
            session (requests.Session): The requests session to use for sending the request
        Returns:
            pd.DataFrame: The dataframe with the results of the function
        Raises:
            ValueError: If an invalid endpoint is requested, i.e. other than 'predict' and 'metadata'
            ValueError: If request exits with a status code other than 200
    """
    model_name = model_url.split("/")[-2]
    endpoint = model_url.split("/")[-1]
    if endpoint == "metadata":
        ...
        raise ValueError(
            f"Invalid request: Model {model_name} does not have a '{endpoint}' endpoint"
        )
    ...
    if method == "post":
        try:
            if header is not None:
                ...
                response = session.post(model_url, json=in_data)
        except requests.exceptions.ConnectionError as e:
            raise ValueError(f"Failed to connect to endpoint '{model_url}'") from e
    else:
        try:
            if header is not None:
    ...
    if response.status_code != 200:
        raise ValueError(
            f"Request to {model_url} failed with status code {response.status_code}! See response for details: {response.text}"
        )
    # Get the predictions/metadata
    if endpoint == "metadata":
