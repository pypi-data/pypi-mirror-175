    ...
    **kwargs,
    ...
    if rolling_features is not None and window is not None and depth_column is not None:
        ...
        if calculate_norm_dist:
            # Both min and max is required to do this so make sure kwargs are set
            if not calculate_min or not calculate_max:
                raise ValueError(
                    "Both calculate_min and calculate_max must be True to calculate normalized distance"
                )
            for col in rolling_features:
                df[col + "window_norm_dist"] = (df[col] - df[col + "_window_min"]) / (
