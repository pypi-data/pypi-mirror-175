...
class ErrorStore:
    ...
    def add(self, object: dict, score: Union[int, float], description=""):
        oh = object["objectHash"]
        if oh not in self.errors or self.errors[oh][1] > score:
            self.errors[oh] = (object, score, description)


class MissingObjectsIndexer(Indexer):
        ...
        for data_unit, img_pth in iterator.iterate(desc="Looking for broken tracks"):
            ...
            for i in range(len(objects) - 1, -1, -1):
                if polygons[i] is None:
                    error_store.add(object=objects[i], score=0.0)
                    objects.pop(i)
                    polygons.pop(i)
            ...
            for obj, poly in zip(objects, polygons):
                ...
                if best_iou < self.threshold / 2:  # Skip objects that are not similar enough.
                    error_store.add(object=obj, score=1.0)
                    continue
