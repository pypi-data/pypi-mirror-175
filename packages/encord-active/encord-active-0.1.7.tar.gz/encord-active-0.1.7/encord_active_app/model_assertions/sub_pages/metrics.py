...
class MetricsPage(ModelAssertionsPage):
    ...
    @staticmethod
    def create_indexer_importance_charts(model_predictions, num_samples):
        ...
        if num_tps < 50:
            st.warning(
                f"Not enough true positives ({num_tps}) to calculate reliable index importance. "
                "Try increasing the number of samples or lower the IoU threshold in the side bar."
            )

        scores = _predictions["iou"] * _predictions["tps"]
        indexers = _predictions[st.session_state.prediction_indexer_names]

        correlations = indexers.fillna(0).corrwith(scores, axis=0).to_frame("correlation")
        correlations["index"] = correlations.index.T

        mi = pd.DataFrame.from_dict(
            {"index": indexers.columns, "importance": mutual_info_regression(indexers.fillna(0), scores)}
        )
        sorted_indexers = mi.sort_values("importance", ascending=False, inplace=False)["index"].to_list()

        mutual_info_bars = (
            alt.Chart(mi, title="Index Importance")
            .mark_bar()
            .encode(
                alt.X("importance", title="Importance", scale=alt.Scale(domain=[0.0, 1.0])),
                alt.Y("index", title="Index", sort=sorted_indexers),
        ...
        correlation_bars = (
            alt.Chart(correlations, title="Index Correlations")
            .mark_bar()
            .encode(
    ...
    def build(
        ...
        with st.container():
            with st.expander("Description"):
                ...
                st.markdown(
                    "- **Index importance**: measures the *strength* of the dependency between and indexer and model "
                ...
                st.markdown(
                    "- **Index [correlation](https://en.wikipedia.org/wiki/Correlation)**: measures the *linearity "
            ...
            with st.spinner("Computing index importance..."):
                chart = self.create_indexer_importance_charts(
                    model_predictions,
