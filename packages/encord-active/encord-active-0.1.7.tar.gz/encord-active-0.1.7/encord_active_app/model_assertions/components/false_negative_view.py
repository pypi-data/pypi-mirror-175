...
def __build_card(
    ...
    with st_col:
        ...
        build_data_tags(label, st.session_state[state.PREDICTIONS_LABEL_INDEX])
...
def false_negative_view(false_negatives, model_predictions, color: Color):
    ...
    selected_index = st.session_state.get(state.PREDICTIONS_LABEL_INDEX)
    subset = get_df_subset(false_negatives, selected_index)
