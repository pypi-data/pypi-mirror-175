...
class Property:
    ...
    def get_key(self, index_name):
        return self.key

    def get_name(self, index_name):
        return self.name
...
properties: List[Property] = [
    IndexProperty("index", "tag", "purple", ""),
...
def build_data_tags(row: pd.Series, index_name: str):
    tag_list = []
    for p in properties:
        key = p.get_key(index_name)
        if not key in row:
            continue
        _value = row[key]
        value = ""
        ...
        if value:
            tag_list.append(TAG_TEMPLATE % (p.icon, p.get_icon_color(value), value, p.get_name(index_name)))
