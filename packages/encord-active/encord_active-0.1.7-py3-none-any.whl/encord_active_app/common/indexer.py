...
def load_index(indexer: IndexerData, normalize: bool, *, sorting_key="score") -> pd.DataFrame:
    """
    ...
    :param indexer: The indexer to load data from.
    ...
    """
    df = pd.read_csv(indexer.path).sort_values([sorting_key, "identifier"], ascending=True).reset_index()

    if normalize:
        min_val = indexer.meta.get("min_value")
        max_val = indexer.meta.get("max_value")
        if min_val is None:
