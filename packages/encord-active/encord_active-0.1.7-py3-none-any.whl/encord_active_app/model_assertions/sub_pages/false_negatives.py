...
from encord_active_app.common.colors import Color
from encord_active_app.model_assertions.components import false_negative_view
...
class FalseNegativesPage(ModelAssertionsPage, HistogramMixin):
    ...
    def sidebar_options(self):
        st.selectbox(
            "Select indexer for your labels",
            st.session_state.label_indexer_names,
            key=state.PREDICTIONS_LABEL_INDEX,
    ...
    def build(
        ...
        index_name = st.session_state[state.PREDICTIONS_LABEL_INDEX]
...
""",
            ...
            self.indexer_details_description(index_name)
        fns_df = labels[labels["fns"]].dropna(subset=[index_name])
        if fns_df.shape[0] == 0:
            st.write("No false negatives")
        else:
            histogram = self.get_histogram(fns_df, index_name)
