...
""",
        ...
        fp_df = model_predictions[model_predictions["tps"] == 0.0].dropna(subset=[index_name])
        if fp_df.shape[0] == 0:
            st.write("No false positives")
        else:
            histogram = self.get_histogram(fp_df, index_name)
            st.altair_chart(histogram, use_container_width=True)
            index_view(fp_df, box_color=color)
