...
import encord_active_app.model_assertions.data as pred_data
from encord_active_app.common.components import sticky_header
from encord_active_app.common.utils import setup_page
from encord_active_app.model_assertions.map_mar import compute_mAP_and_mAR
from encord_active_app.model_assertions.settings import common_settings
from encord_active_app.model_assertions.sub_pages import Page
...
def model_assertions(page: Page):
    def render():
        ...
        st.session_state.labels, st.session_state.label_indexer_names = pred_data.get_labels()
        st.session_state.gt_matched = pred_data.get_gt_matched()
        st.session_state.indexer_meta = pred_data.get_metadata_files()
        ...
        # Sort predictions and labels according to selected indexes.
        pred_sort_column = st.session_state.get(state.PREDICTIONS_INDEX, st.session_state.prediction_indexer_names[0])
        st.session_state.sorted_model_predictions = st.session_state.model_predictions.sort_values(
            [pred_sort_column], axis=0
        )

        label_sort_column = st.session_state.get(state.PREDICTIONS_LABEL_INDEX, st.session_state.label_indexer_names[0])
