...
def plot_result(*args):
    ...
    names_lst = ['Fig. {}'.format(image) for image in range(1, number_images)]
...
def plot_histogram(image):
    ...
    for index, (ax, color) in enumerate(zip(axis, color_lst)):
        ...
        sns.histplot(ax = ax, data = image[:, :, index].ravel(), bins = 256, color = color, alpha = 0.8)
    fig.tight_layout()
    plt.show()
