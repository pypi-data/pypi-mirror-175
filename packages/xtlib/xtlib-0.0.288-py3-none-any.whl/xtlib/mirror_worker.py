            ...
            if not self.wildcard or fnmatch(basename, self.wildcard):
                ...
                if process_event:
                    ...
                    if self.mirror_dest == "grok":
                        ...
                        if self.show_calls:
                            console.print("on_any_event BEFORE: fn_src: {}, src_rel_path: {}, fn_dest: {}".format(fn_src, src_rel_path, fn_dest))

                        src_blob_path = "mirrored/" + self.dest_folder + "/" + src_rel_path

                        if fn_dest:
                            dest_rel_path = fn_dest[plen:]
                            dest_rel_path = dest_rel_path.replace("\\", "/")

                            dest_blob_path = "mirrored/" + self.dest_folder + "/" + dest_rel_path
                        ...
                        if self.show_calls:
                            console.print("on_any_event AFTER: src_blob_path: {}, dest_blob_path: {}".format(src_blob_path, dest_blob_path))
