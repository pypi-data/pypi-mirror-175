...
from distkv.data import node_attr, res_get, res_update
...
async def attr_(obj, bus, group, vars_, eval_, path_):
    ...
    if vars_ or eval_ or path_:
        res = await node_attr(
            obj, obj.cfg.knx.prefix + path, vars_, eval_, path_, res=res
        )
        if obj.meta:
            yprint(res, stream=obj.stdout)
    else:
...
async def server_(obj, bus, name, host, port, delete):
    ...
    if not name:
        ...
        async for r in obj.client.get_tree(
            obj.cfg.knx.prefix, bus, min_depth=1, max_depth=1
        ):
            print(r.path[-1], file=obj.stdout)
        return
    elif len(name) > 1:
