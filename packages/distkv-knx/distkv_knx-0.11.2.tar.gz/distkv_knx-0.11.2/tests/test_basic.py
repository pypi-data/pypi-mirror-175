...
knx_mock = load_ext("distkv_ext.knx.mock", err=True)
Tester = knx_mock.Tester

knx_task = load_ext("distkv_ext.knx.task", err=True)
task = knx_task.task

knx_model = load_ext("distkv_ext.knx.model", err=True)
...
async def test_basic():
    async with stdtest(test_0={"init": 125}, n=1, tocks=200) as st, st.client(
        ...
        await st.run(
            "knx addr -t out -m binary test 2/3/5 -a src test.some.other.switch"
        )
        await st.run(
            "knx addr -t out -m percentU8 test 2/3/6 -a src test.some.other.percent"
        )
