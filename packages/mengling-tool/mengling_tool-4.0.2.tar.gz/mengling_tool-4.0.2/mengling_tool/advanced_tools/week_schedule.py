...
from ..time_tool2 import getNowDatetime, date_str
from ..functions.goodlib import ThreadGoods
from ..database_tool2.redis import RedisExecutor
from ..notice_tool import emailSend, customize_emailSend, Sender
import re
...
class WeekSchedule:
    ...
    def __run_one__(self, name):
        ...
        while True:
            # 休眠判断
            while not any((tp[0] <= date_str(getNowDatetime(), '%H:%M:%S') <= tp[1]) for tp in wser.time_periods):
                wser.status = '休眠中'
                time.sleep(10)
            try:
                wser.status = '执行中'
                ...
                r.hset(f'{self.week_name}_error', name,
                       f'{date_str(getNowDatetime(), gs="%Y/%m/%d %H:%M:%S")}\n{error}')
