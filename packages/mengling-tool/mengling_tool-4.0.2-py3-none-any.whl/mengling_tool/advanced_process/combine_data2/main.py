...
from ...time_tool1 import getNowStamp_f
...
from .insert import inMysql_old, inMysql, inMysql_strict, checknum
import traceback
...
class Combination(Config):
                ...
                with self.__lock__:
                    for dbdt in self.__public_itdts__:
                        ...
                        if dbdt['makeid'] is None:
                            t = int(dbdt['time'] + dbdt['sleeptime'] - getNowStamp_f())
                            # 是否在休眠
                            if t <= 0:
                                # 次数是否耗尽
                                if dbdt['ci'] > 0:
    ...
    def inMysql_new(self, dbname, table, connect, lies=None, columnclassdict: dict = None, key=None, threadnum=10,
                    ifmyisam=True, ifdeltable=True, n=50_0000, ifchecknum=True, ifdayupdate=False,
                    if_auto_data=True):
    ...
    def close(self, iftz=True, close_distribute_func=lambda x: x.close()):
