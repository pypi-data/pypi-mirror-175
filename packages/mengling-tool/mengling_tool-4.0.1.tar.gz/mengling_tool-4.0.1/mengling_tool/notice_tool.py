...
def windowTip(title, content, ifcancel=False):
    # 值为1或2
    return win32api.MessageBox(0, content, title, win32con.MB_OKCANCEL if ifcancel else win32con.MB_OK) == 1


# 弹窗警告
def windowWarning(title, content):
    return win32api.MessageBox(0, content, title, win32con.MB_OK) == 1
...
def setSysPrint(str_writedfunc=None):
    sys.stdout = _Printer(str_writedfunc)
