...
class PolygonCaller:
    '''
    This class repeatedly calls the polygon api every 1 seconds for 24 hours
    ...
    def make_api_calls(self):
        ...
        while count < 86400:  # 86400 seconds = 24 hours
            ...
            for currency in self.currency_pairs:
                ...
                try:
                    resp = self.client.forex_currencies_real_time_currency_conversion(from_, to, amount=100, precision=2)
                except:
                    continue
    ...
    def reset_raw_data_tables(self, engine, currency_pairs):
        with engine.begin() as conn:
            for curr in currency_pairs:
                ...
                conn.execute(text(
                    "CREATE TABLE IF NOT EXISTS " + curr[0] + curr[
                        1] + "_raw(ticktime text, fxrate  numeric, inserttime text);"))
    ...
    def initialize_aggregated_tables(self, engine, currency_pairs):
        with engine.begin() as conn:
            for curr in currency_pairs:
                conn.execute(text("CREATE TABLE IF NOT EXISTS " + curr[0] + curr[
