...
class GetApi:
    def __init__(self, api_type=1, value="", name="", yapi_url="", app_key=""):
        """api_type: 1时，value值为yapi项目token,
                     2时，value是yapi下载的json文件名，文件放在file目录下,
                     0时，value是swagger的api，json的url
    ...
    def get_api_fun_list(self):
        """获取已维护方法列表，无则创建文件标题"""
        self.name2 = self.name[0].upper() + self.name[1:]
        ...
        if os.path.exists(self.fun_file_path):
            ...
            self.fun_file_str = "from commen.unit_fun import FunBase\n\n\nclass %sFun(FunBase):\n" \
                                "    def __init__(self):\n        super().__init__()\n        self.res = None\n\n" \
                                "    def run_mysql(self, sql_str):\n" \
                                "        # id是后台http://47.96.124.102/admin 项目数据库链接的id\n" \
                                '        out = requests.post("http://47.96.124.102/sql/running_sql/", ' \
        ...
        if os.path.exists(self.api_testcase_file_path):
            ...
            self.api_testcase_file_str += '@allure.epic("针对单api的测试")\n@allure.feature("场景：")\nclass TestApi:\n' \
                                          '    def setup(self):\n        self.f = %sApiFun()\n\n' % self.name2
        self.api_testcase_list = re.findall("    def +tese_(.*)?\(", self.api_testcase_file_str)

    def set_file_path(self):
        """创建项目目录"""
        commen_dir = os.path.dirname(os.path.realpath(__file__))
        self.dir = os.path.dirname(commen_dir)
        if not self.name:
            self.name = os.path.basename(sys.argv[0])
        if not os.path.exists(os.path.join(self.dir, "service")):
            os.makedirs(os.path.join(self.dir, "service"))
        if not os.path.exists(os.path.join(self.dir, "service", self.name)):
            os.makedirs(os.path.join(self.dir, "service", self.name))
        self.service_dir = os.path.join(self.dir, "service", self.name)
        ...
        if not os.path.exists(os.path.join(self.dir, "testcase")):
            os.makedirs(os.path.join(self.dir, "testcase"))
        if not os.path.exists(os.path.join(self.dir, "testcase", self.name)):
            os.makedirs(os.path.join(self.dir, "testcase", self.name))
        self.testcase_dir = os.path.join(self.dir, "testcase", self.name)
    ...
    def get_init_value(self):
        ...
        value = 'from commen.init_project import GetApi\n' \
                ...
                'def http_requester(method, url, params=None, data=None, headers=None, **kwargs):\n' \
                '    if headers.get("headers"):\n        _headers = init_headers()\n' \
                '        tmp = headers.pop("headers")\n        if isinstance(tmp, dict):\n' \
                '            _headers.update(headers)\n            _headers.update(tmp)\n    else:\n' \
                '        headers.pop("headers")\n\n\n' \
                '    res = unit_http_requester(method, url, params=params, data=data, ' \
                'headers=_headers, host=HOST, timeout=TIMEOUT)\n    return res\n\n\n' \
                'if __name__ == "__main__":\n' \
                "    GetApi(api_type=%s, value='%s', name='%s', yapi_url='%s').run()\n" % (
                    self.app_key, self.name, self.api_type, self.value, self.name, self.yapi_url)
        return value
    ...
    def list_add(self, _list):
        ...
        _list2 = []
        for l in _list:
            if l.get("name") not in _list1:
                _list1.append(l.get("name"))
                _list2.append(l)
        return _list2
    ...
    def get_sql_fun_str(self, api_fun_name):
        ...
        sql_fun_str += "        # name = kwargs.pop('name')  # 单独处理字段\n" \
                       "        # self.kwargs_replace(kwargs, likes=[], ins=[])  # 模糊查询字段，数组包含查询字段\n" \
    ...
    def get_api_fun_fun_str(self, api_fun_name):
        ...
        data_list_tmp = []
        size_names = ("pageSize", "size")
        page_names = ("pageNum", "current")
        page_and_size = size_names + page_names
        for n in data_list:
            if n in size_names:
                data_list_tmp.append(n.strip() + '=10')
            elif n in page_names:
        ...
        data_list = [n for n in data_list if n not in page_and_size and n != 'headers']
    ...
    def get_assert_fun_str(self, fun_name):
        ...
        assert_fun_str += '        # flag = self.compare_json_list(self.res.rsp.data.records, out, [%s])\n' % \
                          ', '.join(['"%s"' % i for i in data_list])
...
if __name__ == '__main__':
    GetApi(app_key="d3ed61d3-9b31-409d-a8a6-7e172566ee76")
