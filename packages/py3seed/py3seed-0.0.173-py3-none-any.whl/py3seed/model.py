    ...
    def schema(cls):
        ...
        def _gen_schema(type_: Type, parents=[]):
            ...
            if isinstance(type_, SimpleEnumMeta):
                ...
                obj['read_formats'] = list(dict.fromkeys(properties[f]['format'] for f in obj['read_fields'] if 'format' in properties[f]))
                obj['form_fields'] = list(iterate_layout(obj['form'], obj['groups']))
                obj['form_formats'] = list(dict.fromkeys(properties[f]['format'] for f in obj['form_fields'] if 'format' in properties[f]))
                # searchables, [field__comparator]
                obj['searchables'] = [('{}__{}'.format(*s) if s[1] != Comparator.EQ else s[0]) for s in searchables]
                obj['search_formats'] = list(dict.fromkeys(properties[s[0]]['format'] for s in searchables if 'format' in properties[s[0]]))
