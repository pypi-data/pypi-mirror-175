...
from sberpm._utils import generate_data_partitions
            ...
            if time_column is not None and not is_datetime64_any_dtype(time_column):
                if time_format is None:
                    ...
                    df[time_column] = to_datetime(
                        df[time_column], dayfirst=dayfirst, yearfirst=yearfirst, utc=True
                    )
                    ...
                    df[time_column] = to_datetime(df[time_column], format=time_format, errors="coerce", utc=True)
                elif time_errors == "auto_convert":
                    dayfirst, yearfirst = DataHolder._check_dayfirst_yearfirst(dayfirst, yearfirst)
                    result = to_datetime(df[time_column], format=time_format, errors="coerce", utc=True)
                    na_mask = result.isna()
                    non_converted_timestamps = df[time_column][na_mask]
                    res = to_datetime(non_converted_timestamps, dayfirst=dayfirst, yearfirst=yearfirst, utc=True)
