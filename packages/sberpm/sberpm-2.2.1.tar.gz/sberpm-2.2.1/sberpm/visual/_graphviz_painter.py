...
def _get_gv_node(node, node_color_dict):
    ...
    if node.type == NodeType.TASK:
        ...
        gv_node = GvNode(
            node_id, node_label_with_metrics, shape="box", style="filled", fillcolor=node_color_by_metric
        )
...
class GraphvizPainter:
    ...
    def _add_node_to_digraph(self, gv_node):
        ...
        """
        self._digraph.node(
            gv_node.id, shape=gv_node.shape, fillcolor=gv_node.fillcolor, style=gv_node.style, label=gv_node.label
        )
