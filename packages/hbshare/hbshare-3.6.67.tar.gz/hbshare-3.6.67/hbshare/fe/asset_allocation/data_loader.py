...
class Loader:
    ...
    def read_private_index_daily_k_given_indexs(self, indexs, start, end):
        indexs = "'" + "','".join(indexs) + "'"
        sql = "SELECT zsdm AS INDEX_CODE, tjyf AS TRADE_MONTH, spjg AS CLOSE_INDEX FROM st_hedge.t_st_sm_hmzs WHERE zsdm in ({0}) AND tjyf >= {1} AND tjyf <= {2} AND m_opt_type <> '03'".format(indexs, start, end)
