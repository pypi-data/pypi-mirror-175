                    ...
                    for date in reblance_list:
                        ...
                        if min(ret_density) < 0.3 or min(cov_density) < 0.3:
                            ...
                            if min(ret_density) < 0.3 and min(cov_density) >= 0.3:
                                print('{0}: {1} have no enough nav data for computing return model!'.format(date, ','.join([self.asset_list[i] for i in ret_density_ser.index.tolist()])))
                            if min(ret_density) >= 0.3 and min(cov_density) < 0.3:
                                print('{0}: {1} have no enough nav data for computing risk model!'.format(date, ','.join([self.asset_list[i] for i in cov_density_ser.index.tolist()])))
                            if min(ret_density) < 0.3 and min(cov_density) < 0.3:
                                print('{0}: {1} have no enough nav data for computing return model and risk model!'.format(date, ','.join([self.asset_list[i] for i in list(set(ret_density_ser.index.tolist() + cov_density_ser.index.tolist()))])))
                            interval_data = dict()
