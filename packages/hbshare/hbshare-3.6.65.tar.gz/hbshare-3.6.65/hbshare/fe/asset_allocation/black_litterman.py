...
class BlackLitterman:
    ...
    def _init_data(self):
        ...
        if not (self.nav_df.empty or self.nav_df.reindex(date_list).interpolate().dropna(how='all').empty or self.nav_df.dropna(axis=1, how='all').shape[1] != len(self.asset_list)):
            nav_df = self.nav_df.reindex(date_list).interpolate().dropna().sort_index(how='all')
            ...
            for date in reblance_list:
                ...
                if min(density) < 0.3:
                    ...
                    print('{0}: {1} have no enough nav data for computing risk model!'.format(date, ','.join([self.asset_list[i] for i in density_ser.index.tolist()])))
                    interval_data = dict()
