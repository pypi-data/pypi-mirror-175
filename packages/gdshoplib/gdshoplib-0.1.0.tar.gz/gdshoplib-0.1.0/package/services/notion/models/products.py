...
from services.notion.models.user import User
...
properties_keys_map = {
    ...
    #
    'COmf': dict(name='Материалы / Состав', key='description_materials'),
    'HI%5DA': dict(name='Фото на лошади', key='photo_horse'),
    ...
    'sXND': dict(name='Примечания', key='description_notes'),
...
properties_type_parse_map = {
    "rich_text": lambda data: " ".join([t.get('plain_text', "") for t in data['rich_text']]) or "",
    "number": lambda data: data['number'],
    "select": lambda data: data.get('select').get('name') if data.get('select') else None,
...
class Product(BaseModel):
    ...
    settings: Optional[ProductSettings]

    def generate_dict(self):
        _d = self.dict()
        return {**_d, **{prop.key: prop.value for prop in self.properties}}
