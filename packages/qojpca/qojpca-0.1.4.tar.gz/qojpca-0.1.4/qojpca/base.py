...
def qojpca(X_,Y_,l_x, l_y,P_=np.array([]),Q_=np.array([]), l_ = 1.0 , nb_iterations=10, relative_convergence_criteria= 0.05):
    ...
    if(X_.shape[0]==Y_.shape[0]):
        ...
        for i in range(1,nb_iterations): 
            ...
            PQT = P.dot(Q.transpose())
            newOrthoScore = l*np.linalg.norm(PQT)
            ...
            logInfo("X score ="+ str(k*Xscore*samplingFactor)+" | Y score ="+ str(Yscore*samplingFactor)+" | orthogonallity score ="+ str(newOrthoScore)+" | orthogonallity score ="+ str(newOrthoScore/l))
            
            if(abs(orthoScore - newOrthoScore)/ newOrthoScore < relative_convergence_criteria ):
                break
            orthoScore = newOrthoScore
