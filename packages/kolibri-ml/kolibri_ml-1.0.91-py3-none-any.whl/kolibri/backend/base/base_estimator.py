...
class BaseEstimator(Component):
        ...
        if "fixed" in self.hyperparameters:
            for c in self.hyperparameters["fixed"]:
                ...
                if c in self.hyperparameters["tunable"]["model"]["parameters"]:
                    self.hyperparameters["tunable"]["model"]["parameters"][c]["value"] = self.hyperparameters["fixed"][
                        c]
    ...
    def _setup_model(self, model):
        ...
        elif model is not None and not hasattr(model, "fit"):
            raise ValueError(
                f"Estimator {str(model)} does not have the required fit() method."
            )

        self.hyperparameters["tunable"]["model"] = self._get_model(model)
