...
pub enum IndexError {
    #[error(
        "Only ndim of 0 or 1 is supported for index tensors and got {ndim} (tensor: {tensor:?})"
    )]
    TensorNDimTooLarge { ndim: usize, tensor: Tensor },
...
impl TryFrom<Tensor> for IndexTensor {
    ...
    fn try_from(tensor: Tensor) -> Result<Self> {
        if tensor.ndim() > 1 {
            bail!(IndexError::TensorNDimTooLarge {
                ndim: tensor.ndim(),
                tensor,
            });
        }
    ...
    pub fn repr_bijection(&self) -> String {
        ...
        for (i, idx) in self.0.iter().enumerate() {
            result.push_str(&match idx {
                ...
                TensorAxisIndex::Tensor(tensor) => {
                    save_tensor((**tensor).clone(), false).unwrap();
