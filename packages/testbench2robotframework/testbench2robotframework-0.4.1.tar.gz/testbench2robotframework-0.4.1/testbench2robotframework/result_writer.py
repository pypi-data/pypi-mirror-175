...
class ResultWriter(ResultVisitor):
    ...
    def _create_unique_path(attachement_path: Path) -> Path:
        ...
        while attachement_path.exists():
            attachement_path = Path(f"{attachement_path.stem}_{counter}{attachement_path.suffix}")
            counter += 1
    ...
    def _set_itb_testcase_execution_comment(self, itb_test_case, test_chain: List[TestCase]):
        ...
        for test in test_chain:
            ...
            test_phase_name = (
                f"<b>Phase {test_chain_obj.index}/{test_chain_obj.length} : <span {self.render_status(test.status)}>{test.status}</span></b>"
                if test_chain_obj
    ...
    def _create_itb_exec_comment(
        ...
        msg = message.message.replace("<hr>", "<br/>").replace("<br>", "<br/>").strip()
        return (
