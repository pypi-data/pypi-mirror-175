...
class BaseCrud:
    ...
    def _get_one(self, model_kls, kwargs: dict):
        ...
        return self._db_session().query(model_kls).filter_by(**kwargs).first()
        ...
        if not get_objects:
            ...
            for kwarg in kwargs_list:
                ...
                for obj in get_objects:
                    if all([kwarg[key_] == getattr(obj, key_) for key_ in
                            keys]):
    ...
    def update(self, filter_fields, **kwargs):
        ...
        if self.__post_create__ and isinstance(self.__post_create__, dict):
            object_keys: list = list(self.__post_create__.keys())
            post_create_data: dict = dict()
            for key_ in object_keys:
                if key_ not in validated_data:
                    continue
                post_create_data[key_] = validated_data[key_] or []
                del validated_data[key_]

                # We expect a dictionary with lists... Eg
                # claims: List[ClaimsObjects]
                model_ = self.__post_create__[key_]["model_kls"]
                link_field = self.__post_create__[key_]["linked_field"]

                # These tables/relationships need to be purged on each update
                if key_ in ["claims"]:
                    filter_delete = {link_field: obj.id}
                    session.query(model_).filter_by(**filter_delete).delete()
                    session.commit()

                for data in post_create_data[key_]:
                    data[link_field] = obj.id
                    if "id" in data:
                        del data["id"]
                    # We create the object
                    link_obj = model_(**data)
    ...
    def create(self, **kwargs):
        ...
        # Post-Create-Objects
        for field_, objects_ in post_create_data.items():
            # We expect a dictionary with lists... Eg
            # claims: List[ClaimsObjects]
            for data in objects_:
                model_ = self.__post_create__[field_]["model_kls"]
                link_field = self.__post_create__[field_]["linked_field"]
    ...
    def get_obj(self, **kwargs):
        """
        columns: A list of columns to pull from database and not getting the whole row.
    ...
    def get_objects(self, **kwargs):
        ...
        columns: A list of columns to pull from database and not getting the whole row.
                It's preferred to always include the id with the columns.
                E.g: ["id", "columns1", "columns2"]
        count: A boolean that will count the matching rows in db. count overrules any other
                parameters.
        """
