...
class DISA:
    ...
    def standardisation_of_lift(self, i):
        ...
        return (self.lift(i)-omega)/(v-omega) if (v-omega) != 0 else 0
    ...
    def collective_strength(self, i):
        ...
        """
        return (self.patterns[i]["XY"] + self.patterns[i]["_X_Y"] / self.patterns[i]["_X"]) / (self.patterns[i]["X"] * self.patterns[i]["Y"] + self.patterns[i]["_X"] * self.patterns[i]["_Y"]) if (self.patterns[i]["X"] * self.patterns[i]["Y"] + self.patterns[i]["_X"] * self.patterns[i]["_Y"]) != 0 else 0
    ...
    def cosine(self, i):
        ...
        metric : float
            Cosine of subspace
        """
        return self.patterns[i]["XY"] / math.sqrt(self.patterns[i]["X"] * self.patterns[i]["Y"]) if self.patterns[i]["X"] * self.patterns[i]["Y"] != 0 else 0
