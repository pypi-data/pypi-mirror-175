...
This file implements states related to GCP compute.buckets.
GCP credentials must be presented via the `acct Sub`_.
...
async def present(
    ...
    if old:
        changes = hub.tool.gcp.utils.has_changes(old, plan_state)
        ...
        # Perform update
        update_ret = await hub.exec.gcp.storage.buckets.update(
            hub,
            ctx,
            name=name,
            resource_id=resource_id,
        ...
        # Create
        create_ret = await hub.exec.gcp.storage.buckets.insert(
            ctx,
            name=name,
            project=project,
            body=resource_body,
    ...
    get_ret = await hub.exec.gcp.storage.buckets.get(
        ctx, name=name, resource_id=resource_id, userProject=user_project
    )
...
async def absent(
    ...
    :param bucket: The name of the bucket that should not exist.
    :param kwargs: Keyword args passed directly to the GCP SDK API.
    ...
    .. code-block: yaml
        ...
        Ensure Bucket is Absent:
            gcp.storage.buckets.absent:
                - project: {{ project }}
                - bucket: {{ bucket_name }}
                - userProject: {{ billto_project_name }}
    """
