...
def readNewProtocolFileToPandas(filenameIn="~/blabla.ttr", reorder_channels=False, CHANNELS=49, force_16bit_step=True, extract_failsafe=False, file_offset=0, file_last=-1):
    ...
    reorder_channels_type=-1
    if reorder_channels==True:
        if CHANNELS==25:
            reorder_channels_type=25
        elif CHANNELS==37:
            reorder_channels_type=37
    else:
        if type(reorder_channels) is int:
            reorder_channels_type = reorder_channels

    if reorder_channels_type==25:
            order_channels={}
            ...
            print("Applied reorder channel on the first 25 channels as 5x5 detector connected to the TTM")
    elif reorder_channels_type==37:
            order_channels={}
            ordered_list_channel={}
            ...
            for i in range(0,37):
                ...
                order_channels["t_%d"%i]="t_%d"%ordered_list_channel[i]
            df.rename(columns=order_channels,  inplace=True)
            print("Applied reorder channel for the 7x7 on the first 37 channels: 3 channel for each detector corner are ignored")
    else:
            print("Reorder not apply! If you want to apply a reordering\n please use: reorder_channels=25 or reorder_channels=37")
