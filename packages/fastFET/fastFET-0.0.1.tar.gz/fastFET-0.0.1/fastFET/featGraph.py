...
class GraphBase( object ):
    ...
    @staticmethod
    def latestPrimingTopo( raw_fields, path_rib, paths_priming, space=6):    # space8
        '''- return: 事件起始时刻全局路由拓扑 pd_df_rib ['peer_AS', 'dest_pref', 'path_raw' ]。
        - 可通过RIPE-NCC等里的rib表拿到固定时刻的完整拓扑，但离事件起始时刻还有一定间隔，因此还需利用updates消息逐条更新到事件起始时刻
        - 耗时问题：
            - 传统方案：也要实现。逐条特别耗时。
            - polars方案：获取rib-dump时刻到事件起始时刻的updates得到DF，直接对peer-pfx分组，得到每组path列的tail(1)，直接更新到rib表的拓扑
        '''
        # 初始化ldf_rib时，需被 ldf_priming按peer-pfx分组更新一次。此后ldf_rib要在每个slot被 ldf_slot更新，并即时转为拓扑图存下来。
        
        df_rib= utils.csv2df( raw_fields, path_rib )
        
        
        if len(paths_priming):
            df_priming= utils.csv2df( raw_fields, paths_priming, not_priming= False)
        ...
        if len(paths_priming):
            ldf_priming= ( df_priming.lazy()
                .with_column( pl.col('path').str.replace(' \{.*\}', ''))
                .filter(
                    (pl.col("msg_type") == 'A') #&
                    #(~pl.col('path').str.contains('\{'))   # 该方法可能误伤正常的行，改为str.replace
                    )
                .groupby(['peer_AS', 'dest_pref'])
                .tail(1)        # 注：df_priming 是从updates表来的，只需对每个唯一peer-pfx对取最新条目。
                .select( [
                    'peer_AS',
                    'dest_pref',
                    'path'
            ...
            # 开始对rib 和 前部分 updates 合并更新得最新df
        ...
        return ldf_rib.rename( {'path': 'path_raw'}).collect().to_pandas()

    @staticmethod
    @utils.timer
    def perSlotTopo(pd_shared_topo, pd_shared_preDF, j: int, space=12):   # space14()
        ...
        - j: time_bin块号
        - return: 当前slot的rib表中提取的所有边的list(ndarray)
        ...
        #logger.info(f'test: {utils.curMem()}')  
        all_path= ( pd.concat([pd_shared_topo.value, pd_cur_upds], ignore_index=True)
            .groupby([ 'peer_AS', 'dest_pref' ])
            .tail(1)
            ...
            # 后把展开列按path分组算shift
        #logger.info(f'test: {utils.curMem()}')  # debug: 两次log发现，all_path带来巨大内存消耗，但该变量使用完毕后，为啥内存没恢复？？？
    ...
    @staticmethod
    @utils.timer
    def getKcoreNodes(G, space=12):    # space14()
    ...
    def parallFeatFunc( feats_nx, feats_nk, space=12 ):    # space14()
        ...
        #   tips，13个特征中，除了 nd_local_efficiency ，其他都是秒得。因此用for挨个计算特征对效率影响不大；
        ...
        pq= ProcessingQueue( nbProcess= utils.paralNum(2) )
        ...
        if len(feats_nx):
            ...
            if len(feats_nx_cmpx):
                pq= ProcessingQueue( nbProcess= min(len(feats_nx_cmpx), utils.paralNum(2)) )
    ...
    def perSlotComput(pd_shared_topo,pd_shared_preDF,shared_res, feats_dict, raw_dir, j, lock= None, space=10):   #space12()
        ...
        - args: j: 当前需截取 time_bin ≤ j 的pd_all_upds部分
        '''
        ...
        #logger.info(f"对比df_topo内存地址：{id(pd_shared_topo.value)=}; {id(pd_shared_preDF.value)=}")
        # 1, 画最新的全局拓扑
        G = GraphBase.perSlotTopo(pd_shared_topo,pd_shared_preDF, j, space+2)
        
        # 2, 计算当前图中的特征
        # 2.1, TODO: G的简化：图太大，是否需要用k-core缩减图的精度        
        G, nodes= GraphBase.getKcoreNodes(G, space+2)        
