...
def path_AStotal_count( ldf_path_AStotal, obj ):
    ...
    return ldf
@utils.timer
def path_AStotal_rare_( ldf_path_AStotal:pl.LazyFrame, obj, space= 8 ):   # 原来的
    ...
    return ldf
@utils.timer
def path_AStotal_rare___( ldf_path_AStotal:pl.LazyFrame, obj, space= 8 ):   # 测试用
    '''重新设计‘稀有AS’的算法：
...
def peerPfx_dynamic(ldf_peerPfx: pl.LazyFrame, obj):
    ...
    candidate= {
        ...
        ("is_new",):  
            (pl.col('tag_hist_cur').all().alias('has_new'), ),  # 在当前chunk下，但凡有一个0（属于历史条目），这个peer-pfx都不是新的
...
def peerPfx_relateHijack( ldf_peerPfx: pl.LazyFrame, obj ):
    ...
    sel_apend= [pl.col('origin_AS').alias('path_loc0'), # 列path_loc0即此前的`is_MOAS`
...
def peerPfx_editdist_num( df_peerPfx_editdist: pl.DataFrame, obj ):
    ...
    res.columns=['time_bin']+['ED_'+str(i) for i in range(11)]
    res= res.lazy().with_column(pl.col('time_bin').cast(pl.Int16)).groupby('time_bin')
    return res
