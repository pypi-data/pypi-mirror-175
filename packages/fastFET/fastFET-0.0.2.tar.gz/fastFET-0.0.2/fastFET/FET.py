...
version: 1.0
Author: JamesRay
Date: 2022-09-17 03:55:03
LastEditors: Please set LastEditors
LastEditTime: 2022-10-05 12:39:12
'''
...
class FET():
    ...
    __slot__= [ 'slot', 'raw_dir', 'increment', 'duration',  'ASslot', 'need_ratio', 'need_rib'
    ...
    def __init__(self, 
        ...
        ''' main class  
            ...
            args 
            - slot: 统计特征数量的时间间隔
            - require_field: 需要提取的特征大类
            - increment: 定义事件起止时间的增量(h)
            - duration: 当事件缺省结束时间时，将其指定为 start_time+ duration (h)
            - ##### event_path: 存储目标事件列表的文件（详见readme.md）
            - ASslot: 用于稀有AS的计算，即每隔ASslot个AS来更新一次rare_list。# TODO: 测试速度最快的值。
            - need_ratio:  获取ratio类特征两个条件：设为True，且自定义集合中要含有指定特征(见`self.postHandler()`)。
        '''
    ...
    def addEvent(self, evt:list):
        '''add any event you want.
        - format: `event_name, start_time, end_time(可空), collector(可多个)`
        - example: `["facebook_outage, 2021/10/4 15:40:00, 2021/10/4 21:40:00, rrc00, rrc06", "GoogleLeak, 2017/08/25 01:00:00, 2017/08/25 06:00:00, rrc06"]`
        '''
    ...
    def  chunkForTradi(self, space):   #space8
        ...
        for path, subFeats in self.feats_dict.items():
            if 'graph' not in path and 'ratio' not in path:     # 排除掉非传统特征
                ...
                cur_res= cur_expr_chain.agg( exprs ).collect()
                logger.info(f' '*(space+2)+ f'func= `{path}`; cost={(time.time()-t2):.3f} sec')
        ...
        # 4、临时备份传统特征
            ...
            with open(f'{p}tradi_feats.txt', 'a') as f:
                ...
                f.write('<< '*20+ f'{self.first_ts=}, a df in a collector in a event')
                f.write(cont)
                f.write('\n\n')
    ...
    def chunkForGraph(self, space ):   #space8()
        ...
        cores= max(utils.paralNum(2), multiprocessing.cpu_count()-2)
        cores= 4    # TODO:
        ...
        #logger.info(f"对比df_topo内存地址：{id(self.pd_shared_topo.value)=}; {id(self.pd_shared_preDF.value)=}")
        # logger.info( utils.logMemUsage(8, inspect.stack()[0][3] ))     # 调试锚点：随删
        ...
        for j in nbJobs[:1]:   # 一个任务即一个slot的数据 #TODO: range( nbJobs )
    ...
    def chunkHandler(self, chunk ,modify_df_topo, space ): # space6(timer用4，函数体内用6)
        ...
        try:   # 当有图特征需求时
            _ = self.pd_shared_topo.value.shape     # 如果没有这个pandas.df，则为null，跳转except
            # 2.1、先准备参数：从self.preDF获得共享大表self.pd_shared_preDF
            #       - 前者留作主进程的传统特征；
            #       - 后者只留更新消息，只留4列。最后pandas化，才能把DF用于子进程。DEBUG: 这里存疑：为啥pl.DF无法在子进程中使用？？
    ...
    def initHandler(self, paths_upd, real_sat_time, path_rib, space=4):      # space6(首尾空4，内部空6)
        ...
        if real_sat_time:   # 即如需图特征
            ...
            paths= paths_upd[idx_watershed: ]
                ...
                # 大表4：3个字段[peer_AS, dest_pref, path]。此后该大表在每slot都会根据分了组的updates来更新
            self.pd_shared_topo.value= GraphBase.latestPrimingTopo( self.raw_fields, path_rib, paths_upd[ :idx_watershed ] ,6)
            #self.pd_shared_topo.value.to_csv('pd_rib.txt', index=False)
        else:
            paths= paths_upd
        ...
        cols_pp_table = [('index',pl.UInt32), ('timestamp', pl.Int32), ('time_bin',pl.Int16), ('msg_type',pl.Int8), ('peer_AS',pl.Int32), 
                        ('dest_pref',pl.Utf8), ('path_raw',pl.Utf8), ('hash_attr',pl.UInt64), ('path_len',pl.Int8), ('path_unq_len',pl.Int8),
    ...
    def postHandler(self, save_path:str, space ):    #space6
        '''- monitorHandler 子函数：所有chunk都收集完了之后的最后一步'''
        #logger.info(' '*6+ 'post handle in cur monitor: ')
        # 1、把MOAS数据导出（这里不用了，而是在featTradition模块的peerPfx_relateHijack方法中顺便实现
        ...
        # 2、修缮特征矩阵。可能出现一个slot有两行的情况，需按列加和为一行，
        df= pl.read_csv( save_path )
        df_= df.groupby('date').sum().sort('time_bin').drop_nulls()

        df_.to_csv( save_path )
        
        # 3、转储当前事件当前monitor下的图特征、传统特征的备份
    ...
    def monitorHandler(self, paths_upd: list, real_sat_time, path_rib, evtNm= '_', monitor= '_' ):    # sapce4(代表函数体内)
        '''eventHandler子函数
        - 当没有从collectData模块获取结构化files，而只想对自己指定的files提取特征时，也可使用该函数
        - paths_upd：updates文件名列表(List)。在有图特征情况下，包含了用于更新初始拓扑的那部分文件。
        - real_sat_time：用于切分paths_upd
        - path_rib： rib文件名(Str/ None)'''
    ...
    def run(self):
        '''- main func，对特征列表进行完整的特征提取。
        - return: 特征存放的目录。
        - 需要先规定好目标特征列表，再执行该主函数'''
        ### 太初之时，设置多进程运行方式
        ...
        grd= GetRawData(event_path, self.increment, self.duration,self.raw_dir , self.need_rib)    # self.raw_dir  '/home/cnic/data/eventDataset/'
        fileDict= grd.run()
...
def preProcess(fields, chunk, slot, first_ts, tag_plen= True, tag_punq= True, tag_oriAS= True, space= None):   # space8
    ...
    add_col_list= []
    if tag_plen:
        sel_list.append( pl.col('path').str.split(" ").arr.lengths().cast(pl.Int8).alias('path_len') )
    if tag_punq:
        sel_list.append( pl.col('path').str.split(" ").arr.unique().alias('path_unq') )
        add_col_list.append( pl.col('path_unq').arr.lengths().cast(pl.Int8).alias('path_unq_len') )
    ...
    df= (df.lazy()
        ...
        .filter((pl.col("msg_type") != 'STATE') &
                ((pl.col('path').is_not_null()) | (pl.col("msg_type") == 'W') ))     # 宣告条目中，path为空的过滤掉
        .select( sel_list )
    ...
    if space != None:
       logger.info(' '*(space+2)+ 'after preprocess: df_mem: %sMb; shape: %s' % (utils.computMem(df), str(df.shape)))
    return df  # 最终决定，预处理的df以非lazy形式给出。一方面分发给传统特征的诸多并行链条上，另一方面分发给图特征函数用。


if __name__=='__main__':
    mobj= FET(raw_dir='/data/bgpdata/')
    mobj.run()
