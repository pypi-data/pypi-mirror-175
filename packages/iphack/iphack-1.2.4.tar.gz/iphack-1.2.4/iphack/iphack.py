...
╚═╝╚═╝░░░░░╚═╝░░╚═╝╚═╝░░╚═╝░╚════╝░╚═╝░░╚═╝"""+red)
        ...
        pr = requests.get("https://raw.githubusercontent.com/mishakorzik/mishakorzik.menu.io/master/%D0%A1%D0%B5%D1%80%D0%B2%D0%B5%D1%80/https.json").json()
        pr_ip = pr["ip"]
        pr_port = pr["port"]
        proxis = pr_ip+":"+pr_port
    ...
    def proxy(data=False):
        ...
        if data:
           ...
           for item in proxy_list:
              ...
              print(" ")
           proxy_list = pyproxify.fetch(count=2, google=False, https=True)
           for item in proxy_list:9
              ip = item["ip"]
              port = item["port"]
              last = item["last_update"]
              anonymity = item["anonymity"]
    ...
    def get(*args, **kwargs):
        if anon == "tor":
                 ...
                 try:
                     ...
                     with eventlet.Timeout(15) as timeoo:
                         with requests.Session() as ss:
                             ...
                             get = ss.get(args, **kwargs, headers={"User-Agent": ua.windows()})
                             if log:
                                 print("[100%] Success")
                             return get
                 except:
            ...
            try:
                ...
                if log:
                    print("[90%] Requests get URL")
                get = requests.get(paste, headers={"User-Agent": ua.linux()})
                if log:
                    print("[100%] Success")
                return get
            except:
