...
@pytest.mark.asyncio
async def test_aggregate_method():
    aggregate = (
        Product.Q.aggregate()
...
@pytest.mark.asyncio
async def test_logical_match_query_in_aggregate():
    aggregate = Product.Q.aggregate().match(Query(title=1) | Query(title=5))
    assert aggregate.pipeline == [{'$match': {'$or': [{'title': '1'}, {'title': '5'}]}}]
