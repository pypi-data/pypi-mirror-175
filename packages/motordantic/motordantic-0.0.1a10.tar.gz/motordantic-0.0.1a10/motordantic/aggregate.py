...
class Aggregate(object):
    ...
    def match(
        ...
        if logical_query is not None:
            ...
            query_params: dict = generate_basic_query(
                self.mongo_model_class, query, False
            )
    ...
    def unwind(self, unwind: str) -> 'Aggregate':
        if not unwind.startswith('$'):
