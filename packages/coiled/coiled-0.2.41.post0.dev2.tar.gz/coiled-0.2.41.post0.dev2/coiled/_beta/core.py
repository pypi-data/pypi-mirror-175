...
class CloudBeta(OldCloud, Generic[IsAsynchronous]):
    ...
    async def _create_senv_package(
        self, package_file: BinaryIO, md5: str, account: Optional[str] = None
    ) -> int:
        ...
        response = await self._do_request(
            ...
            json={"name": Path(package_file.name).name, "md5": md5},
        )
    ...
    async def _create_software_environment_v2(
        ...
        for pkg in file_packages:
            file_id = await self._create_senv_package(pkg["sdist"], md5=pkg["md5"])
            env.append(
                {
                    "name": pkg["name"],
                    "source": pkg["source"],
