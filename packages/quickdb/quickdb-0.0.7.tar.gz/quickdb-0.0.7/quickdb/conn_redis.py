...
class RedisClusterConn:
    def __init__(
        ...
        self.conn = rediscluster.RedisCluster(connection_pool=pool, **cluster_kwargs)
...
# redis 连接
class RedisConn:
    def __init__(self, host: str = '127.0.0.1', port: int = 6379, pwd: str = None, db: int = 0, **kwargs):
        self.host = host
        self.port = port
        self.pwd = pwd
        self.db = db
        ...
        self.conn = redis.Redis(connection_pool=pool, decode_responses=True, **kwargs)
