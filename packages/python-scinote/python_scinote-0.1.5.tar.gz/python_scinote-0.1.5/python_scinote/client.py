...
class SciNoteClient(object):
    ...
    def get_inventory_items(self, inventory_id):
        urls = self.build_url_list(
            self.server_url
            + f"/api/v1/inventories/{inventory_id}/items?include=inventory_cells"
        )
        print(urls)
        ...
        columns_dict = self.get_inventory_columns(inventory_id)
        stock_dict = {}
        ...
        def get_relationships(row):
            ...
            for id in ids:
                ...
                if included_df.loc[id, "attributes.value_type"] == "text":
                    row[ids_columns[column]] = included_df.loc[id, "attributes.text"]
                    ...
                    row[ids_columns[column]] = included_df.loc[
                        id, "attributes.value.invetory_list_item_name"
                    ]
                elif included_df.loc[id, "attributes.value_type"] == "status":
                    row[ids_columns[column]] = included_df.loc[
                        id, "attributes.value.invetory_status_item_name"
                    ]
                elif included_df.loc[id, "attributes.value_type"] == "checklist":
                    row[ids_columns[column]] = included_df.loc[
                        id, "attributes.value.invetory_checklist_item_names"
                    ]
                elif included_df.loc[id, "attributes.value_type"] == "stock":
                    print(included_df.loc[id])
                    row[ids_columns[column]] = included_df.loc[
                        id, "attributes.value.amount"
                    ]
                    row[ids_columns[column + " Units"]] = included_df.loc[
