...
SignalAppLocalMqttOnConnectionCallBackError, SignalAppLocalMqttOConnectionError, \
SignalAppLocalMqttPublishError, SignalAppLocalMqttSubscribeError
...
class LocalMqtt():
    ...
    def remove_event_callbacks(self, topic):
        if topic in self.event_call_backs:
            del self.event_call_backs[topic]
    ...
    def on_message_received(self, client, userdata, message):
        ...
        try:
            if topic in self.event_call_backs:
                logging.info(f"{__name__}: signalsdk: calling event callbacks for topic: {topic}")
                for callback in self.event_call_backs[topic]:
                    logging.info(f"{__name__}: signalsdk: calling event "
                                 f"callback: {callback} and payload: {payload}")
                    callback(payload)
            else:
                logging.info(f"{__name__}: signalsdk: no callback for topic:{topic}")
        except Exception as e:
            logging.info(f"{__name__}: signalsdk: on_message_received callback error: {e}")
            raise SignalAppLocalMqttEventCallBackError from e
    ...
    def on_disconnect(self, client, userdata, rc):
        logging.warning(f"{__name__}: signalsdk::local mqtt disconnecting reason: {str(rc)}")

    def get_client(self):
        return self.mqtt_client

    def connect(self, callback):
        logging.info(f"{__name__}: signalsdk: connect...")
    ...
    def publish(self, topic, payload=''):
        if topic:
            ...
            if result == MQTT_ERR_SUCCESS:
                logging.info(f"{__name__}: signalsdk::localmqtt published topic: {topic}")
            else:
                logging.error(f"{__name__}: signalsdk::localmqtt Failed to publish topic: {topic}")
                raise SignalAppLocalMqttPublishError
        else:
            logging.error(f"{__name__}: signalsdk::localmqtt Failed to publish undefined topic")

    def subscribe(self, topic, is_event_topic):
        if topic:
            result, _ = self.mqtt_client.subscribe(topic)
            if result == MQTT_ERR_SUCCESS:
                logging.info(f"{__name__}: signalsdk::localmqtt subscribed topic: {topic}")
            else:
                logging.error(f"{__name__}: signalsdk::localmqtt "
    ...
    def unsubscribe(self):
        if self.subscribed_topic:
            ...
            if result == MQTT_ERR_SUCCESS:
                logging.info(f"{__name__}: signalsdk::localmqtt "
