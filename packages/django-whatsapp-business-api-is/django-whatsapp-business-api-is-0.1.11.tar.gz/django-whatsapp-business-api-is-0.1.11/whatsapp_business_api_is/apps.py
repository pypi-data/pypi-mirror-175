...
class WhatsappBusinessApiIsConfig(AppConfig):
    ...
    def ready(self):
        try:
            self.incoming_parser = import_string(Conf.INCOMING_PARSER)
            assert callable(self.incoming_parser)
        except AssertionError:
            raise ValueError(f'Conf.INCOMING_PARSER is not a function. Got: {Conf.INCOMING_PARSER}')

        if Conf.SET_STATE:
            try:
                self.set_state = import_string(Conf.SET_STATE)
                assert callable(self.set_state)
            except AssertionError:
