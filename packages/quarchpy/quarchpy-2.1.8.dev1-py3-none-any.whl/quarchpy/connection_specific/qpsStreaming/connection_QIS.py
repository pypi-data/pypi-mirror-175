...
class QisInterface:
    ...
    def startStreamThreadQPS(self, module, fileName, fileMaxMB, streamName, streamAverage, releaseOnData, separator):
        ...
        self.create_dir_structure(fileName)
        ...
        while isRun:
            try:
                ...
                #                 maxFileStatus = self.streamBufferStatus(device=module, sock=self.streamSock)
                #                 maxFileExceeded = True
                #                 self.deviceDict[module][0:3] = [True, 'Stopped',
                #                                                 'User defined max filesize reached']
                #             break
                #         # time.sleep(0.01) #reduce speed of loop to stop spamming qis
                #         streamOverrun, removeChar, newStripes = self.streamGetStripesText(self.streamSock, module,
                #                                                                           numStripesPerRead,
                ...
                #     if maxFileExceeded:
                #         logging.warning(
                #             'Max file size exceeded. Some data has not been saved to file: ' + maxFileStatus + '.')
                #
                # # printText('Stripes in buffer now: ' + self.streamBufferStatus(device=module, sock=self.streamSock))
                #
                # if streamOverrun:
...
#            time.sleep(0.001)
    ...
    def create_dir_structure(self, directory=None):
        """
        ...
        f = open(os.path.join(self.qps_record_dir_path, "annotations.xml"), "w")
        f.close()
        f = open(os.path.join(self.qps_record_dir_path, "data000.idx"), "wb")
        f.close()
        f = open(os.path.join(self.qps_record_dir_path, "notes.txt"), "w")
        f.close()
        f = open(os.path.join(self.qps_record_dir_path, "triggers.txt"), "w")
        f.close()
    ...
    def create_index_file(self):
        ...
        tree = ET.ElementTree(ET.fromstring(self.qps_stream_header[:-1]))
