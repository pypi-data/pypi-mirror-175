    ...
    try:
        if os.path.isfile(pdf_path):
            if pdf_path.endswith(".pdf") or  pdf_path.endswith(".PDF"):
                    ...
                    allpagecleaned=[clean_text_(i) for i in textpages]
...
def clean_text_(pagewise):
    lines=pagewise.splitlines()
    j=[]
    for i in lines:
        if i.strip()!="":
            new_string = i.encode('ascii',errors='ignore').decode()
            j.append(new_string.strip())
    cleanedlines=cleanedlines=list(map(partial(clean, stemming=False,stopwords=False,clean_all=False),j))
    spell = Speller(lang='en',fast=True,only_replacements=True)
    outputcleanedlines=[]
