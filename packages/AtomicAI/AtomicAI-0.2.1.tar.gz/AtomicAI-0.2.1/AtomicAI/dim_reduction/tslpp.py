...
def tslpp(features, reduced_dim):
    ...
    knn = 7
    data, outfile_labels = perform_lpp(
        features,
...
def perform_lpp(
   ...
   #PseudoF_max = 0.0
    ...
    for sigma in sigma_list:
        ...
        X_final_lpp, Y_final_lpp = compute_lpp(X, graph_nearest_neighbors, sigma, 100)
        X_final_tslpp, Y_final_tslpp = compute_lpp(X_final_lpp, graph_nearest_neighbors, sigma, number_of_dimensions)
   ...
   #lpp_projection = np.transpose(X_final_lpp)
    ...
    return np.array(lpp_projection_for_all_sigmas), outfile_labels
