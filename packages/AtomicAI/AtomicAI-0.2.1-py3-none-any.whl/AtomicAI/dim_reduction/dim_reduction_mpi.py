...
def mpi_work(variables):
    in_dir, out_directory, reduced_dim, dim_reduc_model, descriptor, ra, rd, count = variables
    outfile_labels = []
    outfile = f'{out_directory}{str(reduced_dim)}D_{dim_reduc_model}_{descriptor}.dat'
    print(f'Running {outfile[:-4]}')
    for a in ra:
        for d in rd:
            try:
                des_file = f'{descriptor}_{d}_{a}_Si_Si.dat'
                selected_descriptors = select_descriptors_and_manual_labels(in_dir+des_file, count)
            except:
...
def dim_reduction_mpi():
    ...
    ra = [1.0] + rd
    descriptors = ['SOAP']#['G2', 'G2G4', 'SOAP']
    dim_reductions = [2]
    dim_reduc_models = ['PCA', 'TsNE', 'UMAP', 'LPP', 'TsLPP']
    count = 1 
    ...
    for reduced_dim in dim_reductions:
        for dim_reduc_model in dim_reduc_models:
            for descriptor in descriptors:
                ...
                variables = (in_dir, out_directory, reduced_dim, dim_reduc_model, descriptor, ra, rd, count)
                jobs.append(pool.apply_async(mpi_work, args=(variables,)))
