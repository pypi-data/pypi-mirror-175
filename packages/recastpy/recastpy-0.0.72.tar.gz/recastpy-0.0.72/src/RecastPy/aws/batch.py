...
def get_job_details(job_ids: list, include_array_job_parents: bool = True, include_array_job_children: bool = False,
    ...
    if len(job_ids) > max_job_ids:
        jobs = []
        job_ids_lists = [job_ids[i * max_job_ids:(i + 1) * max_job_ids] for i in range((len(job_ids) + max_job_ids - 1) // max_job_ids)]
        for job_ids_list in job_ids_lists:
            jobs.extend(get_job_details(
                job_ids=job_ids_list,
                include_array_job_parents=include_array_job_parents,
                include_array_job_children=include_array_job_children,
                region=region
            ))
        return jobs
...
if __name__ == '__main__':
    ...
    jobs = get_jobs()
    job_details = get_job_details([j['jobId'] for j in jobs])
