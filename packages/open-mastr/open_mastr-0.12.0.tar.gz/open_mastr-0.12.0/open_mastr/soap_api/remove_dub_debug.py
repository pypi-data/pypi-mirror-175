...
for basic_units_chunk in basic_units:
    ...
    basic_units_chunk_unique = [
        # TODO: check if EinheitenMastrNummer is double in list of current chuck (chunk entries are given as dicts)
        #       essentially check in a list comprehension if EinheitMastrNummer is not doulbe to other EinheitMastrNummer in that chunk
        unit
        for n, unit in enumerate(basic_units_chunk)
        if basic_units_chunk["EinheitMastrNummer"]
           not in [_["EinheitMastrNummer"] for _ in basic_units_chunk]
    ]
