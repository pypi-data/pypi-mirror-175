...
class File_utils():
	...
	@staticmethod
	def getDirectoryContents(path, returnType='files', exclude=[], filetypes=[], recursive=False, topdown=True, stripExtension=False):
		...
		types = [t.strip().rstrip('s') for t in returnType.split('|')] #strip any whitespace and trailing 's' of the types to allow for singular and plural to be used interchagably. ie. files | dirs becomes [file, dir]
		...
		for root, dirs, files in os.walk(path, topdown=topdown):
			...
			files[:] = [f.rstrip('.'+File_utils.formatFilepath(f, 'ext')) if stripExtension else f 
						for f in files 
							if f not in exclude and (File_utils.formatFilepath(f, 'ext') in Iter_utils.makeList(filetypes) if filetypes else True)] #remove any files in 'exclude', or not matching a given filetype.
			if 'file' in types:
