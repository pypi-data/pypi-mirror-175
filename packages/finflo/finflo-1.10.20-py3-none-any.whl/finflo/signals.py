...
def create(sender, instance, **kwargs):
        ...
        obj2 , created  = workflowitems.objects.update_or_create(transitionmanager = obj ,model_type = a.upper() , event_user = get_current_user() , initial_state = states.description ,interim_state = states.description, final_state = states.description)
        workevents.objects.update_or_create(workflowitems = obj2 , type = a.upper() , event_user = get_current_user() , record_datas = gets_transition.get_record_datas(), initial_state = states.description ,interim_state = states.description,  final_state = states.description)
