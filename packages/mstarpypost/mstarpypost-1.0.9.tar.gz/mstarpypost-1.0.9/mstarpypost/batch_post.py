...
def CreateAllBatchConfig(case_dir="", do_slice=False, do_volume=False, do_particle=False, do_body=False, exclude_rare=True, range_last_only=True, always_show_particles=False) -> BatchPostConfig:
	"""
	...
	if do_slice:
		...
		proto.video_options.enabled = True		
		...
		for src in srcs:
			if src.time_group == DataTimeEnum.slice:
				if src.data_type == DataTypeEnum.walls:
					...
					proto.moving_pipelines.append(MovingBodyPipeline(file=src.filename, color_by=ColorByEnum.solid_color))
				elif always_show_particles and src.data_type == DataTypeEnum.particles:
					proto.particle_pipelines.append(ParticlesPipeline(file=src.filename, color_by=ColorByEnum.solid_color))
		...
		for p in conf.plots:			
			...
			for variable in p.variables:
				...
				for t in plot_times:
					...
					print("Processing: %s - %s - t=%g" % (p.name, variable.name, t) )
