...
def fisher(adata, conditions, condition_a, condition_b):
    ...
    -------
    adata: anndata object
        Original anndata object with six new added columns in adata.var. Columns correspond to 1) fraction of non-zero cells in condition_a, 2) fraction of non-zero cells in condition_b, and 3) Fisher p-value. If Fisher p-value was calculated before it will be overwritten with new values.
    """
    
    x = adata[adata.obs[conditions] == condition_a].X
    y = adata[adata.obs[conditions] == condition_b].X
...
def ks(adata, conditions, condition_a, condition_b, gene, layer = None):
    ...
    d1 = adata[adata.obs[conditions] == condition_a, gene]
    d2 = adata[adata.obs[conditions] == condition_b, gene]
