...
def scvi(model, conditions, condition_a, condition_b, iterations = 50, filter = True, lfc = 1, bayes_factor = 2.5, pval = 0.01, non_zeros_proportion_b = 0.3, layer = 'scvi_normalized'):
    ...
    non_zeros_proportion_b: float
        Min proportion of non zero cells for a gene in condition_b (usually mutant) to be considered as differentially expressed. Default is 0.3
    ...
    a = (1 - scvi_de['non_zeros_proportion1'])*model.adata[model.adata.obs[conditions] == condition_b].obs.shape[0]
    b = (1 - scvi_de['non_zeros_proportion2'])*model.adata[model.adata.obs[conditions] == condition_a].obs.shape[0]
    c = scvi_de['non_zeros_proportion1']*model.adata[model.adata.obs[conditions] == condition_b].obs.shape[0]
    d = scvi_de['non_zeros_proportion2']*model.adata[model.adata.obs[conditions] == condition_a].obs.shape[0]
    ...
    if layer is not None:
        d1 = model.get_normalized_expression(library_size = 10e4)[model.adata.obs[conditions] == condition_b][scvi_de.index]
        d2 = model.get_normalized_expression(library_size = 10e4)[model.adata.obs[conditions] == condition_a][scvi_de.index]
    else:
        d1 = model.get_normalized_expression(library_size = 10e4)[model.adata.obs[conditions] == condition_b][scvi_de.index]
        d2 = model.get_normalized_expression(library_size = 10e4)[model.adata.obs[conditions] == condition_a][scvi_de.index]
