...
from aicc_tools.utils.monitor import LossMonitor, CheckpointMointor, ProfileMonitor
from aicc_tools.utils.cloud_adapter import Obs2Local, Local2ObsMonitor, mox_adapter
...
class CFTS:
    ...
    def _send_file(self, src_url, obs_url):
        ...
        if src_url is not None and obs_url is None:
            Validator.check_type(src_url, str)
            src_url = format_path(src_url)
            _final = src_url.split('/')[-1]
            if self.special_id is not None:
                target_dir = os.path.join(self.obs_path, 'rank_{}'.format(self.special_id), _final)
                mox_adapter(src_url, target_dir, log=self.log)
            else:
                if self.rank_id % 8 == 0:
                    target_dir = os.path.join(self.obs_path, 'rank_{}'.format(self.rank_id), _final)
                    mox_adapter(src_url, target_dir, log=self.log)
