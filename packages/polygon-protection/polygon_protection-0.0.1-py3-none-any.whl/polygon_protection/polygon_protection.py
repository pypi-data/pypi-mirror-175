...
class PolygonProtection:
    """
    A class to protect apiKey of polygon access with related used function in main function

    WARNING: In the future, the function named main() should be changed to another, since it is informal name
    """
    def __init__(self):
        pass
    ...
    # This creates a table for storing the (6 min interval)
    # aggregated price data for each currency pair in the SQLite database
    def initialize_aggregated_tables(self, engine, currency_pairs):
        with engine.begin() as conn:
            for curr in currency_pairs:
                conn.execute(text(
                    "CREATE TABLE " + curr[0] + curr[
    ...
    def aggregate_raw_data_tables(self, engine, currency_pairs):
        with engine.begin() as conn:
            for curr in currency_pairs:
                result = conn.execute(
                    text("SELECT AVG(fxrate) as avg_price, COUNT(fxrate) as tot_count FROM " + curr[0] + curr[
                        1] + "_raw;"))
                for row in result:
                ...
                # exec("print(\"The average standard deviation of the return for
                # "+curr[0]+curr[1]+" is:"+str(curr_avg_std)+" \")")
    ...
    # Function which clears the raw data tables once we have aggregated the data in a 6 minutes interval
    ...
    def main(self, currency_pairs):
        # The api key given by polygon official website - Stefan Su's apiKey
        key = "sYowzPXBxPFSIZWn9ilAyyceYwFAQIr4"
