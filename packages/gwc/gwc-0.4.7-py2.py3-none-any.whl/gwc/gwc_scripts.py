...
from gwc.common.excel_loader import ExcelLoader
...
def _get_gwl_workflow(workflow_name, version, output, account=None, out_type="dir"):
    """下载工作流配置文件
    ...
    if not os.path.exists(output) and out_type == "dir":
        os.makedirs(output)
    ...
    outpath = os.path.join(output, file_name) if out_type == "dir" else output
...
def pull_all_conf(args):
    ...
    (nodelists, account_name) = _get_gwl_workflow(args.name, workflow_version, workflow_config_path, out_type="file")
...
class TasksApp(object):
    ...
    def __init__(self, parser):
        ...
        subparser_task_restart = subparser_task.add_parser('restart', help='Restart task(only wdl)')
        subparser_task_restart.add_argument('-i', '--taskid', required=True, help='task id')
        subparser_task_restart.add_argument('-r', '--restartconfig', help='restart config file path')
        subparser_task_restart.set_defaults(func=self.restart_task)
        ...
        subparsers_task_getjobs.add_argument('-s', '--structured', action='store_true', help="show jobs structured")
    ...
    def _get_job_runner(cls, args, res_account_name=None):
        ...
        runner_type = task_info.get("type", "gwl")
        if runner_type != 'wdl':
            runner_type = 'gwl'
    ...
    def restart_task(cls, args):
        ...
        Args:
            ...
            args.taskid: 指定task id
            args.restartconfig: 重启的job参数文件
        """
        runner = cls._get_runner(args)
        task = runner.tasks
        task_id = args.taskid
        restart_config_file_path = args.restartconfig
        if restart_config_file_path:
            if not os.path.exists(restart_config_file_path):
                print("restart config file {} don't exists".format(restart_config_file_path), file=sys.stderr)
                sys.exit(1)
            debug_params = gdpy.yml_utils.yaml_loader(restart_config_file_path)
        else:
            debug_params = []
        try:
            task.restart_task(task_id, debug_params=debug_params)
        except gdpy.exceptions.ServerError as e:
            if e.status // 100 == 4:
                print('Please check the input "task id" or "task type".', file=sys.stderr)
                sys.exit(1)
                ...
                raise e
        _logger.info('Restart task successfully! task id: {}'.format(task_id))
        print('Restart task successfully! task id: {}'.format(task_id), file=sys.stdout)

    @classmethod
    def _get_jobs_list(cls, runner, task_id, structured=False):
        """
        ...
        if runner.runner_type == 'wdl':
            ...
            for t in sub_tasks:
                sub_task_jobs = cls._get_jobs_list(runner, t['task_id'], structured=structured)
                if structured:
                    result["tasks"].append(sub_task_jobs)
                else:
                    jobs.extend(sub_task_jobs)
        try:
        ...
        def cast_job_info(job_info):
            ...
            # wdl use id
            job_info['job_id'] = job_info.get('id') or job_info.get('job_id')

        for item in resp.jobs:
            cast_job_info(item)

        if structured:
            result["jobs"] = resp.jobs
            return result
        return resp.jobs + jobs

    @classmethod
    @utility.catch_exceptions(retry_times=RETRY_TIMES)
    def _print_jobs_info(cls, runner, task_id, quiet, structured):
        """
        ...
        :return:
        """
        task_all_info = cls._get_jobs_list(runner, task_id, structured=structured)
        if structured and not quiet:
            def print_table(task_info, indent=0):
                jobs = task_info["jobs"]
                table = json2table(jobs, None, ["job_id", "job_name", "status", "startTime", "endTime"], 1,
                                   reload_data=False)
                print(" " * 6 * indent + "|-- tasks  " + task_info["task_id"])
                head = True
                _logger.info('Get jobs info successfully! task_id: %s.', task_id)
                for item in table.get_string(start=0).split("\n"):
                    if item.strip():
                        if head:
                            print(" " * (6 * indent + 6) + "|-- jobs  " + item)
                            head = False
                        else:
                            print(" " * (6 * indent + 11) + "|--  " + item)
                for sub_task in task_info["tasks"]:
                    print_table(sub_task, indent + 1)

            print_table(task_all_info)
        else:
            table = json2table(task_all_info, None, ["job_id", "job_name", "status", "startTime", "endTime"], 1,
                               reload_data=False)
            if quiet:
                table.header = False
    ...
    def get_jobs_info(cls, args):
        ...
        cls._print_jobs_info(runner, task_id, quiet, structured)
...
class WorkflowApp(object):
    ...
    def __init__(self, parser):
        ...
        # todo 把校验放在函数里， name和batch file至少要有一个
        subparser_workflow_run.add_argument('-n', '--name', help='workflow name')
        subparser_workflow_run.add_argument('-p', '--parameters', help='workflow parameters file')
        ...
        subparser_workflow_run.add_argument(
            "-b", "--batch_run_file",
            help="batch run workflow info file, the type is xlsx, only support wdl workflow"
        )
...
class BatchWorkflowApp(WorkflowApp):
    def __init__(self, parser):
        batch_workflow_sub_parser = parser.add_parser('batch', description='Batch Workflow operations', help='')
        parser_workflow = batch_workflow_sub_parser.add_subparsers()
        subparser_workflow_run = parser_workflow.add_parser('run', help='run batch workflow')
        subparser_workflow_run.set_defaults(func=self.run_workflow_task_batch)
        subparser_workflow_run.add_argument(
            "-i", "--input",
            help="batch run workflow info file, the type is xlsx, only support wdl workflow"
        )

    def run_workflow_task_batch(self, args):
        excel_loader = ExcelLoader(args.input)
        for index in range(excel_loader.get_sheet_num()):
            workflow_name = excel_loader.get_sheet_name(index)
            print("run batch workflow: {}".format(workflow_name))
            for row, task_params in excel_loader.data(sheet_index=index):
                try:
                    run_workflow_parmas = {}
                    run_workflow_parmas["name"] = workflow_name
                    workflow_version = task_params.pop("workflow_version", None)
                    run_workflow_parmas["version"] = int(workflow_version) if workflow_version is not None else None
                    run_workflow_parmas["account"] = task_params.pop("resource_account", None) or None
                    run_workflow_parmas["task"] = task_params.pop("task_name", None) or None
                    keep_output_structure = task_params.pop("keep_output_structure", None)
                    if keep_output_structure is not None:
                        if isinstance(keep_output_structure, str):
                            keep_output_structure = str2bool(keep_output_structure)
                    else:
                        keep_output_structure = True
                    run_workflow_parmas["keep_output_structure"] = keep_output_structure
                    run_workflow_parmas["remote_output_dir"] = task_params.pop("output_dir", None) or None
                    run_workflow_parmas["inputs"] = {}
                    for key, value in task_params.items():
                        try:
                            value = json.loads(value)
                        except Exception:
                            # todo 只是为了转换结构，如果发现处理不了的数据，再进行优化
                            _logger.warning("a error occurred while processing the value {}".format(str(value)))
                        run_workflow_parmas["inputs"][key] = value
                    run_workflow_parmas["parameters"] = None
                    run_workflow_args = argparse.Namespace(**run_workflow_parmas)
                    self.run_workflow_task(run_workflow_args)
                except Exception as e:
                    print("failed run task, sheet: {} row: {}".format(workflow_name, str(row + 1)))
                    _logger.exception("failed run task, sheet: {} row: {}".format(workflow_name, str(row + 1)))
