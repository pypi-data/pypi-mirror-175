...
class hmm_model:
    ...
    # self.train_log
    def fit(self,data,a0=0,b0=0,pi0=0,step=0,e=0.001,core=None,yeh=False):
        ...
        if step==0:
            a,b,initial,f=train(data,self.hidden_state,self.obs_state,A,B,pi,step,e,core,yeh)
            self.initial=initial[len(initial)-1]
            self.transition=a[len(a)-1]
            self.emission=b[len(b)-1]
            return a,b,initial,f
        else:
            a,b,initial,f,log=train(data,self.hidden_state,self.obs_state,A,B,pi,step,e,core,yeh)
            self.transition=a[len(a)-1]
            self.emission=b[len(b)-1]
            self.initial=initial[len(initial)-1]
            self.train_log=log
