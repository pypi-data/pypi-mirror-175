...
# j: index of z
def ph_seq_mis(obs,A,B,pi,j,hidden_state,obs_state,yeh=False):
    if not yeh:
        indexer=np.where(obs=='None')[0]
        alph,bet=us_alpha_and_beta(obs,A,B,pi,hidden_state,obs_state)
        temp=alph[indexer,j]*bet[indexer,j]
        out=sum(temp)*B[j,:]
        #out=np.array([0 for i in range(0,len(obs_state))])
    else:
        out=np.array([0 for i in range(0,len(obs_state))])
...
def update_B(A,B,pi,data,hidden_state,obs_state,prob,p,yeh=False):
    h=hidden_state
    o=obs_state
    new_B=B.copy()
    ...
    for j in range(0,B.shape[0]):
        ...
        propose2=p.starmap(ph_seq_mis,[(data[r],A,B,pi,j,h,o,yeh) for r in range(0,data.shape[0])])
        #prob=p.starmap(p_obs,[(data[r],A,B,pi,h,o) for r in range(0,data.shape[0]) ])
...
def EMTrain(A,B,pi,data,e,hidden_state,obs_state,p,yeh=False):
    pi=pi
    A=A
    B=B
    h=hidden_state
    ...
    new_B=update_B(A,B,pi,data,h,o,prob,p,yeh)
...
def EMTrain1(A,B,pi,data,e,step,hidden_state,obs_state,p,yeh=False):
    pi=pi
    A=A
    B=B
    h=hidden_state
...
def train(data,hidden_state,obs_state,a0=0,b0=0,pi0=0,step=0,e=0.001,core=None,yeh=False):
    if not core:
        p=Pool(mp.cpu_count())
    else:
        p=Pool(core)
    ...
    if step==0:
        a_trace,b_trace,pi_trace,f=EMTrain(A,B,pi,data,e,hidden_state,obs_state,p,yeh)
        return a_trace,b_trace,pi_trace,f
    else:
        a_trace,b_trace,pi_trace,f,log=EMTrain1(A,B,pi,data,e,step,hidden_state,obs_state,p,yeh)
