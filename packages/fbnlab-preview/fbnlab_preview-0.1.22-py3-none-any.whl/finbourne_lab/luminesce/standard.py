...
class StandardMeasurementSet(LuminesceMeasurementFactory):
    ...
    def _reader_test(self, reader, x_rng, category, name, n_parallel_set, n_columns=None, check_period=0.1, keep=90):
        ...
        for n_p in n_parallel_set:
            c = Convener(e, self.work_dir + f'/{category}', name + f'_NP{n_p}', n_parallel=n_p)
            conveners.append(c)

        return conveners

    def _file_read_test(self, reader, test_data_path, x_rng, name, n_parallel_set, check_period=0.1, keep=90):
        ...
        conveners = []
        for n_p in n_parallel_set:
            c = Convener(e, self.work_dir + '/readers', f'{name}_NP{n_p}', n_parallel=n_p)
            conveners.append(c)

        return conveners

    def _join_test(self, reader, x_rng, m, name, n_parallel_set, check_period=0.1, keep=90):
        ...
        for n_p in n_parallel_set:
            ...
            c = Convener(e, work_dir, name + f'_NP{n_p}', n_parallel=n_p)
            c_base = Convener(e_base, work_dir, name + f'_NP{n_p}_baseline', n_parallel=n_p)
            convener_pairs.append((c_base, c))

        return convener_pairs

    def _file_write_test(self, writer, file_type, x_rng, n_cols, name, n_parallel_set, check_period=0.1, keep=90):
        ...
        for n_p in n_parallel_set:
            baseline_cv = Convener(baseline_ex, self.work_dir + '/writers', f'{name}_cols{n_cols}_NP{n_p}_baseline', n_parallel=n_p)
            writer_cv = Convener(writer_ex, self.work_dir + '/writers', f'{name}_cols{n_cols}_NP{n_p}', n_parallel=n_p)
    ...
    def excel_read_measurement(self, **kwargs):
        ...
        return self._file_read_test(reader, path, x_rng, 'excel_read', n_parallel_set, keep=keep)
    ...
    def csv_read_measurement(self, **kwargs):
        ...
        return self._file_read_test(reader, path, x_rng, 'csv_read', n_parallel_set, keep=keep)
    ...
    def excel_write_measurement(self, **kwargs):
        ...
        return self._file_write_test(writer, 'Excel', x_rng, n_cols, 'excel_write', n_parallel_set, keep=keep)
    ...
    def csv_write_measurement(self, **kwargs):
        ...
        writer = self.atlas.drive_saveas
        return self._file_write_test(writer, 'Csv', x_rng, n_cols, 'csv_write', n_parallel_set, keep=keep)
    ...
    def view_measurement(self, **kwargs):
        ...
        baselines = self._reader_test(t10m, x_rng, 'core', f'select_{n_columns}', n_parallel_set, n_columns, check_period, keep=keep)
        view_tests = self._reader_test(t10m_view, x_rng, 'core', f'select_view_{n_columns}', n_parallel_set, n_columns, check_period, keep=keep)
    ...
    def join_measurement(self, **kwargs):
        ...
        return self._join_test(t10m, x_rng, n_columns, 'join' + f'_{n_columns}', n_parallel_set, check_period, keep=keep)