...
class DeepLParameters(ProcessorParameters):
    target_lang: TargetLang = Field(None, description="""The language into which the text should be translated. Options currently available:</br>
                        <li>`DE` - German
    ...
    formality: Formality = Field(Formality.default, description="""Sets whether the translated text should lean towards formal or informal language. This feature currently only works for target languages DE (German), FR (French), IT (Italian), ES (Spanish), NL (Dutch), PL (Polish), PT-PT, PT-BR (Portuguese) and RU (Russian). Setting this parameter with a target language that does not support formality will fail, unless one of the prefer_... options are used. Possible options are:</br>
                        <li>`default` - default
        ...
        try:
            for document in documents:
                ...
                if document.sentences:
                    ...
                    trans_texts = [deepl_translate(lang, params.target_lang.value, stext, False) for stext in stexts]
                    trans_text = "\n".join(trans_texts)
                else:
                    trans_text = deepl_translate(lang, params.target_lang.value, document.text, False)
