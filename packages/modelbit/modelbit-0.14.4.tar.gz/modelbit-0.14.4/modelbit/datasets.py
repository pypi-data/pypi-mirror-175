...
def _getWithDb(dsName: str, filters: Union[Dict[str, List[Any]], None] = None):
  ck = _cacheKey(dsName)
...
def _getWithCsv(dsName: str, filters: Union[Dict[str, List[Any]], None] = None):
  ck = _cacheKey(dsName)
...
def _dbFromPklBytes(stream: Union[bytes, None]):
  if stream is None:
    return None
...
def _filterDataframe(df: pandas.DataFrame, filters: Union[Dict[str, List[Any]], None]):
  if filters is None:
    return df
...
def _filterDbToDataframe(db: sqlite3.Connection, filters: Union[Dict[str, List[Any]], None]):
  if filters is None:
