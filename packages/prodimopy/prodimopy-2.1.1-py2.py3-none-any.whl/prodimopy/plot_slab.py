    ...
    if overplot:
        for i,d in enumerate(dat):
            ...
            if NLTE:
                ...
                add_flux=d.convLTEflux
            fig_list[0],ax_list[0] = _basic_spectra_plot(add_wave, add_flux,convR= dat[0].convR,sampling=sampling, scaling=scaling_list[i], offset=offset_list[i],  normalise=normalise, fig=fig_list[0], ax=ax_list[0], style=style, figsize=figsize, label=label_list[i], color=color_list[i],lw=lw, where='mid')
        ...
        for i,d in enumerate(dat):
            ...
            if NLTE:
                ...
                add_flux=d.convLTEflux
            fig_list[i],ax_list[i] = _basic_spectra_plot(add_wave, add_flux,convR= dat[0].convR,sampling=sampling, scaling=scaling_list[i], offset=offset_list[i],  normalise=normalise, fig=fig_list[i], ax=ax_list[i], style=style, figsize=figsize, label=label_list[i], color=color_list[i],lw=lw, where='mid')
        if axx_len>1:
            return(fig_list[0],np.array(ax_list))
        else:
            return([(fig,ax) for (fig,ax) in zip(fig_list,ax_list)])
