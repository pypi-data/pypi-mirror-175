...
async def present(
    ...
    if resource_id:
        resource = await hub.tool.boto3.resource.create(
            ctx, "s3", "Bucket", resource_id
        )
        before = await hub.tool.boto3.resource.describe(resource)
    if before:
        convert_ret = await hub.exec.aws.s3.bucket.get(
        ...
        if not result["result"]:
            ...
            return result
        result["old_state"] = copy.deepcopy(convert_ret["ret"].get("ret"))
    ...
    if ctx.get("test", False):
        ...
        if not result["result"]:
            result["comment"] = result["comment"] + convert_ret["comment"]
        result["new_state"] = copy.deepcopy(convert_ret["ret"].get("ret"))
    else:
...
async def absent(hub, ctx, name: str, resource_id: str = None) -> Dict[str, Any]:
    ...
    if not before:
        ...
        if not result["result"]:
            result["comment"] = result["comment"] + convert_ret["comment"]
            return result
        result["old_state"] = convert_ret["ret"].get("ret")
        if ctx.get("test", False):
