...
class UnityToGymWrapper(gym.Env):
    ...
    def __init__(
        ...
        if self.group_spec.action_spec.is_discrete():
            ...
            assert self.group_spec.action_spec.discrete_size == self.group_spec.action_spec.continuous_size, \
                "Discrete action space is not consistent with the continuous one."
            self.action_size = self.group_spec.action_spec.continuous_size
            self._action_space = spaces.Dict()

            branches = self.group_spec.action_spec.discrete_branches
            if self.group_spec.action_spec.discrete_size == 1:
                self._action_space["discrete"] = spaces.Discrete(branches[0])
            else:
                if flatten_branched:
                    self._flattener = ActionFlattener(branches)
                    self._action_space["discrete"] = self._flattener.action_space
                else:
                    self._action_space["discrete"] = spaces.MultiDiscrete(branches)
                    
            high = np.array([1] * self.group_spec.action_spec.continuous_size)
            self._action_space["continuous"] = spaces.Box(-high, high, dtype=np.float32)
    ...
    def step(self, action: List[Any]) -> GymStepResult:
        ...
        if isinstance(action, dict):
            action = {k: np.array(v).reshape((1, self.action_size)) for k, v in action.items()}
        else:
        ...
        elif self.group_spec.action_spec.is_discrete():
