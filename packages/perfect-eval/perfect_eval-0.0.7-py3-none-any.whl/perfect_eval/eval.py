...
def match(name, condition_value_list: list, default=None):
    for condition, value in condition_value_list:
        if name == condition:
            return value
...
def eval_expr(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False,
    ...
    an expression from an untrusted source.
    ...
    try:
        ...
        raise ValueError(f'%s: "{e}" while evaluating\n{expr}')
