...
def check(
    ...
    if "recreate_on_update" in chunk:
        ...
        is_existing_resource = bool(enforced_state)
        prefix = chunk["recreate_on_update"].get("prefix", {})
        for pre in prefix:
            chunk[pre] = prefix[pre] + str(time.time())
...
def is_recreation_required(hub, name, chunk, enforced_state):
    ...
    # we should ignore the prefix keys also, as the change detection does not depend on prefix keys.
    ignore_keys.extend(list(chunk["recreate_on_update"].get("prefix", {}).keys()))
