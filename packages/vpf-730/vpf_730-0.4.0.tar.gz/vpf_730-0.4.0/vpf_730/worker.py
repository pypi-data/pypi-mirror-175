    ...
    def run(self) -> None:
        ...
        try:
            while self.running is True:
                self.msg_per_second = (
                    self.msg_processed / (datetime.now(tz=timezone.utc) - self.start_time).total_seconds()
                )
                print(f'current speed: {self.msg_per_second:.2f} msg/s (msg in queue: {self.queue.qsize()})')
                if self.queue.empty():
