...
# Note: We can only do offsets in the stdlib in py3.2+, py2 doesnt have a builtin concrete tzinfo class
# and we're not doing 3rd party with pytz or dateutil atm, we are waiting for py3.9 and its tz stuff


@pytest.mark.skipif(sys.version_info < (3, 2), reason="py3.2+ only test")  # python3-only test
def test_dec_dt_offset():
    tzinfo = datetime.timezone(datetime.timedelta(days=-1, seconds=77400))
    dt = datetime.datetime(2020, 1, 16, 13, 37, 20, tzinfo=tzinfo)
    assert decode_sched(SBytes("e0 c8 1f 01 10 0d 25 14 a2"), 0, 9) == dt


@pytest.mark.skipif(sys.version_info < (3, 2), reason="py3.2+ only test")  # python3-only test
def test_dec_dt_offset_sub():
    tzinfo = datetime.timezone(datetime.timedelta(days=-1, seconds=77400))
    dt = datetime.datetime(2020, 1, 16, 13, 37, 20, 12345, tzinfo=tzinfo)
    assert decode_sched(SBytes("e2 c8 1f 01 10 0d 25 14 a2 b9 60"), 0, 11) == dt
...
def test_dec_dt_roundtrip():
    ...
    dt = datetime.datetime(2020, 1, 16, 13, 37, 20, 12345, tzinfo=tzinfo)
    buf = encode_sched(dt)
    assert decode_sched(buf, 0, len(buf)) == dt
