...
class GlobusMixin:
    ...
    def _blocking_globus_transfer(self, transfer_data: TransferData) -> None:
        ...
        while not tc.task_wait(
            ...
            events = [e for e in tc.task_event_list(task_id=task_id)]
            last_event = events[-1]
            log_message = f"Transfer status: {last_event['description']=}, {last_event['details']=}, recipe_run_id={self.recipe_run_id}, {task_id=}"
