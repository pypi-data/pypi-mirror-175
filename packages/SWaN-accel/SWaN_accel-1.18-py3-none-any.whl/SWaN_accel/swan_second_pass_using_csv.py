...
def correctPredictionsSingleDate(in_file):

    oriDF = pd.read_csv(in_file, header=0, skiprows=0, sep=',', compression="infer", quotechar='"',
    ...
    if oriDF.dropna().empty:
        print('No prediction data in the file')
        return

    dir_name = os.path.dirname(in_file)
    fi_name = os.path.basename(in_file).split('.csv')[0]

    outPath = os.path.join(dir_name, fi_name+'_final.csv.gz')
    ...
    final_df = oriDF[['START_TIME', 'STOP_TIME', 'PREDICTED_SMOOTH', 'PROB_WEAR_SMOOTH', 'PROB_SLEEP_SMOOTH', 'PROB_NWEAR_SMOOTH']]
...
def main(in_path=None):
    if (in_path is None):
        print("Need swan first pass output file.")
        return
    ...
    correctPredictionsSingleDate(in_path)