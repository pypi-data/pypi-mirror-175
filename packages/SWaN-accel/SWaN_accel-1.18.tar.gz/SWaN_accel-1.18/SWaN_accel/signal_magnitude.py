...
class EnergyFeature:
    ...
    def smv_norm_energy_sum(self):
        smv_norm_energy_sum = np.nansum(self._energies, axis=0)
        result = vec2rowarr(smv_norm_energy_sum)
        result = add_name(result, self.smv_norm_energy_sum.__name__)
        return result

    def smv_norm_energy_var(self):
        smv_norm_energy_var = np.nanvar(self._energies, axis=0)
        result = vec2rowarr(smv_norm_energy_var)
        result = add_name(result, self.smv_norm_energy_var.__name__)
        return result
