...
def correctPredictionsSingleDate(folder, dStr, mode):
    ...
    for feature_file in sorted(glob(os.path.join(prevFolder, '*/*-AccelerationCalibrated-*.featureOri0to180.csv.gz'))):
    ...
    for feature_file in sorted(glob(os.path.join(thisFolder, '*/*-AccelerationCalibrated-*.featureOri0to180.csv.gz'))):
        odf = pd.read_csv(feature_file, header=0, skiprows=0, sep=',', compression="infer", quotechar='"',
                          parse_dates=['HEADER_TIME_STAMP', 'START_TIME', 'STOP_TIME'],
                          date_parser=mhealth_timestamp_parser)
        oriDF = pd.concat([oriDF, odf], ignore_index=True)
    ...
    for feature_file in sorted(glob(os.path.join(nextFolder, '*/*-AccelerationCalibrated-*.featureOri0to180.csv.gz'))):
        odf = pd.read_csv(feature_file, header=0, skiprows=0, sep=',', compression="infer", quotechar='"',
                          parse_dates=['HEADER_TIME_STAMP', 'START_TIME', 'STOP_TIME'],
                          date_parser=mhealth_timestamp_parser)
        oriDF = pd.concat([oriDF, odf], ignore_index=True)
