...
def correctPredictionsSingleDate(oriDF,outPath):

    if oriDF.empty:
        print("No data found in this file.")
        return

    oriDF['HEADER_TIME_STAMP'] = pd.to_datetime(oriDF['HEADER_TIME_STAMP'],format=mhealth_timestamp_parser)
...
def main(df=None,out_path=None):

    if(os.path.exists(out_path)):
        print("Output file already exists.")
        return

    correctPredictionsSingleDate(df,out_path)